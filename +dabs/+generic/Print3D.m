classdef Print3D < dabs.resources.Device & dabs.resources.widget.HasWidget & most.HasMachineDataFile & dabs.resources.configuration.HasConfigPage
    ...
    methods
        function start(obj)            
            ...
            function startFastPrint()
                ...
                try
                    ...
                    pbs = scanimage.components.beams.PowerBox.empty();
                    ...
                    for idx = 1:numel(zs)
                        ...
                        pb.name = sprintf('Print3D z=%f',zs(idx));
                        pb.mask = obj.imageStack{idx};
                        pb.zs   = zs(idx);
                        
                        pbs(idx) = pb;
                    end
                    
                    obj.hSI.hBeams.powerBoxes = pbs;
        ...
        function nextSlice(obj)
            ...
            % set up power box
            powerbox = scanimage.components.beams.PowerBox();
        ...
        function showImages(obj)
            ...
            hFig = most.idioms.figure(...
                'Name','Ablation Images' ...
                ,'CloseRequestFcn',@(varargin)close() ...
