classdef DataRecorder < dabs.resources.Device & most.HasMachineDataFile & dabs.resources.configuration.HasConfigPage & dabs.resources.widget.HasWidget
    ...
    properties (SetObservable)
        ...
        fileBaseName = 'file';       % [char array] name of file
    ...
    % viewer properties
    properties (SetAccess = private, Hidden)
        hViewer; % reference to DataRecorderView;
    end
    ...
    methods
        ...
        function set.fileBaseName(obj,v)
            ...
            obj.fileBaseName = v;
            obj.acquisitionNumber = obj.getNextFileCounter();
        end
    ...
    methods
        function deinit(obj)
            ...
            most.idioms.safeDeleteObj(obj.hViewer);
        end
        ...
        function reinit(obj)
            ...
            try
                ...
                assert(~isempty(obj.configuration),'no analog input channels specified');
                assert(~isempty(obj.fileBaseName),'filename not specified (see Controls tab)');
                assert(~isempty(obj.fileDirectory),'must have a logging directory specified ("Dir" button)');
                obj.acquisitionNumber = obj.getNextFileCounter();

                for cfg = obj.configuration
                    assert(obj.hDAQ == cfg.hIO.hDAQ,'channels must all be on the same daq');
                    assert(~isempty(cfg.name),'cannot have empty recorded names for channels');
                end
    ...
    %% Viewer and helper methods
    methods
        function startViewer(obj)
            most.idioms.safeDeleteObj(obj.hViewer);
            obj.hViewer = dabs.generic.datarecorder.DataRecorderView(obj);
        end

        function val = getNextFileCounter(obj)
            % search for files in current path that have same filestem,
            % then get filecounter value + 1
            fullName = fullfile(obj.fileDirectory,obj.fileBaseName);
            fileList = dir([fullName '*']);
            fileNames = {fileList.name};
            logFileStem_ = regexptranslate('escape',obj.fileBaseName);
    ...
    methods
        ...
        function resetSampleBuffer(obj)
...
s = [...
    ...
    most.HasMachineDataFile.makeEntry('fileBaseName','file','Base name of the file saved')...
