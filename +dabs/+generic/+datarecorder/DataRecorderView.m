classdef DataRecorderView < matlab.apps.AppBase
    ...
    methods (Access = private)
        function refreshView(app)
            ...
            iVisible = find(strcmp(get(app.AnalogLines, 'Visible'), 'on')) .';
            iVisible = iVisible(iVisible <= numel(app.DataRecorder.analogConfiguration));
        ...
        function refreshTimerFcn(app)
            try
                app.refreshView();
            catch ME
                most.ErrorHandler.logAndReportError(ME,'Data recorder viewer error\nData is still being recorded, but the viewer is stopped');
                app.pollTimer.stop();
            end
        end
        ...
        function refreshCheckboxVisibility(app)
            for iCheckbox = 1:length(app.AnalogPanelRows)
                if app.AnalogPanelRows(iCheckbox).IsChecked
        ...
        function lineVisibilityCheckboxValueChanged(app, index, value)
            ...
            if all([app.AnalogPanelRows.IsChecked])
    ...
    methods (Access = private)
        ...
        function startupFcn(app, DataRecorder)
            ...
            app.pollTimer = timer(...
                'Name', 'Data Recorder Viewer Timer', ...
                'TimerFcn', @(~,~)app.refreshTimerFcn(), ...
        ...
        function SelectAllCheckBoxValueChanged(app, event)
            for row = 1:numel(app.AnalogPanelRows)
                app.AnalogPanelRows(row).IsChecked = app.SelectAllCheckBox.Value;
            end
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            % Create UIAxes
            app.LiveViewAxes = most.idioms.axes(app.Figure);
