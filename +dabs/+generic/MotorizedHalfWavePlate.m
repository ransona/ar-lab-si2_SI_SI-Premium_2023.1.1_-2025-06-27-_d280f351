classdef MotorizedHalfWavePlate < dabs.resources.devices.BeamModulatorSlow & most.HasMachineDataFile & dabs.resources.configuration.HasConfigPage
    ...
    properties (SetObservable)
        ...
        hMotorStateListener = event.listener.empty();
        
        hCalibrationOpenShutters = {};
        
        devUnitsPerDegree = 1;
        outputRange_deg = [0, 45];
    ...
    methods
        ...
        function delete(obj)
            obj.deinit();
            most.idioms.safeDeleteObj(obj.hMotorStateListener);
            obj.saveCalibration();
        end
        ...
        function deinit(obj)
            try
                if most.idioms.isValidObj(obj.hMotor) 
                    if isempty(obj.hMotor.errorMsg)
                ...
                obj.errorMsg = sprintf('deinitialization error: %s',ME.message);
        ...
        function reinit(obj)
            ...
            try
                ...
                obj.hMotor.reinit();
                assert(isempty(obj.hMotor.errorMsg),'Motor has error message: %s',obj.hMotor.errorMsg);
                ...
                obj.hListeners(end+1) = most.ErrorHandler.addCatchingListener(obj.hMotor,'ObjectBeingDestroyed',@(varargin)obj.deinit);
                most.idioms.safeDeleteObj(obj.hMotorStateListener);
                obj.hMotorStateListener = most.ErrorHandler.addCatchingListener(obj.hMotor,'errorMsg','PostSet',@obj.matchMotorInitState);
    ...
    methods
        ...
        function matchMotorInitState(obj,varargin)
            if ~isempty(obj.hMotor.errorMsg)
                motorErrorMsg = obj.hMotor.errorMsg;
                obj.deinit();
                obj.errorMsg = sprintf('Motor Error: %s',motorErrorMsg);
            end
        end
    end
    ...
    methods
        ...
        function setPowerFractionAsync(obj, fraction)
            assert(isempty(obj.errorMsg),'Cannot set power; Device is in an error state.');
    ...
    methods
        function setLastKnownPowerFraction(obj)
            ...
            pf = interp1_extended(lut(:,2), lut(:,1),obj.hMotor.lastKnownPosition(obj.motorAxis),'linear','extrap');

            %For a halfwave plate, power should vary by sin^2(2*theta)
            if pf > 1 || pf < 0
                scale_degPerUnit = 45/(max(lut(:,2)) - min(lut(:,2)));
                offset_deg = lut(1,2) * scale_degPerUnit;
                theta = obj.hMotor.lastKnownPosition*scale_degPerUnit - offset_deg;
                pf = sind(2*theta).^2;
            end
        ...
        function processCalibrationData(obj,angle_devUnits,feedback_V)
            ...
            function plotFailedCalibration(angle_deg,feedbackV,idx1,idx2)
                hFig = most.idioms.figure('Name', sprintf('%s: Calibration failed', obj.name));
        ...
        function plotLUT(obj)            
            ...
            hFig = most.idioms.figure('Name', figureName,'CloseRequestFcn',@closeFigure);
    ...
    methods
        ...
        function set.outputRange_deg(obj,val)
            validateattributes(val,{'numeric'},{'size',[1,2],'real','nonnan'});
...
s = [...
    ...
    most.HasMachineDataFile.makeEntry('outputRange_deg',[0 45],'Control angular range in degrees')...
    ...
    most.HasMachineDataFile.makeEntry('calibrationNumPoints',100,'number of equidistant points to measure within the angular output range')...
