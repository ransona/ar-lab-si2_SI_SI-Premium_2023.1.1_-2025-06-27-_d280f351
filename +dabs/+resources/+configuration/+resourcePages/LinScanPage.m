classdef LinScanPage < dabs.resources.configuration.ResourcePage
    ...
    methods
        ...
        function makePanel(obj,hParent)
            ...
            if most.idioms.isValidObj(obj.hResource.hDAQAux)
                auxDAQName = ['/' obj.hResource.hDAQAux.name '/'];
            else
                auxDAQName = '';
            end
        
                most.gui.uicontrol('Parent',hTab,'Style','text','RelPosition', [51 85 100 20],'Tag','txSampleClock','String','Pixel Clock','HorizontalAlignment','right');
                obj.pmPixelClock = most.gui.uicontrol('Parent',hTab,'Style','popupmenu','RelPosition', [180 82 160 20],'Tag','pmPixelClock','Enable','On','String',{''});

                most.gui.uicontrol('Parent',hTab,'Style','text','RelPosition', [51 110 100 20],'Tag','txLineClock','String','Line Clock','HorizontalAlignment','right');
                obj.pmLineClock = most.gui.uicontrol('Parent',hTab,'Style','popupmenu','RelPosition', [180 107 160 20],'Tag','pmLineClock','Enable','On','String',{''});

                most.gui.uicontrol('Parent',hTab,'Style','text','RelPosition', [51 135 100 20],'Tag','txFrameClock','String','Frame Clock','HorizontalAlignment','right');
                most.gui.uicontrol('Parent',hTab,'Style','edit','RelPosition', [180 132 160 20],'Tag','etFrameClock','Enable','Off','String',[auxDAQName obj.hResource.hTrig.FRAME_CLOCK_TERM_OUT]);

                most.gui.uicontrol('Parent',hTab,'Style','text','RelPosition', [51 160 100 20],'Tag','txReferenceClock','String','Reference Clock','HorizontalAlignment','right');
                most.gui.uicontrol('Parent',hTab,'Style','edit','RelPosition', [180 157 160 20],'Tag','etReferenceClock','Enable','Off','String',[auxDAQName obj.hResource.hTrig.REFERENCE_CLOCK_TERM_OUT]);
        
                most.gui.uicontrol('Parent',hTab,'Style','text','RelPosition', [51 185 100 20],'Tag','txStartTriggerTermOut','String','Start Trigger Out','HorizontalAlignment','right');
                obj.pmStartTriggerTermOut = most.gui.uicontrol('Parent',hTab,'Style','popupmenu','RelPosition', [180 181 160 20],'Tag','pmStartTriggerTermOut','String',{''});
        end
        ...
        function redraw(obj)
            ...
            if most.idioms.isValidObj(obj.hResource.hDAQAux)
                hPFIs = arrayfun(@(PFI)PFI,obj.hResource.hDAQAux.hPFIs,'UniformOutput',false);
                hDIOsAux = arrayfun(@(DIO)DIO,obj.hResource.hDAQAux.hDIOs,'UniformOutput',false);
                hDIOs = obj.hResourceStore.filterByClass('dabs.resources.ios.DIO');

                nonAuxDIOsMask = ~cellfun(@(hDIO)strcmpi(hDIO.deviceName,obj.pmhDAQAux.pmValue),hDIOs);
                nonAuxDIOs = hDIOs(nonAuxDIOsMask);
                
                hPFIsMinusPFI0 = hPFIs(2:end);
                
                hDIOsAuxAllowingExtTriggering = hDIOsAux(4:end);
                
                obj.pmStartTriggerTermOut.String = [{''} nonAuxDIOs];
                obj.pmPixelClock.String = [{''} hPFIsMinusPFI0];
                obj.pmLineClock.String = [{''} hDIOsAuxAllowingExtTriggering];
                
                obj.pmStartTriggerTermOut.pmValue = obj.hResource.startTriggerTermOut;
                obj.pmPixelClock.pmValue = obj.hResource.pixelClockTermOut;
                obj.pmLineClock.pmValue = obj.hResource.lineClockTermOut;
                
                obj.pmStartTriggerTermOut.Enable = ~obj.hResource.mdlInitialized;
                obj.pmPixelClock.Enable = ~obj.hResource.mdlInitialized;
                obj.pmLineClock.Enable = ~obj.hResource.mdlInitialized;
            end
        end
        ...
        function apply(obj)
            ...
            most.idioms.safeSetProp(obj.hResource,'startTriggerTermOut',obj.pmStartTriggerTermOut.pmValue);
            most.idioms.safeSetProp(obj.hResource,'pixelClockTermOut',obj.pmPixelClock.pmValue);
            most.idioms.safeSetProp(obj.hResource,'lineClockTermOut',obj.pmLineClock.pmValue);
