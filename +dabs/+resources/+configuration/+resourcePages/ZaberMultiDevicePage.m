classdef ZaberMultiDevicePage < dabs.resources.configuration.ResourcePage
    properties
        ...
        etStreamMaxVelocity;              % The maximum speed for stage scanning acquistion and the actual speed of the flyback
        etStreamAcceleration;            % The acceleration for streamed movements while stage scanning
        pmFrameClockIn;                  % Terminal on the Zaber controller to receive an input trigger
    end
    ...
    methods
        ...
        function makePanel(obj,hParent)
            ...
            if obj.hResource.supportsStreamScanning
                hTab = uitab('Parent',hTabGroup,'Title','Stream');
                most.gui.uicontrol('Parent',hTab,'Style','text','RelPosition', [16 130 100 20],'Tag','txVelocity','String','Max Velocity [um/s]','HorizontalAlignment','right');
                obj.etStreamMaxVelocity  = most.gui.uicontrol('Parent',hTab,'Style','edit','RelPosition', [130 128 60 20],'Tag','etVelocity','HorizontalAlignment','right');

                most.gui.uicontrol('Parent',hTab,'Style','text','RelPosition', [2 100 114 20],'Tag','txAcceleration','String',' Acceleration [um/s^2]','HorizontalAlignment','right');
                obj.etStreamAcceleration  = most.gui.uicontrol('Parent',hTab,'Style','edit','RelPosition', [130 98 60 20],'Tag','etAcceleration','HorizontalAlignment','right');
                
                most.gui.uicontrol('Parent',hTab,'Style','text','RelPosition', [16 160 100 20],'Tag','txFrameClockInput','String',' Frame Clock Input','HorizontalAlignment','right');
                obj.pmFrameClockIn  = most.gui.uicontrol('Parent',hTab,'Style','popupmenu','RelPosition', [130 158 60 20],'Tag','etFrameClockInput','HorizontalAlignment','right');
            end
        end
        ...
        function redraw(obj)            
            ...
            if obj.hResource.supportsStreamScanning 
                obj.etStreamMaxVelocity.String = num2str(obj.hResource.streamMaxVelocity_umPerS);
                obj.etStreamAcceleration.String = num2str(obj.hResource.streamAcceleration_umPerSs);
                obj.pmFrameClockIn.String = arrayfun(@(num)num2str(num),1:obj.hResource.hIO.getChannelsInfo().getNumberDigitalInputs(),'UniformOutput',false);
            end
        end
        ...
        function apply(obj)
            ...
            if obj.hResource.supportsStreamScanning
                most.idioms.safeSetProp(obj.hResource,'streamMaxVelocity_umPerS',str2double(obj.etStreamMaxVelocity.String));
                most.idioms.safeSetProp(obj.hResource,'streamAcceleration_umPerSs',str2double(obj.etStreamAcceleration.String));
                most.idioms.safeSetProp(obj.hResource,'streamFrameClockIn',str2double(obj.pmFrameClockIn.pmValue));
            end
