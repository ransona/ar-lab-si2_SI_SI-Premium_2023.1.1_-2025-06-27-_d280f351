classdef ResourcePage < handle
    ...
    methods
        ...
        function makeTitle(obj,hParent)
            ...
            if isa(obj.hResource,'dabs.resources.widget.HasWidget')
                cbShowWidget = most.gui.uicontrol('Parent',container,'Style','checkbox','String','Show Widget','callback',@(src,evt)obj.setWidgetVisibility(src,evt),'HorizontalAlignment','right','WidthLimits',[100 100]);
                if isa(obj.hResource,'most.HasMachineDataFile') && isfield(obj.hResource.mdfData,'widgetVisibility')
                    cbShowWidget.Value = obj.hResource.mdfData.widgetVisibility;
                else
                    cbShowWidget.Value = true;
                end
            end
        ...
        function setWidgetVisibility(obj,src,evt)
            obj.hResource.widgetVisibility = src.Value;
        end
        ...
        function raise(obj)
            most.idioms.figure(ancestor(obj.hParent,'figure'));
        end
