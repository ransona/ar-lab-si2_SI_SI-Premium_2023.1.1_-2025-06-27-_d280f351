classdef vDAQ_Config < dabs.resources.Device & dabs.resources.DAQ & most.HasMachineDataFile & dabs.resources.configuration.HasConfigPage
    ...
    methods
        function reinit(obj)
            try
                ...
                obj.hardwareRevision = nfo.hardwareRevision;
                obj.moduleId = hFpga.moduleId;
                devSn = hFpga.deviceSerialNumber;
                delete(hFpga);

                assert(obj.moduleId ~= 0, 'No vDAQ daughtercard found');
                assert(obj.moduleId < 3, 'Unsupported vDAQ daughtercard detected');
                obj.isH = obj.moduleId == 2;
                ...
                if ~isempty(obj.bitfileName)
                    ...
                    obj.bitfileName = obj.defaultBitfileName;
                end
    ...
    methods
        ...
        function val = get.availableBitfiles(obj)
            ...
            if ~obj.isH
                % currently no custom bitfiles are available for MS vDAQ
                val = obj.defaultBitfileName;
            else
                val = bitfiles;
            end
        end

        function val = get.defaultBitfileName(obj)
            val = sprintf('vDAQR%d_SI.dbs', obj.hardwareRevision);
        end
    end
end
