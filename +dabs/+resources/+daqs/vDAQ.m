    ...
    methods
        function obj = vDAQ(name)
            ...
            if ~obj.simulated
                ...
                try
                    s = dabs.resources.daqs.private.vDAQPCIeInfo();
                    idx = obj.vDAQNumber+1;
                    obj.PCIeCurrentLink  = s(idx).PCIeCurrentLink;
                    obj.PCIeCurrentWidth = s(idx).PCIeCurrentWidth;
                    obj.PCIeMaxLink      = s(idx).PCIeMaxLink;
                    obj.PCIeMaxWidth     = s(idx).PCIeMaxWidth;
            ...
            if obj.PCIeCurrentLink<obj.PCIeMaxLink || obj.PCIeCurrentWidth<obj.PCIeMaxWidth
                expectedSpeed = sprintf('Gen%dx%d',obj.PCIeCurrentLink,obj.PCIeCurrentWidth);
                actualSpeed   = sprintf('Gen%dx%d',obj.PCIeMaxLink,obj.PCIeMaxWidth);
                obj.warnMsg   = sprintf('Slow PCIe link: %s Expected speed: %s',actualSpeed,expectedSpeed);
            end
        end
    end
