...
RggScans = rs.filterByClass('scanimage.components.scan2d.RggScan');
if isempty(RggScans)
    handles.pbSignalConditioning.Visible = 'off';
end
...
handles.hModel.hChannels.colorMap = strs{val};
...
% handles    structure with handles and user data (see GUIDATA)
try
    if isprop(handles.hController,'hSignalConditioningControls')
        handles.hController.hSignalConditioningControls.raise();
    else
        error('Signal Conditioning Controls is available only for vDAQ based acquisition systems.');
    end
catch ME
