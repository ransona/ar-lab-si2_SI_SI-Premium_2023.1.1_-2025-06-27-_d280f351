classdef ResonantStageScanner < scanimage.mroi.scannerset.ScannerSet
    %RESONANTMOTOR Summary of this class goes here
    ...
    properties (Constant)
        optimizableScanners = {'Z'}; % Cell array of strings e.g. {'G','Z'}
    end
    ...
    properties (Hidden)
        CONSTRAINTS = struct(...
                    ...
                    @scanimage.mroi.constraints.stageScannerMaxSpeed @scanimage.mroi.constraints.singleRoi}}...
                    );
    end
    ...
    methods(Static)
        function obj=default()
            ...
            g=scanimage.mroi.scanners.StageScanner.default();
            z=scanimage.mroi.scanners.FastZ.default();
            obj=scanimage.mroi.scannerset.ResonantStageScanner(r,g,z);
            obj.refToScannerTransform = eye(3);
        end
    end
    
    methods
        function obj = ResonantStageScanner(name,resonantx,galvox,galvoy,stageScanner,fastBeams,slowBeams,fastz,fillFractionSpatial)
            %% Describes a resonant-Streamable motor axis scan system
            ...
            scanimage.mroi.util.asserttype(stageScanner,'scanimage.mroi.scanners.StageScanner');
            
            obj.name = name;
            obj.scanners={resonantx,galvox,galvoy,stageScanner};
        ...
        function path_FOV = refFovToScannerFov(obj,path_FOV)
            ...
            s = path_FOV.S;
            s(:,3) = 0;
            s = scanimage.mroi.coordinates.Points(obj.hCSReference,s);
            s = s.transform(obj.hCSAxesPosition.hParent);
            s = s.points;
            path_FOV.S = s(:,1:2);
            ...
            assert(all(path_FOV.G(:,2) >= rng(1)-tol) && all(path_FOV.G(:,2) <= rng(2)+tol), 'Current ROI Group goes beyond Y galvo scanner range.');
            path_FOV.G(path_FOV.G(:,2) < rng(1),2) = rng(1);
            path_FOV.G(path_FOV.G(:,2) > rng(2),2) = rng(2);
            
            %Stage scanner
            stageScanner = obj.scanners{4};
            hMotorDevice = stageScanner.hDevice;

            %Stage scanner x axis
            rng = hMotorDevice.range_um(stageScanner.axesXY(1),:);
            rng = diff(rng);
            halfrng = rng/2;
            assert(all(path_FOV.S(:,1) >= -halfrng-tol) && all(path_FOV.S(:,1) <= halfrng+tol), 'Attempted to scan outside x-axis stage scanner FOV.');
            path_FOV.S(path_FOV.S(:,1) < -halfrng,1) = -halfrng;
            path_FOV.S(path_FOV.S(:,1) >  halfrng,1) =  halfrng;
            
            %Stage scanner y axis
            rng = hMotorDevice.range_um(stageScanner.axesXY(2),:);
            rng = diff(rng);
            halfrng = rng/2;
            assert(all(path_FOV.S(2) >= -halfrng-tol) && all(path_FOV.S(1) <= halfrng+tol), 'Attempted to scan outside y-axis stage scanner FOV.');
            path_FOV.S(path_FOV.S(:,2) < -halfrng,2) = -halfrng;
            path_FOV.S(path_FOV.S(:,2) >  halfrng,2) =  halfrng;
        end
        ...
        function [path_FOV, seconds] = scanPathFOV(obj,scanfield,roi,actz,actzRelative,dzdt,zActuator,maxPtsPerSf)
            ...
            %     field
            % 2.  Streamable motor is continuously moving down the field.
            ...
            gfov(:,2) = zeros(gsamples,1);
            
            hysz = scanfield.sizeXY(2)/2;
            sfov = [0 scanfield.centerXY(2)+hysz;
                    0 scanfield.centerXY(2)-hysz]; % Let the X Galvo do all the moving    
            ...
            for beamIdx = 1:numel(obj.beams)
                ...
                if(isa(hBeam,'scanimage.mroi.scanners.FastBeam'))
                    ...
                    if obj.scanners{1}.reverseLineRead
                        beamSamplePositionXX = fliplr(beamSamplePositionXX);
                    end
                    ...
                    for pb = hBeam.powerBoxes
                        ...
                        if pb.useSampleAbsolute
                            %Define beam sample position relative to the
                            %top left corner of the scanner FOV (X: -angularRange_X/2, Y: -angularRange_Y/2)
                            %using normalized units (0-1)
                            sfCornerPointsNorm = [scanfield.centerXY - scanfield.sizeXY/2; scanfield.centerXY + scanfield.sizeXY/2] ./ obj.angularRange + 0.5;
                            sfSizeNorm = scanfield.sizeXY ./ obj.angularRange;
                            
                            beamSamplePositionXX = beamSamplePositionXX .* sfSizeNorm(1) + sfCornerPointsNorm(1,1);
                            beamSamplePositionYY = beamSamplePositionYY .* sfSizeNorm(2) + sfCornerPointsNorm(1,2);
                        end
                    ...
                    if hBeam.turnAroundBlanking || interlaceDecimation>1
        ...
        function path_FOV = interpolateTransits(obj,path_FOV,tuneZ,zWaveformType)
            ...
            for idx = 1:numel(obj.beams)
                ...
                elseif ~hBeam.flybackBlanking && hBeam.turnAroundBlanking
                    path_FOV.B(:,idx) = scanimage.mroi.util.flatInterpolateCircularNaNRanges(path_FOV.B(:,idx));
                    path_FOV.B(end,idx) = 0; %For Safety
                else
                ...
                if obj.hasPowerBox
                    ...
                    elseif ~hBeam.flybackBlanking && hBeam.turnAroundBlanking
                        path_FOV.Bpb(:,idx) = scanimage.mroi.util.flatInterpolateCircularNaNRanges(path_FOV.Bpb(:,idx));
                        path_FOV.Bpb(end,idx) = 0; %For Safety
                    else
        ...
        function path_FOV = zFlybackFrame(obj, frameTime)
            ...
            if obj.scanners{2}.useScannerTimebase
                path_FOV.R = [];
                path_FOV.G = nan(round(obj.nsamples(obj.scanners{2},frameTimeRTB)),2);
                path_FOV.S = [];
            else
                path_FOV.R = [];
                path_FOV.G = nan(round(obj.nsamples(obj.scanners{2},frameTime)),2);
                path_FOV.S = [];
            end
        ...
        function path_FOV = padFrameAO(obj, path_FOV, frameTime, flybackTime, zWaveformType)
            ...
            elseif obj.scanners{3}.useScannerTimebase
                ...
                Ns = ceil(obj.nsamples(obj.scanners{2},frameTime + flybackTime/2));
            end
    ...
    methods        
        function v = frameFlybackTime(obj)
            v = obj.scanners{4}.flybackTimeSeconds;
        end
        ...
        function seconds = transitTime(obj,scanfield_from,scanfield_to)
            ...
            elseif isnan(scanfield_to)
                seconds = max(obj.scanners{4}.flybackTimeSeconds,obj.scanners{4}.flybackTimeSeconds);
            else
                seconds = max(obj.scanners{4}.flytoTimeSeconds,obj.scanners{4}.flytoTimeSeconds);
            end
        end
        ...
        function samplesPerTrigger = samplesPerTriggerForAO(obj,outputData)
            ...
            samplesPerTrigger.S = hSI.hStackManager.framesPerSlice;
        end
    ...
    methods
        function v = get.angularRange(obj)
            hMotor = obj.scanners{4}.hDevice;
            stageRange  = diff(hMotor.range_Degrees(2,:));
            
            if obj.extendedRggFov && ~isempty(obj.scanners{2})
                v = [obj.scanners{1}.fullAngleDegrees+diff(obj.scanners{2}.hDevice.travelRange) stageRange];
            else
                v = [obj.scanners{1}.fullAngleDegrees stageRange];
            end
        end
    end
end
