    ...
    methods
        ...
        function ao_volts = pathFovToAo(obj,path_FOV)
            ...
            if obj.hasSlm && isfield(path_FOV, 'SLM')
                ...
                slmSegments = ao_volts.SLM;
                if ~isempty(slmSegments)
                    for idx = 1:numel(obj.beams)
                        if numel(slmSegments) == 1
                            path_FOV.B(:,idx) = path_FOV.B(:,idx) / slmSegments.mask.efficiency;
                        else
                            beamPath = path_FOV.B(:,idx);
                            sampleRateHz = obj.beams(idx).sampleRateHz;
                            segmentSamples = arrayfun(@(seg)round(seg.duration * sampleRateHz), slmSegments);
                            if ~isempty(segmentSamples)
                                sampleDelta = numel(beamPath) - sum(segmentSamples);
                                segmentSamples(end) = segmentSamples(end) + sampleDelta;
                            end
                            
                            startSample = 1;
                            for segIdx = 1:numel(slmSegments)
                                segCount = segmentSamples(segIdx);
                                if segCount <= 0
                                    continue;
                                end
                                endSample = min(numel(beamPath), startSample + segCount - 1);
                                if endSample < startSample
                                    continue;
                                end
                                
                                efficiency = slmSegments(segIdx).mask.efficiency;
                                beamPath(startSample:endSample) = beamPath(startSample:endSample) / efficiency;
                                startSample = endSample + 1;
                                if startSample > numel(beamPath)
                                    break;
                                end
                            end
                            
                            path_FOV.B(:,idx) = beamPath;
                        end
        ...
        function [lineScanPeriod,lineAcquisitionPeriod] = linePeriod(obj,scanfield)
            ...
            if ~isempty(obj.acqSampleRate)
                ...
                ctlSamplesMask = most.floatingpoint.fuzzyEquals(ctlSamples,round(ctlSamples));
                ctlSamples = round(ctlSamples(ctlSamplesMask));
                assert(~isempty(ctlSamples), 'Could not find a line period fitting the desired scan parameters. Try adjusting the scanner control rate, acq sample rate or ROI pixel counts.');
        ...
        function path_FOV = interpolateTransits(obj,path_FOV,tuneZ,zWaveformType)
            ...
            offset_ms = obj.scanners{1}.actuatorLag_ms;
            if logical(offset_ms)
                offset_samples = round(offset_ms * 1e-3 * obj.scanners{1}.sampleRateHz);
                path_FOV.G(:,1) = circshift(path_FOV.G(:,1),offset_samples);
            end

            offset_ms = obj.scanners{2}.actuatorLag_ms;
            if logical(offset_ms)
                offset_samples = round(offset_ms * 1e-3 * obj.scanners{2}.sampleRateHz);
                path_FOV.G(:,2) = circshift(path_FOV.G(:,2),offset_samples);
            end
...
%                 end
            ...
            function [path_FOV,seconds] = generateGalvoPathImaging(path_FOV)
                ...
                if isempty(obj.acqSampleRate)
                    ...
                    assert(most.floatingpoint.fuzzyEquals(nx,round(nx)), 'Invalid sample rate');
                    nx = round(nx);
...
%                         nBlank = nBlank * hBeam.turnAroundBlanking;
%                         nxAcq = nx - nBlank*2;
                        ...
                        if hBeam.turnAroundBlanking
                            powerFracs(beamSamplePositionXX<0|beamSamplePositionXX>1) = 0;
                            powerFracsPb(beamSamplePositionXX<0|beamSamplePositionXX>1) = 0;
                        end
                        
                        powerFracs(end,end) = powerFraction .* ~hBeam.flybackBlanking;
                        powerFracsPb(end,end) = powerFraction .* ~hBeam.flybackBlanking;
