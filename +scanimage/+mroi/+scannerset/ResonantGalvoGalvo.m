classdef ResonantGalvoGalvo < scanimage.mroi.scannerset.ScannerSet
    ...
    properties (Hidden)
        CONSTRAINTS = struct(...
            ...
            @scanimage.mroi.constraints.sameWidth @scanimage.mroi.constraints.yCenterInRange @scanimage.mroi.constraints.rotation180...
    ...
    methods
        function obj = ResonantGalvoGalvo(name,resonantx,galvox,galvoy,fastBeams,slowBeams,fastz,fillFractionSpatial)
            ...
            if ~isempty(galvox) || isa(resonantx.hDevice,'dabs.mirrorcle.ResonantAxis')
        ...
        function path_FOV = refFovToScannerFov(obj,path_FOV)
            ...
            assert(all(path_FOV.G(:,1) >= rng(1)-tol) && all(path_FOV.G(:,1) <= rng(2)+tol), 'Current ROI Group goes beyond X galvo scanner range.');
        ...
        function [path_FOV, seconds] = scanPathFOV(obj,scanfield,roi,actz,actzRelative,dzdt,zActuator,maxPtsPerSf)
            ...
            % flip if 180 degrees
            if abs(mod(scanfield.rotationDegrees,360) - 180) < 1e-6
                gfov(:,2) = flipud(gfov(:,2));
            end
        ...
        function path_FOV = interpolateTransits(obj,path_FOV,tuneZ,zWaveformType)
            ...
            offset_ms = obj.scanners{3}.actuatorLag_ms;
            if logical(offset_ms)
                offset_samples = round(offset_ms * 1e-3 * obj.scanners{3}.sampleRateHz);
