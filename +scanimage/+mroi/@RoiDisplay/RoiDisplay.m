    ...
    methods
        function obj = RoiDisplay(hSI,parent,chan)
            ...
            obj.hSurfContextMenu = handle(uicontextmenu('Parent',obj.hFig,'Callback',@obj.contextMenuOpen));
                ...
                hDisplayMoveMenu = uimenu('Parent', obj.hSurfContextMenu,...
                    'Separator', 'on',...
                    'Label', 'Display Dragging Device');
                    uimenu('Parent', hDisplayMoveMenu,...
                            'Label', 'Motor',...
                            'Tag', 'displayDraggingDeviceMotor',...
                            'Callback', @(src,evt)obj.setDisplayDraggingDevice('Motor'));
                    uimenu('Parent', hDisplayMoveMenu,...
                            'Label', 'Galvo',...
                            'Tag', 'displayDraggingDeviceGalvo',...
                            'Callback', @(src,evt)obj.setDisplayDraggingDevice('Galvo'));
                    uimenu('Parent', hDisplayMoveMenu,...
                            'Label', 'SLM',...
                            'Tag', 'displayDraggingDeviceSLM',...
                            'Callback', @(src,evt)obj.setDisplayDraggingDevice('SLM'));

                hMotionCorrectionMenu = uimenu('Parent', obj.hSurfContextMenu,...
                    'Separator', 'on',...
                    'Label', 'Motion Correction');
                    uimenu('Parent', hMotionCorrectionMenu,...
                        'Separator', 'on',...
                        'Label', 'Set roi as motion correction ref',...
                        'Callback', @(src,evt)obj.surfContextMenuCallback(@obj.setMotionReferenceThisRoi));
                    hideInMergeWindow(end+1) = uimenu('Parent', hMotionCorrectionMenu,...
                    ...
                    hideInMergeWindow(end+1) = uimenu('Parent', hMotionCorrectionMenu,...
                        'Label', 'Add roi slice as motion correction ref',...
                        'Callback', @(src,evt)obj.surfContextMenuCallback(@obj.addMotionReferenceThisRoiSlice));
                    hideInMergeWindow(end+1) = uimenu('Parent', hMotionCorrectionMenu,...
...
%             end
            ...
            function [hSurf,centerpoint] = prepareSurface(imcoordsX,imcoordsY,imcoordsZ,imData,varargin)
                tooThin = max(abs(diff(imcoordsY(:)))) / max(abs(diff(imcoordsX(:)))) < 20/1920; %Is image less than 20 lines tall on a 1080p monitor
                if tooThin
                    imcoordsY = imcoordsX';
                end
            ...
            function rebindHistograms()
                hLiveHistograms = obj.hSI.hDisplay.hLiveHistograms;
                if isempty(hLiveHistograms)
                    return;
                end

                for hHistIdx = 1:numel(hLiveHistograms)
                    hHist = hLiveHistograms(hHistIdx);
                    try
                        if hHist.channel == obj.chan
                            zSurfMap = obj.roiMap(hHist.roi.uuiduint64);
                            surf = zSurfMap(hHist.z);
                            surf.hHist = hHist;
                            zSurfMap(hHist.z) = surf;
                        end
                    catch ME
                    end
                end
        ...
        function reSize(obj,varargin)
            if ~obj.is3dview
                ...
                if windowAsp > canvasAsp
                    ...
                    %TODO: Put VSCode style scroll bar in
                else
...
%             end
    ...
    methods
        ...
        function set.CLim(obj,val) 
            ...
            hLiveHistograms = obj.hSI.hDisplay.hLiveHistograms;
            hLiveHistogramsForChannelMask = [hLiveHistograms.channel] == obj.chan;
            hLiveHistograms = hLiveHistograms(hLiveHistogramsForChannelMask);
            for hLiveHistogram = hLiveHistograms
                hLiveHistogram.lut = obj.CLim;
            end
        end
    ...
    methods
        ...
        function contextMenuOpen(obj,src,evt)
            ...
            if isempty(obj.hSI.hScan2D.hSlmScan)
                slmDragVisible = 'off';
            else
                slmDragVisible = 'on';
            end
            ...
            set(findall(src,'Tag','displayDraggingDeviceSLM'),'Visible',slmDragVisible);
            set(findall(src,'Tag','displayDraggingDeviceMotor'),'Checked',motorSelected);
            set(findall(src,'Tag','displayDraggingDeviceSLM'),'Checked',slmSelected);
            set(findall(src,'Tag','displayDraggingDeviceGalvo'),'Checked',galvoSelected);
        ...
        function setMotionReferenceThisVolume(obj)
            ...
            %Add absolute position where these ROIs are taken from (for
            %sample power boxes)
            absStagePosition = obj.hSI.hMotors.getPosition(obj.hSI.hMotors.hCSSampleAbsolute);
            absStagePosition = absStagePosition.points;
            
            for roiIdx = 1:numel(roiDatas)
                roiData = roiDatas(roiIdx);
                roiData.absStagePosition = absStagePosition;
            end
        ...
        function setMotionReferenceThisRoi(obj,hSurf,clearAll)
            ...
            if ~isempty(obj.hSI.hController)
                ...
                hSICtl.showGUI('MotionDisplay');
                hSICtl.raiseGUI('MotionDisplay');
                hSICtl.hMotionDisplay.currentZ = roiData.zs(1);
            end
        end
        ...
        function setMotionReferenceThisRoiSlice(obj,hSurf,clearAll)
            if nargin < 3 || isempty(clearAll)
                clearAll = true;
            end
            ...
            if ~isempty(obj.hSI.hController)
                ...
                hSICtl.showGUI('MotionDisplay');
                hSICtl.raiseGUI('MotionDisplay');
                hSICtl.hMotionDisplay.currentZ = z;
            end
        end
        
        function addMotionReferenceThisRoiSlice(obj,hSurf)
            clearAll = false;
            obj.setMotionReferenceThisRoiSlice(hSurf,clearAll);
        end
        ...
        function showSurfHistogram(obj,hSurf)
            ...
            %If an existing histogram has the same qualities, then raise it
            hLiveHistograms = obj.hSI.hDisplay.hLiveHistograms;
            for hLiveHistogram = hLiveHistograms
                if hLiveHistogram.channel == obj.chan && ...
                        hLiveHistogram.roi.isequalish(roi) && ...
                        hLiveHistogram.z == z
                    most.idioms.figure(hLiveHistogram.hFig);
                    hHist = hLiveHistogram;
                    return;
                end
            end

            %Otherwise, make a new one.
            hHist = scanimage.mroi.LiveHistogram(obj.hSI,obj.chan,roi,z);
            ...
            obj.hSI.hDisplay.addLiveHistogram(hHist);
            surfs.hHist = hHist;
            zSurfMap(z) = surfs;
        end
        ...
        function showVolumeHistogram(obj)
            ...
            hHist = scanimage.mroi.LiveHistogram(obj.hSI,NaN,NaN,NaN);
            res = obj.hSI.hScan2D.channelsAdcResolution;
            hHist.dataRange = [-(2^(res-1)),2^(res-1)-1];
            hHist.lut = obj.CLim;
            hHist.viewRange = mean(obj.CLim) + [-1.5 1.5].*double(diff(obj.CLim))./2;
        ...
        function setDisplayDraggingDevice(obj,dragDeviceStr)
            obj.dragDevice = dragDeviceStr;
        end
        ...
        function dragSlm(obj,hSurf)
            hCS = obj.hSI.hScan2D.hSlmScan.hCSScannerToRef;
            ...
            isAligned = isSetToParentAffine || isSetFromParentAffine;
            ...
            hSlm = obj.hSI.hScan2D.hSlmScan.hSlm;
            if isempty(hSlm)
                most.idioms.warn('SLM device not found.');
                return;
            else
                startPoint_SlmScannerObjectiveUM = obj.hSI.hScan2D.hSlmScan.hSlm.hPtLastWritten;
            end
            ...
            function pointSlm(async)                
                try
                    if isempty(obj.hFig.CurrentModifier)
                        ...
                        move_vector_Ref = scanimage.mroi.coordinates.Points(obj.hSI.hCoordinateSystems.hCSReference,move_vector_Ref);
                        move_vector_SLMScannerObjectiveUM = move_vector_Ref.transform(hSlm.hCoordinateSystem);
                        move_vector_SLMScannerObjectiveUM = move_vector_SLMScannerObjectiveUM.points;
                        move_vector_SLMScannerObjectiveUM = move_vector_SLMScannerObjectiveUM(2,:) - move_vector_SLMScannerObjectiveUM(1,:);

                        %Starting point seems to always be in Reference
                        %space at the start of acquisition rather than
                        %Objective micron, and then Objective micron
                        %thereafter.
                        startPoint_SlmScannerObjectiveUM = startPoint_SlmScannerObjectiveUM.transform(hSlm.hCoordinateSystem);

                        newPt = startPoint_SlmScannerObjectiveUM - move_vector_SLMScannerObjectiveUM;
                        hSlm.pointScanner(newPt);
                        lastMove = tic();
                    end
                catch ME
                    abort();
        ...
        function dragGalvo(obj,hSurf)
            ...
            hRoiManager = obj.hSI.hRoiManager;
            startPoint_ScannerSpace_deg = [hRoiManager.scanAngleShiftFast , hRoiManager.scanAngleShiftSlow 0];
            ...
            function pointGalvos(async)
                try
                    if isempty(obj.hFig.CurrentModifier)
                        ...
                        pointXY_Ref = getMouseSurfPixel(obj,hSurf);
                        move_vector_Ref = pointXY_Ref - startPoint_Ref;

                        if ~obj.hSI.hScan2D.hasXGalvo
                            move_vector_Ref(1) = 0;
                            if move_vector_Ref(1) > 2
                                most.idioms.warn('Cannot drag image in X without an X-Galvo in the path.');
                            end
                        end

                        newPt = startPoint_ScannerSpace_deg - move_vector_Ref;
                        obj.hSI.hRoiManager.scanAngleShiftFast = newPt(1);
                        obj.hSI.hRoiManager.scanAngleShiftSlow = newPt(2);
                        lastMove = tic();
                    end
                catch ME
                    abort();
