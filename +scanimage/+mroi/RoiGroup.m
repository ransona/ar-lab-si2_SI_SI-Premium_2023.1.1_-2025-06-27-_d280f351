classdef RoiGroup < scanimage.mroi.RoiTree    
    ...
    properties(SetAccess = private,Dependent)
        ...
        imagingRois;  % subset of rois, where roi.enable == true and roi.scanfields are of class scanimage.mroi.scanfield.ImagingField
    ...
    methods        
        % public
        function [ao_volts,samplesPerTrigger,sliceScanTime,path_FOV,frameTime] = scanStackAO(obj,scannerset,zs,zsRelative,zWaveform,flybackFrames,zActuator,sliceScanTime,tuneZ)
            ...
            [path_FOV,samplesPerTrigger,sliceScanTime,frameTime] = obj.scanStackFOV(scannerset,zs,zsRelative,zWaveform,flybackFrames,zActuator,sliceScanTime,tuneZ);
            ao_volts = arrayfun(@(fov)scannerset.pathFovToAo(fov),path_FOV);
        end
        
        % private
        function [path_FOV,samplesPerTrigger,sliceScanTime,frameTime] = scanStackFOV(obj,scannerset,zs,zsRelative,zWaveform,flybackFrames,zActuator,sliceScanTime,tuneZ,maxPtsPerSf,applyConstraints,skipInterpolateTransits)
            if nargin < 7 || isempty(zActuator)
                zActuator = 'fast';
            end
            if nargin < 8
            ...
            if nargin < 12 || isempty(skipInterpolateTransits)
                skipInterpolateTransits = false;
            end
            ...
            if skipInterpolateTransits
                %Skip interpolation if our ROI group is actually a grouping
                %of diffraction limited SLM points (Which only happens
                % for aesthetic purposes as a means of facilitating display 
                % in the ROI Group Editor).
            ...
            if ~isa(scannerset,'scanimage.mroi.scannerset.SLM')
                xWaveform = dataPoints.G(:,1); %GG -> line clock; RGG -> roi clock.
                XG = ones(length(xWaveform),1);
                XG(isnan(xWaveform) | isinf(xWaveform)) = 0;

                path_FOV.clocks.xg = XG;
            end
        end
        ...
        function [path_FOV, slcEmpty] = scanSliceFOV(obj,scannerset,z,zRelative,dzdt,zActuator,frameTime,flybackTime,zWaveformType,maxPtsPerSf)
            ...
            if numel(scanRois) > 0
                if isa(scannerset,'scanimage.mroi.scannerset.ResonantStageScanner') && numel(scanRois) > 1
                   error('Cannot do multiple ROIs with a resonant stage scanner configuration. Use Tile View instead.'); 
                end
    ...
    methods
        ...
        function N = getNumNonPause(obj)
            scanfields = [obj.rois.scanfields];
            if numel(scanfields)
                N = sum(~[scanfields.isPause]);
            else
                N = 0;
            end
        end
    ...
    methods
        ...
        function val = get.imagingRois(obj)
            if ~isempty(obj.rois)
                roiMask = arrayfun(@(roi)isa(roi.scanfields,'scanimage.mroi.scanfield.ImagingField'),obj.rois);
                val = obj.rois(roiMask & [obj.rois.enable]);
            else
                val = [];
            end
        end
