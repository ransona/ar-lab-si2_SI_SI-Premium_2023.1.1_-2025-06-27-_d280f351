classdef Display < scanimage.interfaces.Component
    ...
    properties (SetObservable, Hidden)
        ...
        hLiveHistograms = scanimage.mroi.LiveHistogram.empty();
    end
    ...
    methods
        ...
        function set.lineScanHistoryLength(obj,val)
            validateattributes(val,{'numeric'},{'scalar','integer','>=',2});
    ...
    methods
        ...
        function resetDisplayFigs(obj,chansToReset,resetMergeTF,preserveCameraProps,deactivatedChans)
            ...
            if nargin < 2 || isempty(chansToReset)
                ...
                if iscolumn(chansToReset)
                    chansToReset = chansToReset';
                end
            end
            ...
            if nargin < 5 || isempty(deactivatedChans)
                deactivatedChans = [];
            end
            
            if (~obj.hSI.mdlInitialized && ~obj.duringInit) || (isempty(chansToReset) && isempty(deactivatedChans))
                return;
            end
            ...
            for deactivatedChan = deactivatedChans
                hRoiDisplay = obj.hAxes{deactivatedChan}{1};
                hFig = hRoiDisplay.hFig;
                hFig.Visible = most.gui.OnOff(false);
            end
        ...
        function restoreCameraProps(obj,chansToReset)
            ...
            chansToReset = intersect(chansToReset, obj.hSI.hChannels.channelDisplay);
    ...
    methods (Hidden)
                        ...
                        if isempty(obj.rollingStripeDataBuffer{zIdx}{1}.roiData)
                            ...
                            for iterRoiData = 1:length(obj.rollingStripeDataBuffer{zIdx}{1}.roiData)
                                ...
                                if ~isempty(newStripe.roiData)
                                    ...
                                    obj.rollingStripeDataBuffer{zIdx}{1}.roiData{iterRoiData}.frameNumberAcqMode = newStripe.roiData{iterRoiData}.frameNumberAcqMode;
                                    obj.rollingStripeDataBuffer{zIdx}{1}.roiData{iterRoiData}.zs = newStripe.roiData{iterRoiData}.zs;
                                    obj.rollingStripeDataBuffer{zIdx}{1}.roiData{iterRoiData}.frameTimestamp = newStripe.roiData{iterRoiData}.frameTimestamp;
                                end
                            end
                            obj.rollingStripeDataBuffer{zIdx}{1}.focalPoint = newStripe.focalPoint;
                            obj.rollingStripeDataBuffer{zIdx}{1}.stackMode = newStripe.stackMode;
            ...
            elseif ~isempty(newStripe.rawData)
                ...
                if newStripe.startOfFrame && newStripe.endOfFrame
                    ...
                    if obj.displayRollingAverageFactor > 1
                        % performance optimization: copy using inplacewrite instead of Matlab indexing
                        %obj.lineScanRawDataBuffer(:,wp,:) = reshape(newStripe.rawData,frmN,numel(wp),[]);
                        
                        raw = reshape(newStripe.rawData,frmN,numel(wp),[]);
                        raw = cast(raw,'like',obj.lineScanRawDataBuffer);
                        
                        for wpIdx = 1:numel(wp)
                            wp_ = wp(wpIdx);
                            for pageIdx = 1:size(obj.lineScanRawDataBuffer,3)
                                dstIdx = sub2ind(size(obj.lineScanRawDataBuffer),1,wp_,pageIdx);
                                srcIdx = sub2ind(size(raw),1,wpIdx,pageIdx);
                                numElementsToCopy = size(obj.lineScanRawDataBuffer,1);
                                most.memfunctions.inplacewrite(obj.lineScanRawDataBuffer,raw,dstIdx,srcIdx,numElementsToCopy);
                            end
                        end
                        ...
                        for i = 1:numel(wp)
                            ...
                            for pageIdx = 1:size(obj.lineScanAvgDataBuffer,3)
                                rawF = cast(obj.lineScanRawDataBuffer(:,f,pageIdx),'like',obj.lineScanAvgDataBuffer);
                                avgA = obj.lineScanAvgDataBuffer(:,a,pageIdx);
                                rawS = cast(obj.lineScanRawDataBuffer(:,s,pageIdx),'like',obj.lineScanAvgDataBuffer);
                                
                                avgA(isnan(avgA)) = 0;
                                
                                src = rawF + avgA - rawS;
                                
                                dstIdx = sub2ind(size(obj.lineScanAvgDataBuffer),1,f,pageIdx);
                                srcIdx = 1;
                                numElementsToCopy = numel(src);
                                most.memfunctions.inplacewrite(obj.lineScanAvgDataBuffer,src,dstIdx,srcIdx,numElementsToCopy);
                            end
                        end
                    else
                        % performance optimization: copy using inplacewrite instead of Matlab indexing
                        ...
                        raw = reshape(newStripe.rawData,frmN,numel(wp),[]);
                        raw = cast(raw,'like',obj.lineScanAvgDataBuffer);
                        
                        for wpIdx = 1:numel(wp)
                            wp_ = wp(wpIdx);
                            
                            for pageIdx = 1:size(obj.lineScanAvgDataBuffer,3)
                                dstIdx = sub2ind(size(obj.lineScanAvgDataBuffer),1,wp_,pageIdx);
                                srcIdx = sub2ind(size(raw),1,wpIdx,pageIdx);
                                numElementsToCopy = size(raw,1);
                                most.memfunctions.inplacewrite(obj.lineScanAvgDataBuffer,raw,dstIdx,srcIdx,numElementsToCopy);
                            end
                        end
                    end
                else
                    ...
                    if newStripe.startOfFrame
                        ...
                        for pageIdx = 1:size(obj.lineScanAvgDataBuffer,3)
                            dstIdx = sub2ind(size(obj.lineScanAvgDataBuffer),1,wp,pageIdx);
                            srcIdx = sub2ind(size(obj.lineScanAvgDataBuffer),1,obj.lineScanLastFramePtr,pageIdx);
                            numElementsToCopy = size(obj.lineScanAvgDataBuffer,1);
                            most.memfunctions.inplacewrite(obj.lineScanAvgDataBuffer,obj.lineScanAvgDataBuffer,dstIdx,srcIdx,numElementsToCopy);
                        end
                    ...
                    if obj.displayRollingAverageFactor > 1
                        ...
                        raw = reshape(newStripe.rawData,strpN,numel(wp),[]);
                        raw = cast(raw,'like',obj.lineScanRawDataBuffer);
                        
                        for pageIdx = 1:size(obj.lineScanRawDataBuffer,3)
                            dstIdx = sub2ind(size(obj.lineScanRawDataBuffer),strt,wp,pageIdx);
                            srcIdx = sub2ind(size(raw),1,1,pageIdx);
                            numElementsToCopy = size(raw,1);
                            most.memfunctions.inplacewrite(obj.lineScanRawDataBuffer,raw,dstIdx,srcIdx,numElementsToCopy);
                        end
                        ...
                        for pageIdx = 1:size(obj.lineScanAvgDataBuffer,3)
                            rawW = cast(obj.lineScanRawDataBuffer(strt:nd,wp,pageIdx),'like',obj.lineScanAvgDataBuffer); 
                            avgA = obj.lineScanAvgDataBuffer(strt:nd,ap,pageIdx);
                            rawS = cast(obj.lineScanRawDataBuffer(strt:nd,sp,pageIdx),'like',obj.lineScanAvgDataBuffer);

                            avgA(isnan(avgA)) = 0;

                            src = rawW + avgA - rawS;

                            dstIdx = sub2ind(size(obj.lineScanAvgDataBuffer),strt,wp,pageIdx);
                            srcIdx = 1;
                            numElementsToCopy = numel(src);
                        ...
                        raw = newStripe.rawData;
                        raw = reshape(raw,strpN,1,[]);
                        raw = cast(raw,'like',obj.lineScanAvgDataBuffer);
                        
                        % performance optimization: copy using inplacewrite instead of Matlab indexing
                        % obj.lineScanAvgDataBuffer(strt:nd,wp,:) = raw;
                        for wpIdx = 1:numel(wp)
                            wp_ = wp(wpIdx);
                            for pageIdx = 1:size(raw,3)
                                dstIdx = sub2ind(size(obj.lineScanAvgDataBuffer),strt,wp_,pageIdx);
                                srcIdx = sub2ind(size(raw),1,wpIdx,pageIdx);
                                numElementsToCopy = nd-strt+1;
                                most.memfunctions.inplacewrite(obj.lineScanAvgDataBuffer,raw,dstIdx,srcIdx,numElementsToCopy);
                            end
                        end
                    end
        ...
        function roiDatas = getAveragedRoiDatas(obj)
            numZs = numel(obj.rollingStripeDataBuffer);
            ...
            for zidx = 1:numZs
                stripeData_ = obj.rollingStripeDataBuffer{zidx}{1};
            ...
            if obj.channelsMergeEnable
                ...
                for iter = 1:numel(lclStripeData.roiData)
                    ...
                    for chanIdx = 1:numel(chansToDisp)
                        ...
                        chanLut = double(obj.(chanProp)) * obj.displayRollingAverageFactor; % adjust LUT to perform frame averaging
        ...
        function displayStripe(obj,channelNumber,buffer,forceAll)
            ...
            if obj.hSI.hRoiManager.isLineScan
                ...
                if size(obj.hAxes,2) < channelNumber || isempty(obj.hAxes{channelNumber})
                    return;
                else
    ...
    methods (Hidden)
        function addLiveHistogram(obj,hHist)
            if most.idioms.isValidObj(hHist) && isa(hHist,'scanimage.mroi.LiveHistogram')
                obj.hLiveHistograms(end + 1) = hHist;
                obj.hSI.hController{1}.registerGUI(hHist.hFig);
            end
        end
        
        function removeLiveHistogram(obj,hHist)
            hHistMask = obj.hLiveHistograms ==  hHist;    
            obj.hLiveHistograms(hHistMask) = [];
            obj.hSI.hController{1}.unregisterGUI(hHist.hFig);
        end
        ...
        function numChannelFigs = ziniCreateFigs(obj)
            ...
            obj.duringInit = true;
            obj.resetDisplayFigs([],true,false);
        ...
        function zzzSetImageFigureAxesLimits(obj)
            if ~isempty(obj.hSI.hChannels.channelDisplay)
                for i=1:numel(obj.hSI.hChannels.channelDisplay)
                    ...
                    most.idioms.figure(obj.hFigs(chan));
            ...
            if obj.hSI.hRoiManager.isLineScan
                ...
                if ~isempty(fieldnames(APB))
                    try
                        ...
                        obj.lineScanRawDataBuffer = zeros(APB.samplesPerFrame,obj.lineScanHistoryLength,numel(APB.channelsActive),'int16');
                        obj.lineScanDataBufferChannels = APB.channelsActive;
                        obj.lineScanLastFramePtr = obj.lineScanHistoryLength;
                        obj.lineScanAvgDataBuffer = nan(APB.samplesPerFrame,obj.lineScanHistoryLength,numel(APB.channelsActive),'single');
    ...
    methods (Access = protected, Hidden)
        function componentStart(obj)
            ...
            if obj.hSI.hRoiManager.isLineScan
                obj.lineScanHistoryLength = round(30/obj.hSI.hRoiManager.scanFramePeriod); %~30 seconds of history.
            else
