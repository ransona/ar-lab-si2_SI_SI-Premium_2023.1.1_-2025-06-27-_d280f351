classdef WaveformManager < scanimage.interfaces.Component
    ...
    methods
        ...
        function updateWaveforms(obj,forceOptimizationCheck)
            ...
            [ao_volts_raw, ao_samplesPerTrigger, sliceScanTime, pathFOV, frameTime] = ...
            ...
            if isfield(ao_volts_raw,'G')
                ...
                switch class(ss)
                    case {'scanimage.mroi.scannerset.ResonantGalvoGalvo', 'scanimage.mroi.scannerset.ResonantStageScanner'}
            ...
            if ~forceOptimizationCheck && ...
               ...
               isfield(obj.scannerAO,'frameTime') && isequal(obj.scannerAO.frameTime,frameTime) && ...
            ...
            scannerAO_.frameTime            = frameTime;
        ...
        function blankPowerBoxBeams(obj)
            if ~isempty(obj.scannerAO) && ~isempty(obj.scannerAO.ao_volts) && ~isempty(obj.scannerAO.ao_volts.Bpb)
                [row, col] = size(obj.scannerAO.ao_volts.Bpb);
                obj.scannerAO.ao_volts.Bpb = zeros(row, col);
            end
        end
        ...
        function plotWaveforms(obj,scanner)
            ...
            hFig = most.idioms.figure('Name','Waveform Output');
