classdef MotionManager < scanimage.interfaces.Component & most.HasClassDataFile
    ...
    methods        
            ...
            for idx = 1:length(hMEs)
                ...
                if isa(hME,obj.estimatorClassName)
                    ...
                    if hME.outOfDate
                        ...
                        hME_roiData = hME.roiData;
                        hME_enable = hME.enable;
                        hME.delete();
                        hME_new = obj.addEstimator(hME_roiData); % create new estimator from 
                        hME_new.enable = hME_enable;
                    end
                end
            end
        end
        ...
        %
        function tfActivated = activateMotionCorrectionSimple(obj,channel)
            tfActivated = false;
            if ~isempty(obj.hMotionEstimators)
        ...
        function resetMotionCorrection(obj,moveAxes)
            ...
            if most.idioms.isValidObj(obj.hSI.hPhotostim.hScan) && isprop(obj.hSI.hPhotostim.hScan, 'Ctl') && most.idioms.isValidObj(obj.hSI.hPhotostim.hScan.hCtl)
                obj.hSI.hPhotostim.hScan.hCtl.writeOffsetAngle([0,0]);
            end
...
%                 obj.setClassDataVar('lastFile',tifPath);
    ...
    methods
        ...
        function set.estimateOnNewAveragedFrames(obj,val)
            obj.estimateOnNewAveragedFrames = val;
            obj.nextFrameToEstimate = 0;
        end
    end
    ...
    methods (Hidden)
        ...
        function stripeData = estimateMotion(obj, stripeData)
            ...
            % don't compute motion on volume flyback frames (NaN indices)
            ...
            if obj.useAveragedStripe
                zs = unique(stripeData.zSeries);
                numZs = numel(zs);
                zIdx = find(zs == stripeData.zSeries(stripeData.zIdx),1);
                averagedStripeData = obj.hSI.hDisplay.rollingStripeDataBuffer{zIdx}{1}; % the first cell element of obj.stripeDataBuffer{zIdx} contains the averaged result
                roiDatas = averagedStripeData.roiData;
                if obj.estimateOnNewAveragedFrames % throttle 
                    currentFrame = roiDatas{1}.frameNumberAcqMode;
                    if obj.nextFrameToEstimate <= currentFrame || currentFrame < 10
                        estimate = true;
                        
                        currentFrame = roiDatas{1}.frameNumberAcqMode;
                        averageFactor = obj.hSI.hDisplay.displayRollingAverageFactor;
                        
                        if numZs == 1
                            %When planar imaging, Just image every average
                            %factor number of frames
                            obj.nextFrameToEstimate = currentFrame + averageFactor;
                        else
                            %When stack imaging, wait until a volume where all
                            %the averaged images are new and estimate for each
                            %slice in the volume.
                            
                            framesPerSlice = sum(stripeData.zSeries == zs(1));
                            framesPerVolume = framesPerSlice * numZs;
                            
                            slicesPerAveragedFrame = ceil(averageFactor/framesPerSlice);
                            currentVolume = floor(currentFrame/framesPerVolume);
                            estimateOnThisVolume = ~logical(mod(currentVolume,slicesPerAveragedFrame));
                            
                            if estimateOnThisVolume
                                obj.nextFrameToEstimate = currentFrame + framesPerSlice;
                            else
                                obj.nextFrameToEstimate = ceil(currentVolume/slicesPerAveragedFrame) * slicesPerAveragedFrame * framesPerVolume;
                            end
                        end
                    else
                        estimate = false;
        ...
        function updateMotionHistory(obj,hMotionEstimator,roiData,drPixel,confidence,correlation,userData,userDataStr)
            ...
            currentStagePosition = obj.hSI.hMotors.getPosition(obj.hSI.hMotors.hCSSampleAbsolute);
            currentStagePosition = currentStagePosition.points;
            
            %For estimaters from loaded tiffs. This will lead to erroneous sample power box
            %location correction until the tiff is reloaded when the live sample image is centered on the tiff image.
            if isempty(hMotionEstimator.roiData.absStagePosition)
               hMotionEstimator.roiData.absStagePosition = currentStagePosition; 
            end
            ...
            historyEntry.drStage = hMotionEstimator.roiData.absStagePosition - currentStagePosition; %um
                ...
                if correctionZPossible && useFastZ
                    if obj.hSI.hFastZ.outputActive
                        ...
                        for idx = 1:numel(hFastZs)
                            ...
                            aoz_min_ref(idx) = aoz_ref(1);
                            aoz_max_ref(idx) = aoz_ref(2);
                            
                            z_lims_ref  = sort( hFastZ.volts2Position(hFastZ.position2Volts(hFastZ.travelRange)) );
                            z_lim_min_ref(idx) = z_lims_ref(1);
                            z_lim_max_ref(idx) = z_lims_ref(2);
                        end
