    ...
    methods
        ...
        function set.channelName(obj,val)
            if obj.componentUpdateProperty('channelName',val)
                ...
                if ~obj.hSI.imagingSystemChangeInProgress
                    obj.hSI.hDisplay.resetDisplayFigs([],false);
                end
            end
        end
        ...
        function set.channelDisplay(obj,val)
            ...
            if obj.componentUpdateProperty('channelDisplay',val)
                ...
                deactivatedChans = setdiff(oldVal, val);

                obj.channelDisplay = val;
                
                if obj.mdlInitialized && ~obj.hSI.imagingSystemChangeInProgress
                    obj.hSI.hDisplay.resetDisplayFigs(activatedChans,false,[],deactivatedChans);
                end
            end
        end
        ...
        function set.channelInputRange(obj,val)
            if ~isempty(obj.hSI.hScan2D) && (iscell(val) || ~any(isnan(val(:))))
                if ~iscell(val)
                    ...
                    assert(all(size(val) == [obj.hSI.hScan2D.physicalChannelsAvailable 2]),'Value supplied for channelsInputRanges is of the wrong size.')
                    val = num2cell(val,2);
                end
        ...
        function set.colorMap(obj,val)
            ...
            hController = obj.hSI.hController;
            if isempty(hController)
                return;
            end

            hController = hController{1};
            channelImageHandler = hController.hGUIData.channelControlsV4.channelImageHandler;
            if ~isempty(hController) && ~isempty(channelImageHandler)
                channelImageHandler.updateTable(val);
                channelImageHandler.applyTableColorMapsToImageFigs();
                hController.hScanfieldDisplayControls.updateColorMaps();
            end

            obj.colorMap = val;
            pmImageColormap = hController.hGUIs.channelControlsV4.Children(7);
            pmImageColormap.Value = find(strcmpi(pmImageColormap.String,val));
        end
    end
    ...
    methods
        function registerChannels(obj,channelCountUpdateOnly)   
            ...
            if (nargin > 1) && channelCountUpdateOnly
                if any(obj.channelSave > obj.channelsAvailable)
                    obj.channelSave = obj.channelSave;
                end
                if any(obj.channelDisplay > obj.channelsAvailable)
