        ...
        function searchForFastZs(obj)
            ...
            if ~isequal(oldFastZs,obj.hFastZs)
                if ~isempty(obj.hFastZs)
                    obj.positionInterpolationAlgorithm = obj.hFastZs{1}.positionInterpolationAlgorithm;
                end
        ...
        function attachFastZListeners(obj)
            ...
            if ~isempty(obj.hFastZs)
                obj.hListeners(end+1) = most.ErrorHandler.addCatchingListener( ...
                    obj.hFastZs{1}, 'positionInterpolationAlgorithm', 'PostSet' ...
                    , @updateAlgorithm);
            end
            ...
            for idx = 1:numel(obj.hFastZs)
                ...
                if isempty(hFastZ.errorMsg)
                    try
                        ...
                        warnMessage = sprintf( ...
                            'Failed to park FastZ %s. Error:\n%s' ...
                            , hFastZ.name, ME.message);
                        most.idioms.warn(warnMessage);
                    end
                end
                
                obj.hListeners(end+1) = most.ErrorHandler.addCatchingListener( ...
                    hFastZ, 'targetPosition', 'PostSet', @dummySetPosition);
            end
            
            obj.hListeners(end+1) = most.ErrorHandler.addCatchingListener( ...
                obj.hSI, 'hScan2D', 'PostSet', @dummySetPosition);
            
            function updateAlgorithm(~, Event)
                hFastZ = Event.AffectedObject;
                obj.positionInterpolationAlgorithm = hFastZ.positionInterpolationAlgorithm;
            end
    ...
    methods
        ...
        function scanners = wrapFastZs(obj,hFastZs)
            ...
            for idx = 1:numel(hFastZs)
                ...
                scanner.positionInterpolationAlgorithm = hFastZ.positionInterpolationAlgorithm;
        ...
        function val = numDiscardFrames(obj)
            isRggScan = isa(obj.hSI.hScan2D,'scanimage.components.scan2d.RggScan');
            if (strcmp(obj.waveformType, 'sawtooth') || isRggScan) && length(obj.hSI.hStackManager.zs) > 1 && obj.hSI.hStackManager.actualNumVolumes > 1
                if ~obj.hasFastZ
                    val = 0;
                else
