...
classdef MariusMotionEstimator < scanimage.interfaces.IMotionEstimator
    ...
    methods (Access = protected)
        ...
        function motion_estimator_result = estimateMotionInternal(obj,roiData)            
            ...
            zIdx = find(round(roiData.zs .* 10) == round(obj.zs .* 10));% round to nearest 100 nm
    ...
    methods
        function preprocessVolume(obj,referenceRoiData)
            ...
            sliceMeans = mean(reshape(Z,[],1,nZs));
            Z  = bsxfun( @minus,   Z, sliceMeans );

            sliceRMSs = mean(reshape(Z.^2,[],1,nZs),1).^0.5;
            sliceRMSs(sliceRMSs==0) = 1;
            Z  = bsxfun( @rdivide, Z, sliceRMSs);
