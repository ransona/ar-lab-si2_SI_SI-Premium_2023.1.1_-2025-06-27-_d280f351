classdef RoiManager < scanimage.interfaces.Component
    ...
    methods
        ...
        function set.linesPerFrame(obj,val)
            ...
            if isa(hScan2D,'scanimage.components.scan2d.RggScan') && hScan2D.isStreamScanning
                stageScanner = obj.hSI.hMotors.hMotorXYZ{2};
                height = obj.currentRoiGroup.activeRois(1).scanfields.sizeXY(2) * obj.hSI.objectiveResolution;
               ...
               if val < minNumLines
                   val = ceil(minNumLines/2)*2;
                   most.idioms.warn('Y Scan Stage is moving too fast. Adjusting Y resolution to %d',val);
               end
            end
            
            if obj.componentUpdateProperty('linesPerFrame',val) && obj.linesPerFrame ~= val
        ...
        function set.pixelsPerLine(obj,val)
            ...
            if obj.hSI.hScanner.isPolygonalScanning
                
                sampleRate = obj.hSI.hScanner.sampleRate;
                pixelBinFactor = obj.hSI.hScanner.pixelBinFactor;
                lineRate = obj.hSI.hScanner.scannerFrequency;
                
                fillFractionSpatial = val * lineRate * pixelBinFactor / sampleRate;
                if obj.hSI.hScan2D.fillFractionSpatial ~= fillFractionSpatial
                    obj.hSI.hScanner.fillFractionSpatial = val * lineRate * pixelBinFactor / sampleRate;
                end
            end
        end
        ...
        function set.scanRotation(obj,val)
            ...
            if obj.componentUpdateProperty('scanRotation',val)
                ...
                %Side effects
                obj.updateLiveNonMroiImaging(false);
            end
        end
        ...
        function set.scanAngleMultiplierSlow(obj,val)
            ...
            if val < 1e-5
                most.idioms.warn('There is currently an unresolved bug saving TIFFs with slow scan angle multiplier < 1e-5. Defaulting to 1e-5.');
                val = 1e-5;
            end

            if val/obj.scanAngleMultiplierFast < 20/1920 % 20 lines on a 1080p monitor
                most.idioms.warn('The aspect ratio of this scan is thinner than can be comfortably visualized on a monitor. Display aspect ratio will be set to 1:1 for viewing, so it will not reflect the scanned aspect ratio.');
            end
        ...
        function set.scanAngleShiftSlow(obj,val)
            ...
            if obj.componentUpdateProperty('scanAngleShiftSlow',val)
                ...
                %Side effects
                obj.updateLiveNonMroiImaging(true);
            end
        end
    ...
    methods(Hidden)
        ...
        function updateLiveNonMroiImaging(obj,noDisplayReset)
            if ~obj.mroiEnable
                if ~obj.preventLiveUpdate
                    obj.hSI.hScan2D.updateLiveValues([],'RG');
                    obj.hSI.hFastZ.liveUpdate();
                    
                    if obj.hSI.active && ~noDisplayReset
    ...
    methods (Access = protected)
        ...
        %   Runs code that starts with the global acquisition-start command
            ...
            hScan2D = obj.hSI.hScan2D;
            isMirrorcleResonantAxis = hScan2D.hasResonantMirror && isa(hScan2D.hResonantScanner,'dabs.mirrorcle.ResonantAxis');
            isStreamScanning = isprop(hScan2D,'isStreamScanning') && hScan2D.isStreamScanning;
            if isMirrorcleResonantAxis || isStreamScanning
                obj.roiLive = false;
            else
                obj.roiLive = true;
            end
