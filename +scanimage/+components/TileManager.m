...
%             obj.hScanControlListeners = [obj.hScanControlListeners most.ErrorHandler.addCatchingListener(obj.hSI.hUserFunctions, 'frameAcquired', @obj.tileImageUpdated)];
            ...
            switch hScan2D.scannerType
                case {'RG', 'RGG', 'ResScan', 'Resscan'}
                    hScan2D.keepResonantScannerOn = 1;
                otherwise
                    hScan2D.keepResonantScannerOn = 0;
            end
            ...
            if obj.loopedAcquisition
                ...
                if isa(hScan2D, 'scanimage.components.scan2d.LinScan')
                    hScan2D.trigAcqInTerm = 'PFI0'; % Needs to be set for GG NI
                else
                    hScan2D.trigAcqInTerm = ''; 
                end
            ...
            if isa(hScan2D,'scanimage.components.scan2d.RggScan') && hScan2D.isStreamScanning && most.idioms.isValidObj(hScan2D.hCtl.hStream)
                nextTileIdx = obj.tileScanIndices(obj.tilesDone);
                obj.hSI.hDisplay.zprvResetBuffers(true);

                %Move just XY to centroid of starting tile. Z is set in startNextTileScan
                obj.hSI.hMotors.moveSample([obj.hScanTiles(nextTileIdx).samplePoint(1:2) nan])
            end            
        ...
        function done = startNextTileScan(obj)
            ...
            if done
                ...
                samplePosition = scanimage.mroi.coordinates.Points(obj.hSI.hCoordinateSystems.hCSSampleRelative,...
                    obj.hScanTiles(nextTileIdx).samplePoint);
                
                if isa(hScan2D,'scanimage.components.scan2d.RggScan') && hScan2D.isStreamScanning && most.idioms.isValidObj(hScan2D.hCtl.hStream)
                    % Zaber axes are busy when a stream is live
                    % regardless of whether any axes are moving. Therefore, the stream must be
                    % disabled before moves between tiles
                    hMotor = obj.hSI.hMotors.hMotorXYZ{2};
                    hScan2D.hCtl.hStream.abort();
                    hMotor.stop();
                    
                    hStackManager = obj.hSI.hStackManager;
                    hStackManager.setFocusPosition(samplePosition, hStackManager.stackActuator)
                else
                ...
                % set to false. 
                if obj.tileScanningInProgress
                    if isa(hScan2D,'scanimage.components.scan2d.RggScan') && hScan2D.isStreamScanning && most.idioms.isValidObj(hScan2D.hCtl.hStream) && obj.tilesDone > 1
                        % Zaber streams are aborted when a move command is issued to the controller
                        % regardless of which axis is commanded to move. Therefore, the stream must be
                        % restarted after each tile
                        hStream = hScan2D.hCtl.hStream;
                        
                        relativePositionPoint = samplePosition.transform(obj.hSI.hMotors.hCSAxesPosition);
                        relativePositionPoint = relativePositionPoint.points(1,1:2);
                        hStream.relativePosition = relativePositionPoint;
        ...
        function abortTileScan(obj, varargin)
            ...
            if isa(obj.hSI.hScan2D,'scanimage.components.scan2d.RggScan') && obj.hSI.hScan2D.isStreamScanning
                hMotor = obj.hSI.hMotors.hMotorXYZ{2};
                hScan2D = obj.hSI.hScan2D;
                hScan2D.hCtl.hStream.abort();
                hMotor.stop();
        ...
        function coerceDisplayAvgFactor(obj)
            avgFactor = obj.hSI.hDisplay.displayRollingAverageFactor;
            
            if obj.scanFramesPerTile < avgFactor
                obj.scanFramesPerTile = avgFactor;
            end
               
            arrayfun(@(st)coerce(st),obj.hScanTiles);
            
            function coerce(st)
               if st.displayAvgFactor < avgFactor
                  st.displayAvgFactor = avgFactor;
               end
            end
        end
