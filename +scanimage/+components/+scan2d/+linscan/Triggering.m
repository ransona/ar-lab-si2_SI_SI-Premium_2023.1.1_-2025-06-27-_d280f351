classdef Triggering < scanimage.interfaces.Class
    ...
    properties
        ...
        startTriggerTermOut = [];
        lineClockTermOut = '/port0/line5';
        pixelClockTermOut = 'PFI12';
    end
    ...
    properties (SetAccess = private)
        ...
        hPixelClockAcq;                        % handle to counter task that generates the Acq sample clock
        hPixelClockAcqChan;                    % handle to counter channel in hSampleClockAcq
        pixelClockAcqTermInt;                 % internal output terminal of the pixel clock task
        ...
        lineClockTask;                   % handle to digital output task indicative of the start of each line.
        hStartTriggerOutTask;            % handle to digital output task that outputs a pulse at the start of acquisition
    ...
    properties (Constant)
        ...
        PIXEL_CLOCK_CTR_CHAN = 3;               % Counter channel to be used to generate the frame clock
    ...
    methods
        ...
        function deinit(obj)
            ...
            obj.zprvClearTask('hPixelClockAcq');
            obj.zprvClearTask('hFrameClock');
            obj.zprvClearTask('hRefClock');
            obj.zprvClearTask('hDITriggerTimes');
            most.idioms.safeDeleteObj(obj.lineClockTask);
            most.idioms.safeDeleteObj(obj.hStartTriggerOutTask);
    ...
    methods
        function start(obj)
            ...
            if ~isempty(obj.lineClockTermOut)
                obj.configureLineClock();
                obj.lineClockTask.start();
            end
            
            if ~isempty(obj.startTriggerTermOut)
                obj.configureStartTriggerOutput();
                obj.hStartTriggerOutTask.start();
            end
            
            obj.hFrameClock.start();
            
            if ~isempty(obj.pixelClockTermOut)
                obj.hPixelClockAcqChan.pulseFreq = obj.sampleClkAcqFreq/obj.hLinScan.pixelBinFactor;
                obj.hPixelClockAcq.start();
            end
        ...
        function abort(obj)
            ...
            if most.idioms.isValidObj(obj.lineClockTask)
                obj.lineClockTask.abort();
                obj.lineClockTask.setChannelOutputValues(0);
            end
            
            if most.idioms.isValidObj(obj.hStartTriggerOutTask)
                obj.hStartTriggerOutTask.abort();
            end
            
            obj.zprvSaveAbortTask('hFrameClock');
            
            if ~isempty(obj.pixelClockTermOut)
                obj.zprvSaveAbortTask('hPixelClockAcq');
            end
    ...
    methods (Hidden)    
        ...
        function ziniPrepareTasks(obj)
            ...
            % set up pixel clock task
            if ~isempty(obj.pixelClockTermOut)
                obj.hPixelClockAcq = most.util.safeCreateTask([obj.hLinScan.name '-Pixel Clock']);
                obj.hPixelClockAcqChan = obj.hPixelClockAcq.createCOPulseChanFreq(deviceNameAux,...
                    obj.PIXEL_CLOCK_CTR_CHAN, 'Pixel Clock Acquisition Output', 1.25e6, 0.25);
                obj.hPixelClockAcq.cfgImplicitTiming('DAQmx_Val_ContSamps',2);
                obj.hPixelClockAcq.cfgDigEdgeStartTrig(obj.sampleClockAcqTermInt); % pixel clock is triggered by sample acq clock
                
                get(obj.hPixelClockAcqChan,'pulseFreq');
                set(obj.hPixelClockAcqChan,'pulseTerm',obj.pixelClockTermOut); % disconnect standard output terminal (for RTSI routing)
                obj.pixelClockAcqTermInt = sprintf('/%sInternalOutput',obj.hPixelClockAcqChan.chanNamePhysical);
            end
            ...
            %Set up beam clock that is not used as a start trigger for
            %anything
            if ~isempty(obj.lineClockTermOut)
                obj.lineClockTask = dabs.vidrio.ddi.DoTask(deviceNameAux, [obj.hLinScan.name '-Line Clock']);
                obj.lineClockTask.addChannel(obj.lineClockTermOut, [obj.hLinScan.name, '_Chan']);
                
                obj.lineClockTask.sampleMode = 'finite';
                obj.lineClockTask.startTrigger = obj.frameClockTermInt;
                obj.lineClockTask.startTriggerEdge = 'rising';
                obj.lineClockTask.allowRetrigger = true;
                obj.lineClockTask.hTask.sampClkTimebaseSrc = obj.hLinScan.hCtl.hAO.sampClkTimebaseSrc;
            end
            
            %Set up start trigger out task that just pulses to indicate
            %start of acquisition.
            if ~isempty(obj.startTriggerTermOut)
                obj.hStartTriggerOutTask = dabs.vidrio.ddi.DoTask(obj.startTriggerTermOut.deviceName, [obj.hLinScan.name '-Start Trigger Out']);
                obj.hStartTriggerOutTask.addChannel(obj.startTriggerTermOut.channelName, [obj.hLinScan.name, '_Chan']);

                obj.hStartTriggerOutTask.sampleMode = 'finite';
                obj.hStartTriggerOutTask.startTrigger = obj.frameClockTermInt;
                obj.hStartTriggerOutTask.triggerOnStart = false;
                obj.hStartTriggerOutTask.startTriggerEdge = 'rising';
                obj.hStartTriggerOutTask.allowRetrigger = false;
                obj.hStartTriggerOutTask.hTask.sampClkTimebaseSrc = obj.hLinScan.hCtl.hAO.sampClkTimebaseSrc;
            end
            ...
            if ~isempty(obj.referenceClockTermInt) && ~auxTrigBoardIsRefClkSrc
                ...
                if ~isempty(obj.pixelClockTermOut)
                    set(obj.hPixelClockAcq.channels(1),'ctrTimebaseSrc',obj.referenceClockTermInt);
                    set(obj.hPixelClockAcq.channels(1),'ctrTimebaseRate',obj.referenceClockRateInt);
                end
    ...
    methods
        ...
        function set.startTriggerTermOut(obj,val)
            if isempty(val)
                val = '';
            else
                isDIO = ~isempty(regexpi(val,'/port[0-9]/line[0-9]+$','match','once'));
                assert(isDIO,'Invalid value for acquisition start output trigger');
            end
            
            hDIO = obj.hLinScan.hResourceStore.filterByName(val);
            
            if ~isempty(hDIO)
                obj.startTriggerTermOut = hDIO;
            end
        end
        
        function set.lineClockTermOut(obj,val)
            if isempty(val)
                val = '';
            else
                val = regexpi(val,'/port[0-9]/line[0-9]+$','match','once');
                isDIO = ~isempty(val);
                assert(isDIO,'Invalid value for line clock out');
            end
            
            obj.lineClockTermOut = val;
        end
        
        function set.pixelClockTermOut(obj,val)
            if isempty(val)
                val = '';
            else
                val = regexpi(val,'PFI[0-9]+$','match','once');
                isPFI = ~isempty(regexpi(val,'PFI[0-9]+$','match','once'));
                assert(isPFI,'Invalid value for pixel clock out');
            end
            
            obj.pixelClockTermOut = val;
        end
    end
    ...
    methods (Access = private)
        ...
        function configureLineClock(obj)
            buffer = obj.generateBeamClockOutput;
            obj.lineClockTask.samplesPerTrigger = size(buffer,1);
            obj.lineClockTask.sampleRate = obj.hLinScan.sampleRateCtl;
            obj.lineClockTask.writeOutputBuffer(buffer);
            obj.lineClockTask.sampleClockTimebaseSource = obj.hLinScan.hCtl.hAO.sampClkTimebaseSrc;
            obj.lineClockTask.sampleClockTimebaseRate = obj.hLinScan.hCtl.hAO.sampClkTimebaseRate;
        end
        
        function configureStartTriggerOutput(obj)
            buffer = [1;0];
            obj.hStartTriggerOutTask.samplesPerTrigger = size(buffer,1);
            obj.hStartTriggerOutTask.sampleRate = obj.hLinScan.sampleRateCtl;
            obj.hStartTriggerOutTask.writeOutputBuffer(buffer);
            obj.hStartTriggerOutTask.sampleClockTimebaseSource = obj.hLinScan.hCtl.hAO.sampClkTimebaseSrc;
            obj.hStartTriggerOutTask.sampleClockTimebaseRate = obj.hLinScan.hCtl.hAO.sampClkTimebaseRate;
        end
        ...
        function waveform = generateBeamClockOutput(obj)
            persistent hWM
            
            if isempty(hWM) || ~most.idioms.isValidObj(hWM)
                rs = dabs.resources.ResourceStore();
                hWM = rs.filterByName('SI WaveformManager');
                if isempty(hWM) || ~most.idioms.isValidObj(hWM)
                   most.idioms.warn('Could not generate beam clock output.');
                   waveform = 0;
                   return;
                end
            end
            
            waveform = logical(hWM.scannerAO.pathFOV.clocks.xg);
            waveform(end) = 0;
        end
