        ...
        function updateBufferedOffsets(obj)
            if ~isempty(obj.acqParamBuffer)
                if ~obj.useFpgaOffset
                    channelsActive = obj.acqParamBuffer.channelsActive;
                    tmpValA = cast(obj.hLinScan.hSI.hChannels.channelOffset(channelsActive),obj.hLinScan.channelsDataType);
                    tmpValB = cast(obj.hLinScan.hSI.hChannels.channelSubtractOffset(channelsActive),obj.hLinScan.channelsDataType);
                    channelsOffset = tmpValA .* tmpValB .* int16((-1).^obj.hLinScan.channelsInvert(channelsActive));
                    obj.acqParamBuffer.channelsOffset = channelsOffset;
                else
                    channelsActive = obj.acqParamBuffer.channelsActive;
                    N = numel(obj.hLinScan.hSI.hChannels.channelOffset);
                    obj.acqParamBuffer.channelsOffset = zeros(1,numel(channelsActive),obj.hLinScan.channelsDataType);
                    ...
                    obj.hFpga.AcqParamLiveChannelOffsets(1:N) = -channelsOffset_FPGA .* double((-1).^obj.hLinScan.channelsInvert);
        ...
        function zzConfigInputEveryNAndBuffering(obj)
            ...
            if obj.isLineScan
                ...
                if obj.hLinScan.recordScannerFeedback
                    ...
                    obj.hLinScan.lastFramePositionData = nan(obj.acqParamBuffer.fdbkSamplesPerFrame,3);
                else
