classdef Acquisition < scanimage.interfaces.Class
    ...
    properties (Hidden, Dependent)
        ...
        enableFpgaPixelCorrection;
        fpgaPixelCorrectionMultiplier;
    end
    ...
    methods        
        ...
        function reinit(obj)
            ...
            obj.acquisitionEngineIdx = obj.hScan.acquisitionEngineIdx;
    ...
    methods
        ...
        function val = get.hostPixelCorrectionMultiplier(obj)
            rawMultiplier = obj.hScan.hostPixelCorrectionMultiplier;

            if isnumeric(rawMultiplier)
                val = rawMultiplier;
            elseif ischar(rawMultiplier)
                val = obj.parsePixelCorrectionString(rawMultiplier);
            else
                val = 1;
            end
        end

        function val = get.enableFpgaPixelCorrection(obj)
            val = obj.hScan.enableFpgaPixelCorrection;
        end

        function val = get.fpgaPixelCorrectionMultiplier(obj)
            rawMultiplier = obj.hScan.fpgaPixelCorrectionMultiplier;
    ...
    methods (Hidden)
        function zprpUpdateMask(obj)
            % constant coefficient pixel multiplication
            coefficient = most.idioms.ifthenelse(obj.enableFpgaPixelCorrection,obj.fpgaPixelCorrectionMultiplier,1);

            if obj.acqModeIsLinear
                maxBinSize = obj.hScan.sampleRateDecim;
                validateBinSize(maxBinSize); % validate before writing acquistion regs
                obj.hAcqEngine.acqParamUniformSampling = 1;
                obj.hAcqEngine.acqParamUniformBinSize = uint32(maxBinSize);
                obj.hAcqEngine.acqParamUniformDivisor = obj.hAcqEngine.convertToFixedPointPrecision(1/double(maxBinSize)*coefficient);
            elseif obj.hScan.uniformSampling
                ...
                assert(all(obj.mask > 0),sprintf('Under the current acquisition configuration, several pixels won''t receive any samples.\nI.e., the horizontal pixel resolution is too high for the acquisition sample rate. Reduce pixels per line or enable uniform sampling.'));
                ...
                validateBinSize(maxBinSize); % validate before writing acquistion regs/mask table

                for i = 0:(N-1)
                    binVal = obj.mask(i+1);
                    multiplier = 1/double(binVal) * coefficient;
                    obj.hAcqEngine.writeMaskTable(i,binVal,multiplier,obj.hFpga.ACCUM_COEFFICIENT_FIXED_POINT_WIDTH);
                end
            ...
            assert((maxBinSize == 1) || obj.hAcqEngine.ACCUM_MULTIPLY_SUPPORT, 'Current FPGA bitfile is incompatible with requested scan.');

            % ensure the bin size is within what the FPGA can support & count up to
            function validateBinSize(binSize)
                maxPossibleBinSize = 2^obj.hFpga.ACCUM_BITS - 1;
                assert(binSize <= maxPossibleBinSize, ['This resolution / sample rate setting would require binning more '...
                    'samples into a pixel than is currently possible, try reducing sample rate or increasing pixel resolution.\n'...
                    'Max samples per pixel bin: %d, attempted to use %d samples per bin which is too high.'], maxPossibleBinSize, binSize);
            end
        end
        ...
        function zprpResizeAcquisition(obj)
            ...
            if ~obj.isLineScan
                ...
                %In case of partial intersection of stack slices with ROI
                nonEmptyScanFieldsMask = cellfun(@(sf)~isempty(sf),obj.acqParamBuffer.scanFields);
                
                if ~all(nonEmptyScanFieldsMask)
                    most.idioms.warn('Some stack slices do not intersect with any ROIs.');
                end

                sf = obj.acqParamBuffer.scanFields{find(nonEmptyScanFieldsMask,1)}{1};
        ...
        function initI2c(obj)
            ...
            if enable
                ...
                if obj.hScan.i2cSendAck
                    ...
                    aeId = obj.hScan.acquisitionEngineIdx-1;
    ...
    methods (Hidden)
        ...
        function applyVirtualChannelSettings(obj)
            ...
            % check if lasergating is valid
            clockTrigger = isa(obj.hScan.LaserTriggerPort,'dabs.resources.ios.CLKI') || obj.hScan.useCustomFilterClock;
            diTrigger = isa(obj.hScan.LaserTriggerPort,'dabs.resources.ios.DI');
            multiplier = obj.hScan.customFilterClockPeriod;
            compatibleMultiplier = mod(multiplier,1)==0 && multiplier > 1; % integer > 1
            laserGatingAvailable = diTrigger || (clockTrigger && compatibleMultiplier);

            % lasergating only valid when trigger port is enabled and clock multiplier > 1 and integer
            % otherwise, disable lasergating to prevent frame errors
            if ~laserGatingAvailable
...
%                 obj.lastLinearStripe = [];
                ...
                if acqStatus.numDroppedFramesLogger > 0
                    ...
                    acqType = obj.hScan.hSI.acqState;
                    obj.hScan.hSI.abort();
                    
                    errordlg(errorMsg,'Error during acquisition','modal');
                    success = false;
                    evtData = most.GenericEventData(acqType);
                    obj.hScan.hSI.hUserFunctions.notify('lostFrames',evtData);
                    return
                end
                
                if acqStatus.fifoOverflows
                    errorMsg = 'Acquisition data lost. Data processing may lag behind acquisition if PCIe bandwidth is insufficient.';
        ...
        function tfFilterClockPeriodChanged = updateFilterClockParams(obj)
            if isempty(obj.hAcqEngine) || isempty(obj.hScan.virtualChannelSettings)
                % This function is also called when loading the RGG virtual channel settings from 
                % the MDF before the Acquisition engine is initialized. 
                % This seems to prematurely update the sample rate. 
                % Until we call obj.reinit, let's not try to update the sample rate.
                tfFilterClockPeriodChanged = false;
                return
            end
        ...
        function bufferAcqParams(obj,live)
            ...
            elseif ~isempty(obj.acqParamBuffer.scanFields{1})
                if obj.hScan.uniformSampling
                    ...
                    calculatedPixelsPerLine = 1/linesPerPeriod * obj.stateMachineSampleRate * obj.hScan.fillFractionTemporal / (obj.hScan.scannerFrequency * obj.hScan.pixelBinFactor);
                    if calculatedPixelsPerLine - floor(calculatedPixelsPerLine) < 0.001
                        obj.pixelsPerLine = floor(calculatedPixelsPerLine);
                    else
                        obj.pixelsPerLine = ceil(calculatedPixelsPerLine);
                    end
                else
...
%                     end
            ...
            function plotMeasurement(measurements)
                ...
                if isempty(hFig) || ~ishghandle(hFig)
                    hFig = most.idioms.figure('Name','Resonant scanner frequency');
                end
    ...
    methods (Access = private)        
        function fpgaStartAcquisitionParameters(obj)
            ...
            periodTriggerDebounceWidth = 10;
            maxPeriodTriggerDebounce = 2^periodTriggerDebounceWidth-1;
            userPeriodTriggerDebounce = ceil(obj.stateMachineLoopRate * obj.hScan.mdfData.PeriodClockDebounceTime);
            obj.hAcqEngine.acqParamPeriodTriggerDebounce = min([userPeriodTriggerDebounce maxPeriodTriggerDebounce]);

            triggerDebounceWidth = 5;
            maxTriggerDebounce = 2^triggerDebounceWidth-1;
            userTriggerDebounce = ceil(obj.stateMachineLoopRate * obj.hScan.mdfData.TriggerDebounceTime);
            obj.hAcqEngine.acqParamTriggerDebounce = min([userTriggerDebounce maxTriggerDebounce]);
            ...
            if obj.auxTriggersEnable
                auxTriggerDebounceWidth = 5;
                maxAuxTriggerDebounce = 2^auxTriggerDebounceWidth-1;
                calculatedAuxTriggerDebounce = floor(obj.hScan.auxTriggersTimeDebounce * obj.hFpga.dataClkRate * 1e6);
                obj.hAcqEngine.acqParamAuxTriggerDebounce = min([calculatedAuxTriggerDebounce maxAuxTriggerDebounce]);
        ...
        function val = parsePixelCorrectionString(obj,str)
            switch lower(str)
                case 'minbin'
                    val = min(obj.mask);
                case 'maxbin'
