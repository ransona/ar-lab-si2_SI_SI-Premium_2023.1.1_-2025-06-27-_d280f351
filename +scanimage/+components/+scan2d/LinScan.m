classdef LinScan < scanimage.components.Scan2D & most.HasMachineDataFile & dabs.resources.configuration.HasConfigPage
    ...
    methods
        function loadMdf(obj)
            ...
            success = success & obj.safeSetPropFromMdf('startTriggerTermOut', 'startTriggerTermOut');
            success = success & obj.safeSetPropFromMdf('lineClockTermOut', 'lineClockTermOut');
            success = success & obj.safeSetPropFromMdf('pixelClockTermOut', 'pixelClockTermOut');
        ...
        function saveMdf(obj)
            ...
            obj.safeWriteVarToHeading('startTriggerTermOut', obj.startTriggerTermOut);
            obj.safeWriteVarToHeading('lineClockTermOut', obj.lineClockTermOut);
            obj.safeWriteVarToHeading('pixelClockTermOut', obj.pixelClockTermOut);
    ...
    methods        
        ...
        function set.startTriggerTermOut(obj,val)
            if isempty(val)
                ...
                isDIO = ~isempty(regexpi(val,'/port[0-9]/line[0-9]+$'));
                assert(isDIO,'Invalid value for start trigger output');
            end
            
            obj.startTriggerTermOut = val;
            
            if ~obj.hTrig.routesInitted
                obj.hTrig.startTriggerTermOut = obj.startTriggerTermOut;
            end
        end
        ...
        function set.lineClockTermOut(obj,val)
            if isempty(val)
                ...
                isDIO = ~isempty(regexpi(val,'/port[0-9]/line[0-9]+$'));
                assert(isDIO,'Invalid value for beam clock out');
            end
            
            obj.lineClockTermOut = val;
            
            if ~obj.hTrig.routesInitted
                obj.hTrig.lineClockTermOut = obj.lineClockTermOut;
            end
        end
        ...
        function set.pixelClockTermOut(obj,val)
            ...
            oldVal = obj.pixelClockTermOut;
            obj.pixelClockTermOut = val;
            
            if ~obj.hTrig.routesInitted
                obj.hTrig.pixelClockTermOut = obj.pixelClockTermOut;
            end
        end
    ...
    methods (Access = protected, Hidden)
        function val = accessScannersetPostGet(obj,~)
            ...
            if ~isempty(obj.hBeams)
                if obj.hCtl.beamShareGalvoDAQ || obj ~= obj.hSI.hScan2D
...
function s = defaultMdfSection()
    s = [...
        ...
        most.HasMachineDataFile.makeEntry('Output Clocks')... % comment only
        most.HasMachineDataFile.makeEntry('startTriggerTermOut', '','Port on digital IO DAQ (PX.X) to output pulse at start of acquisition.')...
        most.HasMachineDataFile.makeEntry('lineClockTermOut', '','Port on digital IO DAQ (PX.X) to output start of line.')...
        most.HasMachineDataFile.makeEntry('pixelClockTermOut', '','Port on digital IO DAQ (PFI) to output start of pixel.')...
        ];
end
