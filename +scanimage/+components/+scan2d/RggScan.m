classdef RggScan < scanimage.components.Scan2D & most.HasMachineDataFile & dabs.resources.configuration.HasConfigPage
    ...
    properties (SetObservable)
        ...
        % for multi-arm, multi-scanimage instance acquisitions
        acquisitionEngineIdx = 1;            % [numeric] the index of the vDAQ acquisition engine to use
    end
    ...
    properties (Hidden)
        ...
        scanModePropsToCache = struct('linear',{{'sampleRate' 'pixelBinFactor' 'fillFractionSpatial' 'bidirectional' 'stripingEnable' 'linePhase'}},...
            ...
            'stage',{{'uniformSampling' 'pixelBinFactor' 'fillFractionSpatial' 'bidirectional' 'stripingEnable'}});
        sampleRateDecim = 1;
    ...
    properties (Transient, Hidden, SetObservable)
        ...
        LRRLaserSyncPort = dabs.resources.Resource.empty();
        ...
        enableFpgaPixelCorrection = false;
        fpgaPixelCorrectionMultiplier = 'MAXBIN';
    end
    ...
    properties(SetAccess = {?scanimage.SI})
        isPolygonalScanning = false;  % logical, indicates if acquisition should be configured for polygonal scanning
        isStreamScanning = false;     % Logical indicating if the yGalvo is currently a motor
    end
    ...
    methods
        ...
        function validateConfiguration(obj)
            try
                ...
                if obj.isStreamScanning
                    assert(~isempty(obj.frameClockOut), 'When stage scanning, you need to define the frame clock out trigger');
                end
        ...
        function reinit(obj)
            ...
            try
                ...
                % load initial channel settings (after FPGA initialization)
                ...
                obj.isPolygonalScanning = isa(obj.hResonantScanner,'dabs.resources.devices.PolygonalScanner') && strcmpi(obj.scanMode,'resonant');
                if obj.isPolygonalScanning
...
%                     obj.fillFractionSpatial = obj.fillFractionTemporal;
    ...
    methods
        function loadMdf(obj)
            ...
            success = success & obj.safeSetPropFromMdf('desiredSampleRateCtl', 'desiredSampleRateCtl');
            ...
            success = success & obj.safeSetPropFromMdf('LRRLaserSyncPort', 'LRRLaserSyncPort');
            ...
            success = success & obj.safeSetPropFromMdf('enableFpgaPixelCorrection', 'enableFpgaPixelCorrection');
            success = success & obj.safeSetPropFromMdf('fpgaPixelCorrectionMultiplier', 'fpgaPixelCorrectionMultiplier');
            ...
            success = success & obj.safeSetPropFromMdf('acquisitionEngineIdx','acquisitionEngineIdx');
        ...
        function saveMdf(obj)
            ...
            obj.safeWriteVarToHeading('desiredSampleRateCtl',obj.desiredSampleRateCtl);
            ...
            obj.safeWriteVarToHeading('LRRLaserSyncPort', obj.LRRLaserSyncPort);
            
            obj.safeWriteVarToHeading('enableFpgaPixelCorrection', obj.enableFpgaPixelCorrection);
            obj.safeWriteVarToHeading('fpgaPixelCorrectionMultiplier', obj.fpgaPixelCorrectionMultiplier);
            ...
            obj.safeWriteVarToHeading('acquisitionEngineIdx', obj.acquisitionEngineIdx);
    ...
    methods
        ...
        function set.yGalvo(obj,val)
            ...
            if ~isequal(val,obj.yGalvo)
                if most.idioms.isValidObj(val)
                    validateattributes(val,{'dabs.resources.devices.GalvoAnalog','dabs.resources.devices.MotorController'},{'scalar'});
                end
                ...
                obj.yGalvo = val;
                if obj.isStreamScanning
                    obj.yGalvo.registerUser(obj,'Y Stream');
                else
        ...
        function set.LRRLaserSyncPort(obj,val)
            val = obj.hResourceStore.filterByName(val);
            ...
            obj.LRRLaserSyncPort.unregisterUser(obj);
            obj.LRRLaserSyncPort = val;
            obj.LRRLaserSyncPort.registerUser(obj,'LRR Laser Sync');
        ...
        function val = get.LRRLaserSyncPort(obj)
            val = obj.LRRLaserSyncPort;
            if ~most.idioms.isValidObj(val)
                val = dabs.resources.Resource.empty();
            end
        end
        ...
        function set.sampleRateDecim(obj,val)
            validateattributes(val,{'numeric'},{'scalar','nonempty','nonnan','finite','real','positive'});
            obj.sampleRateDecim = val;
        end
        
        function set.sampleRateCtl(obj,~)
            minDecim = max(ceil(obj.ctlTimebaseRate/obj.sampleRateCtlMax),2);
            ctlDecims = minDecim:(100*minDecim);
            ...
            if obj.hSI.hRoiManager.isLineScan || ~obj.scanModeIsLinear
                ...
                desiredDecim = obj.ctlTimebaseRate/obj.desiredSampleRateCtl;
                obj.sampleRateCtlDecim = round(desiredDecim);
                ...
                if ~isempty(sf)
                    ...
                    for i = 1:numel(lineAcqSamps)
                        ...
                        validCtlTbPulses = most.floatingpoint.fuzzyEquals(ctlTbPulses, round(ctlTbPulses));
                        ctlTbPulses = round(ctlTbPulses(validCtlTbPulses));
                        
                        res = ctlMults * ctlTbPulses;
                        validResults = most.floatingpoint.fuzzyEquals(res, round(res));
                        res(validResults) = round(res(validResults));
                        
                        idxs = find(validResults);
                    ...
                    desiredCtlMult = obj.desiredSampleRateCtl/obj.ctlTimebaseRate;
                    ctlMult = interp1(ctlMults,ctlMults,desiredCtlMult,'nearest','extrap');
                    obj.sampleRateCtlDecim = 1/ctlMult;
                    obj.ctlRateGood = true;
                end
            end
        end
            ...
            if obj.mdlInitialized && obj.numInstances > 0
                if ~obj.active && obj.scanModeIsResonant
                    ...
                    if obj.keepResonantScannerOn
                        deg = obj.hCtl.nextResonantFov();
                    else
                        deg = 0;
                    end
                    
                    if isa(obj.hResonantScanner,'dabs.resources.devices.PolygonalScanner')
        ...
        function rg = get.angularRange(obj)
            ...
            if ~obj.isStreamScanning
                ...
                hMotors =  obj.hSI.hMotors;
                hMotorY = hMotors.hMotorXYZ{2};
                yAxisIdx = hMotors.motorAxisXYZ(2);
                %Take this with a grain of salt as the stage may be rotated relative to the scanning
                %axes, in which case the scannable range may be less than the full single axis
                %range depending on starting stage location.
                y = diff(hMotorY.range_Degrees(yAxisIdx,:));
            end
            rg = [x y];
        end
        ...
        function set.scanMode(obj,v)
            ...
            elseif obj.numInstances && ~strcmp(v,obj.scanMode)
                ...
                switch v
                    ...
                    case 'stage'
                        hMotorsXY = obj.hSI.hMotors.hMotorXYZ([1 2]);
                        assert(all(cellfun(@(hMotor)isa(hMotor,'dabs.zaber.ZaberMultiDevice'),hMotorsXY)),'Motors need to be a zaber stage that supports streamed movements and has digital inputs');
                        obj.isStreamScanning = true;
                    otherwise
        ...
        function set.virtualChannelSettings(obj,v)
            ...
            % stay empty if settings are still loading
            % (empty is checked during reinit, default settings will be set then if mdf is really empty)
            if isempty(v) || isempty(ds)
                obj.virtualChannelSettings = [];
                return
            end
            
            % setup and validate channel settings
            nms = fieldnames(ds);
        ...
        function set.customFilterClockPeriod(obj,v)
            try
                validateattributes(v,{'numeric'},{'nonnegative','real','finite','scalar'});
            catch ME
                most.ErrorHandler.logAndReportError(false,ME);
            end
        ...
        function set.acquisitionEngineIdx(obj,v)
            validateattributes(v,{'numeric'},{'scalar' 'integer' 'nonnegative'});

            if most.idioms.isValidObj(obj.hAcq) && most.idioms.isValidObj(obj.hAcq.hFpga)
                numAcquisitionEngines = max([obj.hAcq.hFpga.NUM_AE 1]);
                assert(v <= numAcquisitionEngines, 'Cannot use acquisition engine %d, the number of engines in this configuration is %d', v, numAcquisitionEngines);
            end

            if obj.mdlInitialized && v ~= obj.acquisitionEngineIdx
                most.idioms.warn('Please restart ScanImage for Acquisition Engine changes to take affect');
            end

            obj.acquisitionEngineIdx = v;
        end
    end
...
%                 error('Bidirectional scanning is unsupported when using a polygonal Scanner');
        ...
        function val = accessLinePhasePreSet(obj,val)
            if obj.scanModeIsResonant && ~obj.robotMode && obj.mdlInitialized
                ...
                if (currentAmplitude_deg > 0) && ~obj.internalPhaseUpdate && ~obj.isPolygonalScanning
        ...
        function val = accessFlybackTimePerFramePostGet(obj,val)
            if obj.isStreamScanning
                hMotor = obj.hSI.hMotors.hMotorXYZ{2};
                v = hMotor.streamMaxVelocity_umPerS;
                a = hMotor.streamAcceleration_umPerSs;
                
                mn = v/a + diff(hMotor.range_um(obj.hSI.hMotors.motorAxisXYZ(2),:))/v + v/a;
            else
                if obj.scanModeIsResonant
                    mn = 1/obj.scannerFrequency;
                else
    ...
    methods
        ...
        function updateLiveValues(obj,regenAO,waveforms)
            ...
            function estimateLinePhase()
                if obj.scanModeIsResonant && ~obj.isPolygonalScanning
    ...
    methods        
        function resFreq = measureScannerFrequency(obj)
            ...
            if obj.componentExecuteFunction('measureScannerFrequency')
                ...
                if isa(obj.hResonantScanner,'dabs.mirrorcle.ResonantAxis')
                    resFreq = obj.hResonantScanner.currentFrequency_Hz;
                    return;
                end
                ...
                elseif isa(obj.hResonantScanner,'dabs.resources.devices.PolygonalScanner')
    ...
    methods (Hidden)
        function ss = getScannerset(obj, enableResonant, forStim)
            ...
            if most.idioms.isValidObj(obj.xGalvo)
                ...
                xGalvoScanner.actuatorLag_ms = obj.xActuatorLag_ms;
            ...
            yGalvoScanner.actuatorLag_ms = obj.yActuatorLag_ms;
            yGalvoScanner.sampleRateHz = ctlFs;
            
            % Define Stage Scanning Hardware
            if obj.isStreamScanning
                stageScanner = scanimage.mroi.scanners.StageScanner(obj.hSI.hMotors.hMotorXYZ{2});
                stageScanner.flytoTimeSeconds = obj.coercedFlytoTime;
                stageScanner.flybackTimeSeconds = flybackTime;
                stageScanner.axesXY = obj.hSI.hMotors.motorAxisXYZ([1 2]);
            end
            ...
            if enableResonant
                ...
                if ~obj.isStreamScanning
                    ...
                    ss=scanimage.mroi.scannerset.ResonantStageScanner(obj.name,r,xGalvoScanner,yGalvoScanner,stageScanner,fastBeams, slowBeams,fastZScanners,obj.fillFractionSpatial);
                    ss.useScannerTimebase = obj.hCtl.useScannerSampleClk;
                end
            else
            ...
            ss.hCSAxesPosition = obj.hSI.hMotors.hCSAxesPosition;
    ...
    methods (Hidden)
        ...
        function v = coercedTime(obj,v)
            if obj.scanModeIsResonant
                ...
                validRates = most.floatingpoint.fuzzyEquals(acqTicks, round(acqTicks));
                assert(any(validRates),'No solution for current scan configuration. Adjust and retry.');
                acqTicks = round(acqTicks(validRates));
                times = acqTicks / obj.sampleRate;
    ...
    methods (Hidden)
        function arm(obj)
            if obj.scanModeIsResonant
                ...
                if resAmplitude > 0
                    if isa(obj.hResonantScanner,'dabs.mirrorcle.ResonantAxis')
                        %Mirrorcle resonant scan frequency is arbitrary, no
                        % need to set amplitude to estimate it.
                        newFreq = obj.estimateScanFreq();
                    elseif isa(obj.hResonantScanner,'dabs.resources.devices.ResonantScanner')
        ...
        function newPhase = calibrateLinePhase(obj)
            if obj.scanModeIsLinear
                ...
                assert(~obj.isPolygonalScanning,'Automatic line phase calibration is unsupported when using a polygonal scanner');
    ...
    methods (Access = protected, Hidden)
        function mdlInitialize(obj)
            ...
            function s = defaultModeProps()
                ...
                if obj.isPolygonalScanning
                    desiredPixelBinFactor = 50;
                else
                    desiredPixelBinFactor = 1;
                end
                
                s.linear = struct('pixelBinFactor', 8,'sampleRate', lsr, 'flybackTimePerFrame', obj.defaultFlybackTimePerFrame, 'flytoTimePerScanfield', obj.defaultFlytoTimePerScanfield,...
                    'fillFractionSpatial', .9, 'stripingEnable', true, 'linePhase', 0);
                s.resonant = struct('pixelBinFactor', desiredPixelBinFactor, 'flybackTimePerFrame', obj.defaultFlybackTimePerFrame, 'flytoTimePerScanfield', obj.defaultFlytoTimePerScanfield, ...
...
function s = defaultMdfSection()
    s = [...
        ...
        most.HasMachineDataFile.makeEntry('desiredSampleRateCtl',200000,'Sampling rate to use for galvo, beams, and fastZ control')...
        most.HasMachineDataFile.makeEntry()... % blank line
        ...
        most.HasMachineDataFile.makeEntry('enableFpgaPixelCorrection', false, 'Enable constant coefficient pixel multiplication on the vDAQ FPGA')...
        most.HasMachineDataFile.makeEntry('fpgaPixelCorrectionMultiplier', 'MAXBIN', 'Numeric or string, e.g. 500, ''MAXBIN'', ''MINBIN'', ''MEANBIN''')...
        most.HasMachineDataFile.makeEntry('enableHostPixelCorrection', false, 'Enable constant coefficient pixel multiplication on the host PC')...
        most.HasMachineDataFile.makeEntry('hostPixelCorrectionMultiplier', 500, 'Numeric or string, e.g. 500, ''MAXBIN'', ''MINBIN'', ''MEANBIN''')...
        most.HasMachineDataFile.makeEntry()... % blank line
        ...
        most.HasMachineDataFile.makeEntry('LRRLaserSyncPort', '','Digital input where laser trigger is daisy chained (when plugged into CLK_IN) when using lower repetition rate lasers.')...
        most.HasMachineDataFile.makeEntry()... % blank line
