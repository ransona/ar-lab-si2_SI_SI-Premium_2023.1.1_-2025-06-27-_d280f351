classdef Photostim < scanimage.interfaces.Component & most.HasMachineDataFile & dabs.resources.configuration.HasConfigPage
    ...
    properties (Transient)
        ...
        pairStimActiveOutputChannel = true; % If true, Photostim will have ann error state unless it is the case that the configured stim active output is also the auxIn3 port of all other RggScan scanners.
        pairBeamActiveOutputChannel = true; % If true, Photostim will have ann error state unless it is the case that the configured beam active output is also the auxIn4 port of all other RggScan scanners.
    end
    ...
    properties (SetObservable, SetAccess = private)
        ...
        % For on demand mode. Indicates which stimulus is currently in the buffer ready to go
        primedStimulus;
    ...
    properties (Hidden,SetAccess=private)
        ...
        trigTermString;
        syncTermString; % NOTE: only used by NI hardware. Not used by vDAQ
    ...
    methods (Hidden)
        function loadMdf(obj)
            ...
            success = success & obj.safeSetPropFromMdf('stimTriggerTerm','stimTriggerTerm');
            ...
            success = success & obj.safeSetPropFromMdf('pairStimActiveOutputChannel', 'pairStimActiveOutputChannel');
            success = success & obj.safeSetPropFromMdf('pairBeamActiveOutputChannel', 'pairBeamActiveOutputChannel');
        ...
        function saveMdf(obj)
            ...
            obj.safeWriteVarToHeading('stimTriggerTerm',obj.stimTriggerTerm);
            ...
            obj.safeWriteVarToHeading('pairStimActiveOutputChannel', obj.pairStimActiveOutputChannel);
            obj.safeWriteVarToHeading('pairBeamActiveOutputChannel', obj.pairBeamActiveOutputChannel);
        end
        ...
        function validateConfiguration(obj)
            try
                if most.idioms.isValidObj(obj.hScan)
                    ...
                    [stimTriggerPaired, beamTriggerPaired] = obj.checkOrSetRggScansAuxilliaryTriggerPairing(false);
                    stimTriggerPairFailed = obj.pairStimActiveOutputChannel && ~stimTriggerPaired; % pair____ActiveOutputChannel only indicates the intention to pair
                    beamTriggerPairFailed = obj.pairBeamActiveOutputChannel && ~beamTriggerPaired;
                    
                    hRggScans = obj.hResourceStore.filterByClass('scanimage.components.scan2d.RggScan');
                    
                    if obj.isVdaq() && numel(hRggScans) > 1
                        if stimTriggerPairFailed || beamTriggerPairFailed
                            if isempty(obj.stimActiveOutputChannel) && isempty(obj.beamActiveOutputChannel)
                                str = sprintf(['Stim and beam output triggers left unconfigured while pairing is enabled. Assign terminals in Photostim advanced tab' ...
                                    ' and leave ''Pair'' checked to save timestamps of stimulations in TIFF files when doing simultaneous imaging and photostimulation.']);
                            else
                                str = sprintf(['Pairing output triggers to auxiliary inputs failed.\n\n'...
                                    'Go to advanced tab of Photostim page and configure stim and beam output channels.\n'...
                                    'When pairing is enabled, this allows TIFFs acquired in a parallel to record timestamps when stimulations occur.\n'...
                                    'This error can show if aux triggers 3 and 4 of imaging systems were previously configured.']);
                            end
                            error(str);
                        end
                        
                        if ~obj.pairStimActiveOutputChannel && ~stimTriggerPaired && ~obj.pairBeamActiveOutputChannel && ~beamTriggerPaired
                            obj.warnMsg = sprintf('Output trigger ports not paired. This means that timestamps of stimulations won''t be collected for imaging scanners. If this is the preferred behavior, then please ignore this warning.');
                        else
                            obj.warnMsg = '';
                        end
                    end
        ...
        function [stimTriggerPaired, beamTriggerPaired] = checkOrSetRggScansAuxilliaryTriggerPairing(obj,setPair)
            %Returns whether or not the stim trigger shares a terminal with
            %all RggScans' auxiliary trigger 3 and beam trigger shares a
            %terminal with auxilary trigger 4. It will try to pair them if
            %set Pair is set to true, but will fail if the auxiliary ports
            %are already (perhaps intentionally) set to other terminals.
            
            if ~obj.isVdaq()
                stimTriggerPaired = false;
                beamTriggerPaired = false;
                return;
            end
            
            rggScans = obj.hResourceStore.filterByClass('scanimage.components.scan2d.RggScan');
            
            stimTriggerPaired = true;
            beamTriggerPaired = true;
            
            for rggIdx = 1:numel(rggScans)
                rggScan = rggScans{rggIdx};
                
                stimTriggerAlreadyPaired = isequal(obj.stimActiveOutputChannel,rggScan.auxTrigger3In);
                beamTriggerAlreadyPaired = isequal(obj.beamActiveOutputChannel,rggScan.auxTrigger4In);
                
                if setPair
                    if most.idioms.isValidObj(obj.stimActiveOutputChannel) && obj.pairStimActiveOutputChannel && ~stimTriggerAlreadyPaired
                        if ~most.idioms.isValidObj(rggScan.auxTrigger3In)
                            auxTriggerIn = 'auxTrigger3In';
                            most.idioms.safeSetProp(rggScan,auxTriggerIn,obj.stimActiveOutputChannel.name);
                            stimTriggerPaired = stimTriggerPaired && true;
                        else
                            str1 = sprintf('Share stim active output trigger terminal with imaging scanners'' auxiliary trigger 3?');
                            str2 = sprintf(['It is recommended for dual imaging and photostimulating microscopes with vDAQ' ...
                                ' to share photostim output triggers with all imaging systems'' auxiliary inputs (3 and 4).' ...
                                ' The benefit is that these output triggers will be timestamped to concurrently recorded ScanImage TIFFs.\n\n' ...
                                'Auxiliary trigger 3 of scanner %s was already configured with another port, %s. This could be for good reason. Verify cabling.\n\n' ...
                                'Should this port be replaced with Photostim''s currently configured stim trigger port, %s?'], rggScan.name, rggScan.auxTrigger3In.name, obj.stimActiveOutputChannel.name);
                            
                            dialogWindow = most.gui.nonBlockingDialog(str1,str2,{{'Yes' @(varargin)forceReplaceAuxTrigger3()} {'No' @(varargin)leaveTerminalUnpaired()}},...
                                most.constants.Colors.lightBlue,...
                                'Position',[0 0 900 420],...
                                'Name','Photostim Trigger Pairing');
                            uiwait(dialogWindow.hFig)
                        end
                    else
                        stimTriggerPaired = isequal(obj.stimActiveOutputChannel,rggScan.auxTrigger3In);
                    end
                    
                    if most.idioms.isValidObj(obj.beamActiveOutputChannel) && obj.pairBeamActiveOutputChannel && ~beamTriggerAlreadyPaired
                        if ~most.idioms.isValidObj(rggScan.auxTrigger4In)
                            auxTriggerIn = 'auxTrigger4In';
                            most.idioms.safeSetProp(rggScan,auxTriggerIn,obj.beamActiveOutputChannel.name);
                            beamTriggerPaired = beamTriggerPaired && isequal(obj.beamActiveOutputChannel,rggScan.auxTrigger4In);
                        else
                            str1 = sprintf('Share stim active output trigger terminal with imaging scanners'' auxiliary trigger 4??');
                            str2 = sprintf(['It is recommended for dual imaging and photostimulating microscopes with vDAQ' ...
                                ' to share photostim output triggers with all imaging systems'' auxiliary inputs (3 and 4).' ...
                                ' The benefit is that these output triggers will be timestamped to concurrently recorded ScanImage TIFFs.\n\n' ...
                                'Auxiliary trigger 4 of scanner %s was already configured with another port, %s. This could be for good reason. Verify cabling.\n\n' ...
                                'Should this port be replaced with Photostim''s currently configured stim trigger port, %s?'], rggScan.name,rggScan.auxTrigger4In.name, obj.beamActiveOutputChannel.name);
                            
                            dialogWindow = most.gui.nonBlockingDialog(str1,str2,{{'Yes' @(varargin)forceReplaceAuxTrigger4()} {'No' @(varargin)leaveTerminalUnpaired()}},...
                                most.constants.Colors.lightBlue,...
                                'Position',[0 0 900 420],...
                                'Name','Photostim Trigger Pairing');
                            uiwait(dialogWindow.hFig)
                        ...
                        beamTriggerPaired = isequal(obj.beamActiveOutputChannel,rggScan.auxTrigger4In);
                    end
                    
                    rggScan.saveMdf();
                    rggScan.validateConfiguration();
                else
                    stimTriggerPaired = stimTriggerPaired && stimTriggerAlreadyPaired;
                    beamTriggerPaired = beamTriggerPaired && beamTriggerAlreadyPaired;
                end
            end
            
            %%% Nested Functions
            function forceReplaceAuxTrigger3()
                auxTriggerIn = 'auxTrigger3In';
                most.idioms.safeSetProp(rggScan,auxTriggerIn,obj.stimActiveOutputChannel.name);
                stimTriggerPaired = stimTriggerPaired && true;
            end
            
            function forceReplaceAuxTrigger4()
                auxTriggerIn = 'auxTrigger4In';
                most.idioms.safeSetProp(rggScan,auxTriggerIn,obj.beamActiveOutputChannel.name);
                beamTriggerPaired = beamTriggerPaired && isequal(obj.beamActiveOutputChannel,rggScan.auxTrigger4In);
            end
            
            function leaveTerminalUnpaired()
                stimTriggerPaired = false;
            end
        end
        ...
        function reinit(obj)
            ...
            try
                ...
                obj.hListeners = [obj.hListeners lh];
                lh = most.ErrorHandler.addCatchingListener(obj.hSI,'imagingSystem','PostSet',@(src,evt)imagingSystemChangedCallback());
                obj.hListeners = [obj.hListeners lh];
            catch ME
                obj.numInstances = 0;
            ...
            function imagingSystemChangedCallback()
                newFrameClockTermString = obj.hSI.hScan2D.trigFrameClkOutInternalTerm;
                if ~obj.isVdaq() && obj.syncTrigIsFrame
                    if obj.active && obj.parallelSupport && ~strcmpi(obj.syncTermString,newFrameClockTermString)
                        obj.abort();
                        most.idioms.warn('Aborting photostimulation to reroute frame clock for sync.');
                    end
                    obj.syncTriggerTerm = 'frame';
                end
            end
    ...
    methods
        function set.stimTriggerTerm(obj, v)
            try
                assert(~obj.active, 'Cannot change this property while photostimulation active.');
                
                if obj.isVdaq()
                    v = getVdaqPortNumber(v);
                else
                    v = getNiPortNumber(v);
                end
            ...
            function v = getNiPortNumber(v)
                ...
                if isnumeric(v)
                    assert(v >= 0 && v <= 15 && mod(v, 1)==0 ...
                        ,'Incorrect PFI port: ''%d''. Valid Ports are 0-15', v);
                end
            end
            
            function v = getVdaqPortNumber(v)
                if isempty(v) || isempty(obj.hScan)
                    v = '';
                else
                    hDaq = obj.hScan.hDAQ;
                    DIs = unique([{hDaq.hDIOs.channelName},{hDaq.hDIs.channelName}]);
                    allowedValues = [{'frame'}, DIs];
                    isAllowed = strcmpi(v, allowedValues);
                    assert(any(isAllowed) ...
                        ,'Invalid trigger value: ''%s''. Allowed values are: %s' ...
                        ,v,strjoin(allowedValues,', '));
                    v = allowedValues{isAllowed};
                end
            end
        end
        ...
        function set.syncTriggerTerm(obj, v)
            try
                ...
                if obj.isVdaq()
                    obj.syncTriggerTerm = getVdaqPortNumber(v);
                else
                    obj.syncTriggerTerm = getNiPortNumber(v);
                    if isempty(obj.syncTriggerTerm)
                        obj.syncTermString = v;
                        return;
                    end
                    
                    if 0 == obj.numInstances
                        return;
                    end
                    ...
                    if obj.syncTrigIsFrame
                        ...
                        obj.syncTermString = sprintf('/%s/PFI%d' ...
                            , obj.hTaskMain.deviceName, obj.syncTriggerTerm);
                    end
                end
            catch ME
                most.ErrorHandler.logAndReportError(ME);
            ...
            function v = getNiPortNumber(v)
                ...
                if ischar(v)
                    Match = regexp(v, 'PFI(?<port>\d+)', 'names', 'once');
                    assert(~isempty(Match), 'NI Sync Trigger Must be a valid PFI port');
                    v = str2double(Match.port);
                end
                assert(ismember(v, 0:15) ...
                    , 'Incorrect PFI port: ''%d''. Valid Ports are 0-15', v);
            end
            ...
            function v = getVdaqPortNumber(v)
                ...
                assert(any(isAllowed) ...
                    ,'Invalid trigger value: ''%s''. Allowed values are: %s' ...
                    , v, strjoin(allowedValues,', '));
                v = allowedValues{find(isAllowed, 1)};
            end
        end
            ...
            function [aiDaqs,aoCtrDaqs,beamDaqs] = getDAQs(hScan,includeGalvoFeedbackDAQ)
                ...
                switch class(hScan)
                    ...
                    case 'scanimage.components.scan2d.LinScan'
                        ...
                        if isa(hScan.hDAQAcq,'dabs.resources.daqs.NIRIO')
                            aiDaqs = {};
                        else
    ...
    methods
        function start(obj)
            ...
            try
                ...
                if ~obj.hScan.simulated
                    ...
                    [stimTriggerPaired, beamTriggerPaired] = obj.checkOrSetRggScansAuxilliaryTriggerPairing(false);
                    stimTriggerPairFailed = obj.pairStimActiveOutputChannel && ~stimTriggerPaired;
                    beamTriggerPairFailed = obj.pairBeamActiveOutputChannel && ~beamTriggerPaired;
                    if obj.isVdaq() && obj.parallelSupport && (stimTriggerPairFailed || beamTriggerPairFailed)
                        str = sprintf(['Pairing output triggers to auxiliary inputs failed.\n\n'...
                            'Go to advanced tab of Photostim page and configure stim and beam output channels or disable pairing.\n'...
                            'When pairing is enabled, this allows TIFFs acquired in parallel to record timestamps when stimulations occur.\n']);
                        error(str);
                    end
                    ...
                    % prepare z task
                    if obj.hasZ && obj.separateZDAQ && obj.zMode3D
                ...
                obj.abort();
                most.ErrorHandler.logAndReportError(ME);
                most.gui.blockingMsgbox(ME.message,'Photostim Start','error');
            end
        end
        ...
        function triggerStim(obj)
            ...
            try
                ...
                if ~obj.hScan.simulated
                    if obj.isVdaq()
                        % we rely on the .status property for real statuses
                        % which is only updated asynchronously. We give it some
                        % time here to properly update in case the main task
                        % is run immediately.
                        drawnow('nocallbacks');
                    ...
                    obj.status = sprintf('Outputting stimulus');
                end
            catch ME
                most.ErrorHandler.logAndReportError(ME);
            end
        ...
        function onDemandStimNow(obj, stimGroupIdx, verbose)
            ...
            if ~strcmp(obj.status, 'Ready')
                ...
                if ~obj.isVdaq() && obj.hasZ && obj.separateZDAQ
                    obj.hTaskZ.abort();
                end
            end
            
            stimGroupNames = {obj.stimRoiGroups.name};
            obj.status = sprintf('Preparing stimulus %s for output...',stimGroupNames{stimGroupIdx});
            drawnow('nocallbacks');
            ...
            if ~obj.isVdaq() && obj.hasZ && obj.separateZDAQ && obj.zMode3D
                obj.hTaskZ.start();
            end
            if obj.hasGalvos
                obj.hTaskGalvo.start();
            ...
            if obj.stimImmediately
                obj.status = sprintf('Outputting stimulus group %g: %s...',stimGroupIdx, stimGroupNames{stimGroupIdx});
                if verbose
                    fprintf('It took %.4f seconds from the time you commanded to when the trigger was sent.\n',toc(t));
                end
                obj.triggerStim();
            else
                obj.status = sprintf('Stimulus group %g: %s waiting for trigger...',stimGroupIdx, stimGroupNames{stimGroupIdx});
                if verbose
        ...
        function abort(obj)
            ...
            if any(obj.lastMotion)
                ...
                elseif isa(obj.hScan,'scanimage.components.scan2d.RggScan')
                    if most.idioms.isValidObj(obj.hScan.xGalvo)
                        offsetVolts = obj.hScan.xGalvo.position2Volts(0);
                        obj.hTaskGalvo.hChanIp(1).updateOffset(offsetVolts);
                    end
                    
                    if most.idioms.isValidObj(obj.hScan.yGalvo)
                        offsetVolts = obj.hScan.yGalvo.position2Volts(0);
                        obj.hTaskGalvo.hChanIp(2).updateOffset(offsetVolts);
                    end
                end
    ...
    methods (Hidden)
        ...
        function tf = isVdaq(obj)
            tf = ~isempty(obj.hScan) ...
                && ( ...
                isa(obj.hScan, 'scanimage.components.scan2d.RggScan') ...
                || (isa(obj.hScan, 'scanimage.components.scan2d.SlmScan') && obj.hScan.hAcq.isVdaq) ...
                );
        end
        
        function compensateMotion(obj)
            if obj.active && obj.compensateMotionEnabled && ~isempty(obj.hSI.hMotionManager.motionHistory)
                ...
                if ~any(isnan(motion)) && ~isequal(obj.lastMotion,motion)
                    ...
                    elseif isa(obj.hScan, 'scanimage.components.scan2d.RggScan')
                        if most.idioms.isValidObj(obj.hScan.xGalvo)
                            offsetVolts = obj.hScan.xGalvo.position2Volts(offsetAngleXY(1),false);
                            obj.hTaskGalvo.hChanIp(1).updateOffset(offsetVolts);
                        end
                        
                        if most.idioms.isValidObj(obj.hScan.yGalvo)
                            offsetVolts = obj.hScan.yGalvo.position2Volts(offsetAngleXY(2),false);
                            obj.hTaskGalvo.hChanIp(2).updateOffset(offsetVolts);
                        end
                    end
    ...
    methods (Hidden)
        ...
        function nSampleCallback(obj,~,~)
            if strcmp(obj.stimulusMode, 'sequence')
                ...
                obj.status = sprintf('Sequence #%d, position %d. Next stimulus: %d: %s', obj.completedSequences + 1, obj.sequencePosition, obj.nextStimulus, stimGroupNames{obj.nextStimulus});
                if obj.hasSlm
                    if ~obj.slmQueueActive
            ...
            elseif obj.allowMultipleOutputs
                ...
                stimRoiGroupNames = {obj.stimRoiGroups.name};
                obj.status = sprintf('Stimulus group %d: %s output %s. Waiting for next trigger...', obj.primedStimulus, stimRoiGroupNames{obj.primedStimulus}, s);
            else
                obj.clearOnDemandStatus();
...
function s = defaultMdfSection()
    s = [...
        ...
        most.HasMachineDataFile.makeEntry('stimTriggerTerm',1,'Specifies the channel that should be used to trigger a stimulation. This a triggering port name such as D2.1 for vDAQ or PFI1 for the auxiliary IO board of an NI LinScan system.')...
        ...
        most.HasMachineDataFile.makeEntry()... % blank line
        most.HasMachineDataFile.makeEntry('stimActiveOutputChannel','','Digital terminal on stim board to output stim active signal. (e.g. on vDAQ: ''D1.6'' on NI-DAQ hardware: ''/port0/line0''')...
        most.HasMachineDataFile.makeEntry('beamActiveOutputChannel','1.6','Digital terminal on stim board to output beam active signal. (e.g. on vDAQ: ''D1.7'' on NI-DAQ hardware: ''/port0/line1''')...
        most.HasMachineDataFile.makeEntry('slmTriggerOutputChannel','1.7','Digital terminal on stim board to trigger SLM frame flip. (e.g. on vDAQ: ''D1.5'' on NI-DAQ hardware: ''/port0/line2''')...
        most.HasMachineDataFile.makeEntry('pairStimActiveOutputChannel',false,'Whether stimActiveOutputChannel should share its terminal with all the RggScan''s auxiliary trigger 3 input')...
        most.HasMachineDataFile.makeEntry('pairBeamActiveOutputChannel',true,'Whether beamActiveOutputChannel should share its terminal with all the RggScan''s auxiliary trigger 4 input')...
        ];
end
