classdef Beams < scanimage.interfaces.Component
    ...
    properties (SetObservable)
        ...
        flybackBlanking = true;     % [Logical] Indicates whether to blank beam during Y-galvo flyback
        turnAroundBlanking = true;  % [Logical] Indicates whether to blank beam outside of the fill fraction
    ...
    properties (Transient, SetAccess = private, SetObservable)
        ...
        hMotionListener = event.listener.empty();
    end
    ...
    properties (Constant, Hidden)
        ...
        PROP_TRUE_LIVE_UPDATE = ...                     %Cell array of strings specifying properties that can be set while the component is active
            {'powers' 'powerFractions' 'flybackBlanking' 'turnAroundBlanking' 'interlaceDecimation' 'interlaceOffset' 'enablePowerBox' 'powerBoxes' 'powerBoxStartFrame' 'powerBoxEndFrame'};
    ...
    methods (Hidden)        
        function deinit(obj)
            ...
            most.idioms.safeDeleteObj(obj.hMotionListener);
            most.idioms.safeDeleteObj(obj.hTask)
        end
        
        function reinit(obj)
            obj.hResourceStoreListener = most.ErrorHandler.addCatchingListener(obj.hResourceStore,'hResources','PostSet',@(varargin)obj.searchForResources);
            obj.searchForResources();
            obj.deactivateAllBeams();
        end
        
        function searchForResources(obj)
            ...
            hMotionManager = obj.hResourceStore.filterByName('SI MotionManager');
            
            most.idioms.safeDeleteObj(obj.hMotionListener);
            obj.hMotionListener = most.ErrorHandler.addCatchingListener(hMotionManager,'newMotionEstimateAvailable',@(varargin)obj.calculateSampleAbsolutePowerBoxRect);
    ...
    methods
        ...
        function v = get.pzAdjust(obj)
            v = obj.expandVal(obj.pzAdjust,scanimage.types.BeamAdjustTypes.Exponential);
        end
        ...
        function [fastBeams, slowBeams, allBeams] = wrapBeams(obj,hBeams)
            ...
            for idx = 1:numel(hBeams)
                ...
                if isa(hBeam, 'dabs.resources.devices.BeamModulatorFast')
                    ...
                    scanner.powerBoxes          = getPowerBoxes(powerBoxes_,siBeamIdx);
                    scanner.sampleRateHz        = NaN; % filled out by Scan2D
                    scanner.turnAroundBlanking  = obj.turnAroundBlanking;
            ...
            function pbxs = getPowerBoxes(powerBoxes,siBeamIdx)
                if isempty(powerBoxes)
                    pbxs = [];
                    return;
                end
                
                if obj.enablePowerBox
                    numPbxs = length(powerBoxes);
                    for pbxIdx = 1:numPbxs
                        if isstruct(powerBoxes(pbxIdx))
                            pbxs(pbxIdx) = powerBoxes(pbxIdx);
                        else
                            pbxs(pbxIdx) = powerBoxes(pbxIdx).struct();
                        end
                        pbxs(pbxIdx).powers = powerBoxes(pbxIdx).powers(siBeamIdx);
                    end
                else
                    pbxs = [];
                end
        ...
        function set.powerBoxStartFrame(obj,val)
            ...
            if obj.componentUpdateProperty('powerBoxStartFrame',val)
                ...
                if obj.hSI.active
                    if obj.enablePowerBox && ~obj.streamingBuffer && ((obj.powerBoxStartFrame > 1) || (~isinf(obj.powerBoxEndFrame)))
                        most.idioms.warn('Cannot make changes to a time varying power box mid-acquisition.');
                    end
                    obj.updateBeamBufferAsync(true);
                end
            end
    ...
    methods (Hidden)        
        ...
        function updateBeamBufferAsync(obj, reGenerateAO, timeout)
            ...
            if obj.sharingScannerDaq || isa(obj.hSI.hScan2D,'scanimage.components.scan2d.RggScan')
        ...
        function val = zprpBeamScalarExpandPropValue(obj,val,propName)
         ...
         function calculateSampleAbsolutePowerBoxRect(obj, force)
             persistent t
             if isempty(t)
                 t = tic();
                 pause(0.5);
             ...
             sampleAbsolutePowerBoxesMask = arrayfun(@(pb)pb.useSampleAbsolute,obj.powerBoxes);
             sampleAbsolutePowerBoxes = obj.powerBoxes(sampleAbsolutePowerBoxesMask);

             if ~obj.hSI.mdlInitialized || isempty(sampleAbsolutePowerBoxes) || (~force && toc(t) < 0.4)
                 return;
             end

             widthFOV_um = max(diff(obj.hSI.hScan2D.nominalFovCornerPoints(:,1))) * obj.hSI.objectiveResolution;
             heightFOV_um = max(diff(obj.hSI.hScan2D.nominalFovCornerPoints(:,2))) * obj.hSI.objectiveResolution;

             hMotors = obj.hSI.hMotors;
             hPtAbs = hMotors.getPosition(hMotors.hCSSampleAbsolute);
             currentPosition = hPtAbs.points;
             
             motionHistory = obj.hSI.hMotionManager.motionHistory;
             if isempty(motionHistory)
                 motionCorrection = [0 0 0];
             else
                 mostRecentMotionResult = motionHistory(end);
                 drRef = mostRecentMotionResult.drRef;
                 if ~isempty(mostRecentMotionResult.drStage)
                     drStage = mostRecentMotionResult.drStage;
                 else
                     drStage = [0 0 0];
                 end

                 %drRef is the total detected motion from the motion
                 %estimator in optical degrees XY, microns Z including stage movements
                 %and sample movements relative to the stage.
                 %drStage is the motion made with the stage since the
                 %beginning of the acquisition.
                 %Z component is not used.
                 motionCorrection = obj.hSI.objectiveResolution * drRef(1:2) - drStage(1:2);
             end
             
             for pb = sampleAbsolutePowerBoxes
                 diffXFromPBLocation_um = pb.sampleAbsoluteLocation(1) - currentPosition(1) + motionCorrection(1);
                 diffYFromPBLocation_um = pb.sampleAbsoluteLocation(2) - currentPosition(2) + motionCorrection(2);

                 diffXFromPBLocation_norm = diffXFromPBLocation_um / widthFOV_um + (1 - pb.rect(3))/2;
                 diffYFromPBLocation_norm = diffYFromPBLocation_um / heightFOV_um + (1-pb.rect(4))/2;

                 pb.rect(1) = diffXFromPBLocation_norm;
                 pb.rect(2) = diffYFromPBLocation_norm;
             end
