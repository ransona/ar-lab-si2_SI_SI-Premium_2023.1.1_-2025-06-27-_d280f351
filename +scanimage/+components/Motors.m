classdef Motors < scanimage.interfaces.Component & most.HasMachineDataFile & dabs.resources.configuration.HasConfigPage
    ...
    properties (SetObservable,Transient,Hidden)
        ...
        oldCenterPt; %For updating context image Zs
    end
    ...
    methods
        ...
        function setRelativeZero(obj,newCenterPt)            
            ...
            obj.oldCenterPt = obj.hSI.hCoordinateSystems.focalPoint.transform(obj.hSI.hCoordinateSystems.hCSSampleRelative).points;
        ...
        function clearRelativeZero(obj)
            obj.oldCenterPt = -obj.hSI.hCoordinateSystems.focalPoint.transform(obj.hSI.hCoordinateSystems.hCSSampleAbsolute).points;
    ...
    methods (Hidden)        
        ...
        function motorPositionChanged(obj,src,evt)
            ...
            obj.hSI.hBeams.calculateSampleAbsolutePowerBoxRect()
        end
        
        function motorIsMovingChanged(obj,src,evt)
            obj.moveInProgress = NaN; % trigger UI update
            
            if ~evt.AffectedObject.isMoving
                
                obj.queryPosition(); %ensure last known position is accurate.
                force = true;
                obj.hSI.hBeams.calculateSampleAbsolutePowerBoxRect(force)
            end
        end
    ...
    methods
        ...
        function move(obj,hPt,async,axes)
            ...
            if obj.hSI.hBeams.enablePowerBox && any(arrayfun(@(pb)pb.useSampleAbsolute,obj.hSI.hBeams.powerBoxes))% && 0.01 < ((val([1 2]) - obj.samplePosition([1 2])) / max(diff(hSI.hRoiManager.imagingFovUm)))
                obj.hSI.hWaveformManager.blankPowerBoxBeams();
                obj.hSI.hBeams.updateBeamBufferAsync(false);
            end
    ...
    methods
        ...
        function addCalibrationPoint(obj, motorPosition, motion)
            ...
            if nargin < 2 || isempty(motorPosition)
                ...
                % TODO: possibly apply transformation to ensure in hCSAlignment (seems to be that way already)
                validateattributes(motorPosition,{'numeric'},{'vector'});
                motorPt = motorPosition;
                motorPt(end+1:3) = 0;
                motorPt(4:end) = [];
            end
        ...
        function resetCalibrationMatrix(obj)
         ...
         function correctObjectiveResolution(obj)            
            ...
            if isequal(T,eye(size(T,1)))
                str = sprintf('The Imaging System has not been aligned to the stage.\nStage scanner alignment is required to\n- Accurately display FOV size in ROI Group Editor\n- use motion correction\n- measure size of features from the display');
                most.idioms.warn(str);
                disp('<a href = "https://docs.scanimage.org/Basic%2BFeatures/Scanner%2BStage%2BAlignment.html">Alignment between stage and scanner documentation</a>')
                return;
            end
            ...
            aspectRatio = x_degPerUm/y_degPerUm;
            if aspectRatio < 0.95 || aspectRatio > 1.05
                error('The scan aspect ratio (X/Y) is %.4f\nFix the mirror settings in the machine configuration to achieve a scan with an aspect ration of 1, then rerun the Stage-Scanner calibration.',aspectRatio);
            end
            
            obj.hSI.objectiveResolution = mean([1/x_degPerUm, 1/y_degPerUm]);
            obj.hSI.saveMdf();
            
            msg = sprintf('New Objective Resolution: %.2f um/deg',obj.hSI.objectiveResolution);
            most.gui.centerOnScreen(msgbox(msg, 'Resolution update','help'));
         end
    end
