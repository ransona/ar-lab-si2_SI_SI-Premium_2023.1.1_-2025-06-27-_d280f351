...
function paths = pathEvaluation(startIdx, hTiles)    
    ...
    closestTile = getLeftestTileIdx(thisTile, unprocessedTilesArray);
...
function leftestTile = getLeftestTileIdx(thisTile, tilesArray)
    try
        ...
        tlAry = tlAry(:);
        samplePoints = [tlAry.samplePoint];
        sampleX = samplePoints(:,1);
        [~,idx] = min(sampleX);
        
        leftestTile = tilesArray(idx);
    catch ME
