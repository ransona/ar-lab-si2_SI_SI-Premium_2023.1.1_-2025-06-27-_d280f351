classdef tileExpanseTool < scanimage.components.tileTools.tileTool
    ...
    methods
        function drawTileArea(obj, stop, varargin)
            ...
            elseif stop
                ...
                numTilesX_ = 1;
                if sz(1) > maxSz(1)
                    numTilesX_ = ceil(sz(1)/maxSz(1));
                    sz(1) = sz(1)/numTilesX_;
                end

                numTilesY_ = 1;
                if sz(2) > maxSz(2)
                    numTilesY_ = ceil(sz(2)/maxSz(2));
                    sz(2) = sz(2)/numTilesY_;
                end
                ...
                if sz(1) > sz(2)
                    tileResX_ = obj.tileResY * sz(1)/sz(2);
                    obj.tileResX = round(tileResX_/2)*2; % ensure pixelsPerLine is even
                else
                    tileResY_ = obj.tileResX * sz(2)/sz(1);
                    obj.tileResY = round(tileResY_/2)*2; % ensure linesPerFrame is even
                end
                
                obj.numTilesX = numTilesX_;
                obj.numTilesY = numTilesY_;
    ...
    methods
        ...
        function AbsMaxSize = getAbsMaxScanTileSize(obj)
            % Max Tile Size
            cp = obj.hSI.hScan2D.fovCornerPoints;
            scanners = obj.hSI.hScan2D.scannerset.scanners;
            if isa(scanners{1},'scanimage.mroi.scanners.Resonant')
                cp(:,1) = (scanners{1}.fullAngleDegrees/2) .* [-1 1 1 -1];
            end
            cp(:,3) = 0;
            cp = scanimage.mroi.coordinates.Points(obj.hSI.hCoordinateSystems.hCSReference,cp);
            cpUm = cp.transform(obj.hSI.hMotors.hCSMicron);
            cpUm = cpUm.points;
            cpUm(:,3) = [];
