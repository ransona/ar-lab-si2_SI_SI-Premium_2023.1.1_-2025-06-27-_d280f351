classdef Scan2D < scanimage.interfaces.Component
    ...
    properties (Abstract, SetAccess = protected) 
        ...
        hasXGalvo;                          % logical, indicates if scanner has an XGalvo
        hasResonantMirror;                  % logical, indicates if scanner has a resonant mirror
    end
    
    properties (Abstract, SetAccess = {?scanimage.SI})
        isPolygonalScanning;                 % logical, indicates if an imaging system should have acquisition configured for polygonal scanning.
    end
    ...
    methods            
        ...
        function set.channelsInputRanges(obj,val)
            if ~isa(val,'cell')
                ...
                assert(all(size(val) == [obj.hAcq.hFpga.hAfe.physicalChannelCount 2]),'Value supplied for channelsInputRanges is of the wrong size.')
                val = num2cell(val,2);
            end
...
%             end
    ...
    methods
        ...
        function openExcitationShutters(obj)
            excitationMask = cellfun(@(hS)ismember(dabs.resources.devices.shutter.ShutterTarget.Excitation,hS.shutterTarget),obj.hShutters);
            hExcitationShutters = obj.hShutters(excitationMask);
            obj.transitionShutters(true,hExcitationShutters);
        end
        ...
        function transitionShutters(obj,tf,hShutters)
            if nargin<3
                hShutters = obj.hShutters;
            end

            obj.startShuttersTransition(tf,hShutters);
            obj.waitShuttersTransitionComplete(hShutters);
        end
        
        function startShuttersTransition(obj,tf,hShutters)
            if nargin<3
                hShutters = obj.hShutters;
            end
            ...
            for idx = 1:numel(hShutters)
                try
                    hShutters{idx}.startTransition(tf);
                catch ME
        ...
        function waitShuttersTransitionComplete(obj,hShutters)
            if nargin<2
                hShutters = obj.hShutters;
            end
            ...
            for idx = 1:numel(hShutters)
                try
                    hShutters{idx}.waitTransitionComplete();
                catch ME
                    most.ErrorHandler.logAndReportError(ME);
                end
            end
