classdef StackManager < scanimage.interfaces.Component
    ...
    methods (Hidden)
        ...
        function samplePosChanged(obj)
            ...
            obj.hSI.focalPoint = obj.hSI.hCoordinateSystems.focalPoint.transform(obj.hSI.hCoordinateSystems.hCSSampleRelative);
        end
        ...
        function updateZSeries(obj)
            ...
            function updateZSeriesArbitrary()
                ...
                for idx = 1:numActuators
                    ...
                    if obj.arbitraryZsAreSampleRelative
                        hZs_(idx) = scanimage.mroi.coordinates.Points(obj.hSI.hCoordinateSystems.hCSSampleRelative,points);
                    else
                        hZs_(idx) = scanimage.mroi.coordinates.Points(obj.hSI.hCoordinateSystems.hCSReference,points);
                    end
                end
            ...
            switch actuator
                case scanimage.types.StackActuator.motor
                    ...
                    if isa(hScan2D,'scanimage.components.scan2d.RggScan') && hScan2D.isStreamScanning
                        ...
                        obj.hSI.hScan2D.hCtl.hStream.abort()
                        obj.hSI.hMotors.hMotorXYZ{2}.stop();
                    end
                    ...
                    if isa(hScan2D,'scanimage.components.scan2d.RggScan') && hScan2D.isStreamScanning && obj.slicesDone
                        ...
                        % restarted after each slice.
                        % Only do this for slices after the first slice. As the stream must first be
                        % configured by Scan2D.hCtl
                        
                        obj.hSI.hScan2D.hCtl.hStream.start();
                    end
        ...
        function componentAbort(obj)
            try
                if obj.enable && obj.stackReturnHome
                    obj.hSI.hMotors.hMotorXYZ{3}.moveWaitForFinish();
                    obj.goHome();
                    obj.resetHome();
                end
            catch ME
    ...
    methods
        function set.enable(obj,val)
            ...
            if obj.componentUpdateProperty('enable',val)
                ...
                if val
                    obj.hSI.stackMode = obj.stackMode.char;
                else
                    obj.hSI.stackMode = 'plane';
                end
        ...
        function val = get.zPowerReference(obj)
            import scanimage.types.StackDefinition;
            ...
            switch obj.stackDefinition
                case StackDefinition.uniform
                    % stack positions follow current motor position, so
                    % power reference starts with transformed position.
                    hZPoints = obj.hZPowerReference.transform(obj.hCSDisplay);
                    val = hZPoints.points(:,3);
                case {StackDefinition.bounded, StackDefinition.arbitrary}
                    val = obj.zs(1);
                otherwise
                    error('unknown stack type %s', obj.stackDefinition);
            end
        end
