classdef ScanfieldDisplayControls < most.Gui
    ...
    properties (Hidden, SetObservable)
        ...
        hDisplayAxes;
        hDisplayMenu;
        hSurfs = matlab.graphics.primitive.Surface.empty(0,1);
        hTexts = matlab.ui.control.UIControl.empty(0,1);
    ...
    methods
        function obj = ScanfieldDisplayControls(hModel, hController)
            ...
            obj.addUiControl(...
                ...
                'Units','characters',...
                'String','Auto Fill',...
                'Position',[20 0.4 16 1.69230769230769],...
                'Callback',@obj.mnuAutoFill,...
                'Tag','pbAutoFill');
        ...
        function resetDisplay(obj)
            ...
            if isempty(sgl)
                try
                    ...
                    fc = .05*ones(3,1);
                    if ~most.idioms.isValidObj(obj.hDisplayFig)
                        obj.hDisplayFig = most.idioms.figure( ...
                            'Name','Scanfield Display' ...
                            ,'Visible','off' ...
                            ,'SizeChangedFcn',@(varargin)obj.resetDisplay()...
                            ,'Tag','sfDisplay' ...
                            ,'CloseRequestFcn',@obj.figCloseEventHandler ...
...
%                             cols = ceil(N/rows);
        ...
        function mnuAutoFill(obj,~,~)
            obj.mnuRemoveAll();

            hSI = obj.hModel;
            hRois = hSI.hRoiManager.currentRoiGroup.activeRois;
            zs = hSI.hStackManager.zs;

            channel = hSI.hChannels.channelDisplay(1);

            for roiIdx = 1:numel(hRois)
                hRoi = hRois(roiIdx);
                if ~hRoi.display
                   continue; 
                end
                ...
                for z = zs
                    if hRoi.hit(z)
                        name = sprintf('%s',hRoi.name);
                        obj.addDisp(name, channel, roiIdx, z);
                    end
                end
            end
        end
