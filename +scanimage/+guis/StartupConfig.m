classdef StartupConfig < most.HasClassDataFile
    ...
    methods (Static)        
        function [newFileName, hWb] = newMdf(mdfpath)
            ...
            % ensure this file is not a ScanImage code file
            fullpath = fullfile(mdfpath,mdffile);
            if scanimage.SI.isScanImageSourceCodeFile(fullpath)
                errorText = sprintf('The file chosen "%s" is a ScanImage%s source-code file.\nPlease try a different filename.',mdffile,most.constants.Unicode.registered_sign);
                most.ErrorHandler.logAndReportError(errorText);
                errordlg(errorText,'Invalid MDF File Path');
                return
            end
