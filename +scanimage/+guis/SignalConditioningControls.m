classdef SignalConditioningControls < most.Gui
    ...
    properties (Hidden)
        ...
        autoCorrectPhaseOffset = false;

        laserGatingAvailable;
    end
    ...
    properties (Constant)
        PSEUDO_CLOCK_NAME = 'Up-multiplied';
        SAMPLE_PHASE_SHIFT_CORRECTION = 31;

        AUTO_CORRECT_MAX_ERROR = 32;                % maximum error for digital sampling for LRR sync support
        AUTO_CORRECT_BUTTON_CUTOFF_THRESHOLD = 0.1; % 10% threshold
        AUTO_CORRECT_BUTTON_CUTOFF_TICKS = ceil(...
            scanimage.guis.SignalConditioningControls.AUTO_CORRECT_MAX_ERROR / ...
            scanimage.guis.SignalConditioningControls.AUTO_CORRECT_BUTTON_CUTOFF_THRESHOLD);
    end
    ...
    methods
        ...
        function applyClockSettings(obj,varargin)
            ...
            prevClock = obj.hScan2D.externalSampleClock;
            prevRate = obj.hScan2D.externalSampleClockRate;
            prevMultiplier = obj.hScan2D.externalSampleClockMultiplier;
            ...
            try
                ...
                if obj.hScan2D.externalSampleClock && obj.isH
                    if obj.clockMult > 32 && ~obj.hScan2D.hAcq.hAcqEngine.HSADC_LRR_SUPPORT
                        errorText = ['To enable clock multipliers higher than 32 on the HS vDAQ, please change to the low repetition rate bitfile.' newline ...
                            '(see Acquisition Gating for Low Rep Rate Lasers in the scanimage docs).'];
                        error(errorText);
                    elseif obj.clockMult < 32
                        errorText = ['For the high-speed vDAQ, the clock multiplier must be ' char(8805) '32.' newline ...
                            'A special configuration can be installed to support certain higher repetition rate lasers, contact support if this is necessary.'];
                        error(errorText);
                    end
                end
                ...
                obj.hScan2D.externalSampleClock = prevClock;
                obj.hScan2D.externalSampleClockRate = prevRate;
                obj.hScan2D.externalSampleClockMultiplier = prevMultiplier;

                pbApplyClockSettings_startBlinkRed();
                warndlg(ME.message,'Sample Clock Error');
            end

            % apply clock changes to plot
            obj.plotSizeChanged();
        ...
        function showDiscSettings(obj,varargin)
            ...
            function makeFig()
                obj.hPDFig = most.idioms.figure( ...
                    'Name', 'Photon Discriminator Settings' ...
                    , 'CloseRequestFcn', @hideFig ...
                    , 'Position', most.gui.centeredScreenPos([255 132])...
                    , 'Resize', 'off');
    ...
    methods (Hidden)
        ...
        function readSamples(obj,~,data)
            ...
            if obj.isH
                ...
                if ~isempty(obj.laserSel) && strcmp(obj.laserSel,obj.PSEUDO_CLOCK_NAME) && isfield(data.triggers,'LaserTriggerSync')
                    synclaser = logical(data.triggers.LaserTriggerSync);
                else
                    synclaser = [];
                end
            else
            ...
            % find sync rising edges
            if isempty(synclaser)
                syncRisingEdgeIndices = [];
            else
                syncRisingEdgeIndices = find(synclaser(2:end) .* ~synclaser(1:end-1)) + 1;
            end

            % clock setup
            externalClock = obj.hScan2D.hAcq.externalSampleClock;
            pllLocked = obj.hScan2D.hAcq.hFpga.hClockCfg.tfLocked;
            
            % TEMPORARY fix for 80KHz support until the max datascope fifo depth is increased (only get 2 rising edges in 80KHz mode)
            is80kHzLrrMode = obj.hScan2D.hAcq.hAcqEngine.HSADC_LRR_SUPPORT && obj.hFpga.SAMPLE_PHASE_BITS == 15 && obj.hScan2D.useCustomFilterClock && obj.hScan2D.customFilterClockPeriod > 16000;
            numSupportedRisingEdges = most.idioms.ifthenelse(is80kHzLrrMode,2,4);
            if ~obj.laserGatingAvailable || numel(reInds) < numSupportedRisingEdges
                if ~externalClock
                    obj.triggerRate = sprintf('%.3f',obj.hFpga.internalClockSourceRate*1e-6);
                elseif externalClock && pllLocked
                    obj.triggerRate = sprintf('%.3f',obj.hDataScope.digitizerActualSampleRate*1e-6/obj.hScan2D.customFilterClockPeriod);
                else
                ...
                if ~obj.hScan2D.useCustomFilterClock
                    obj.triggerNominalPeriodTicksStr = num2str(obj.hScan2D.externalSampleClockMultiplier);
                end
                ...
                if obj.autoCorrectPhaseOffset && obj.hScan2D.useCustomFilterClock && ~isempty(syncRisingEdgeIndices)
                    filteredlaser_phase = reInds(1);
                    synclaser_phase = syncRisingEdgeIndices(1);
                    phaseoffset = mod(filteredlaser_phase - synclaser_phase, obj.hScan2D.customFilterClockPeriod);
                    obj.laserTriggerPhase = phaseoffset;

                    % only correct once, since the shift will cause the measured phase to be different
                    obj.autoCorrectPhaseOffset = false;
                    obj.tbAutoCorrect.Value = false;
                    most.util.resetButtonFocus(obj.tbAutoCorrect);
                end
                
                N = numel(reInds)-(numSupportedRisingEdges-1);
                ...
                for ind = 1:N
                    reind = reInds(ind + ~is80kHzLrrMode);
                    st = reind+xl(1)+1;
                    nd = reind+xl(2)+1;
        ...
        function wndHit(obj,src,evt)
            ...
            elseif strcmp(evt.EventName, 'WindowMouseMotion')
                ...
                lastSampleAllowed = obj.hScan2D.customFilterClockPeriod-1;
                if numel(inds) == 2
                    mouseMoveResult = min(mouseMoveResult,[lastSampleAllowed-original_size lastSampleAllowed]);
                else
                    mouseMoveResult = min(mouseMoveResult,lastSampleAllowed);
                end
        ...
        function advancedClockSettings(obj,varargin)
            ...
            function makeFig()
                obj.hAdvClkFig = most.idioms.figure( ...
                    'Name', 'Advanced Clock Settings' ...
                    , 'CloseRequestFcn', @hideFig ...
                    , 'Position', most.gui.centeredScreenPos([295 86])...
                    , 'Resize', 'off');
    ...
    methods (Access = protected)
        function initGui(obj)
            ...
            rf = most.gui.uiflowcontainer('Parent',ifl,'FlowDirection','LeftToRight','HeightLimits',24);
            columnWidth = 82;
            tt = 'Clock Input Terminal';
            most.gui.uicontrol('style','text','parent',rf,'HorizontalAlignment','right','string','Clock Input:','WidthLimits',columnWidth,'ToolTip',tt);
            obj.lsrCh = most.gui.uicontrol('parent',rf,'style','popupmenu','Bindings',{{obj 'laserChoices' 'choices'} {obj 'laserSel' 'choice'}},'ToolTip',tt);
            obj.trigFiltRow = most.gui.uiflowcontainer('Parent',ifl,'FlowDirection','LeftToRight','HeightLimits',22);
            tt = 'Laser Clock Filter (in Sample Ticks)';
            most.gui.uicontrol('style','text','parent',obj.trigFiltRow,'HorizontalAlignment','right','string','Filter Ticks:','WidthLimits',columnWidth,'ToolTip',tt);
            most.gui.uicontrol('parent',obj.trigFiltRow,'style','edit','Bindings',{obj 'laserTriggerFilter' 'value'},'KeyPressFcn',@obj.keyFunc,'ToolTip',tt);
            obj.trigPhaseRow = most.gui.uiflowcontainer('Parent',ifl,'FlowDirection','LeftToRight','HeightLimits',22);
            tt = 'Laser Clock Phase Adjust (in Sample Ticks)';
            most.gui.uicontrol('style','text','parent',obj.trigPhaseRow,'HorizontalAlignment','right','string','Phase Adjust:','WidthLimits',columnWidth,'ToolTip',tt);
            most.gui.uicontrol('parent',obj.trigPhaseRow,'style','edit','Bindings',{obj 'laserTriggerPhase' 'value'},'KeyPressFcn',@obj.keyFunc2,'ToolTip',tt);
            tt = 'Auto adjust phase to match sync input';
            obj.tbAutoCorrect = most.gui.uicontrol('parent',obj.trigPhaseRow,'String','Auto','style','togglebutton','callback',@obj.measureLaserSyncPhaseOffset,'WidthLimits',30,'ToolTip',tt);
            rf = most.gui.uiflowcontainer('Parent',ifl,'FlowDirection','LeftToRight','HeightLimits',22);
            tt = 'Detected Laser Clock Frequency (MHz)';
            most.gui.uicontrol('style','text','parent',rf,'HorizontalAlignment','right','string','Rep Rate (MHz):','WidthLimits',columnWidth,'ToolTip',tt);
            most.gui.uicontrol('parent',rf,'style','edit','Bindings',{obj 'triggerRate' 'string'},'BackgroundColor',.95*ones(1,3),'enable','on','ToolTip',tt);
            rf = most.gui.uiflowcontainer('Parent',ifl,'FlowDirection','LeftToRight','HeightLimits',22);
            tt = 'Laser Clock Period (in Sample Ticks)';
            most.gui.uicontrol('style','text','parent',rf,'HorizontalAlignment','right','string','Period (Ticks):','WidthLimits',columnWidth,'ToolTip',tt);
            obj.etLaserPeriod = most.gui.uicontrol('parent',rf,'style','edit','Bindings',{obj 'triggerNominalPeriodTicksStr' 'string'},'BackgroundColor',.95*ones(1,3),'enable','inactive','ToolTip',tt);
    ...
    methods
        function initListeners(obj)
            ...
            obj.hListeners(end+1) = most.ErrorHandler.addCatchingListener(obj.hScan2D.hAcq.hFpga.hClockCfg,'tfLocked','PostSet',@obj.updatePBApplyColor);
        end
        ...
        function updatePBApplyColor(obj, varargin)
            if strcmpi(obj.hScan2D.hAcq.hFpga.hClockCfg.clockSource,'internal')
                obj.pbApplyClockSettings.hCtl.BackgroundColor = most.constants.Colors.lightGray;
                return;
            end
            
            if obj.hScan2D.hAcq.hFpga.hClockCfg.tfLocked
                obj.pbApplyClockSettings.hCtl.BackgroundColor = most.constants.Colors.lightGreen;
            else
                obj.pbApplyClockSettings.hCtl.BackgroundColor = most.constants.Colors.lightRed;
                most.ErrorHandler.logAndReportError(['PLL got unlocked.\nThe ''Apply Clock Settings'''...
                    ' will remain light red until locked again.\nCheck that the laser clock is consistent.']);
            end
        end

        function measureLaserSyncPhaseOffset(obj,varargin)
            persistent lastClick;

            doubleClick = ~isempty(lastClick) && toc(lastClick) < 0.25;
            if doubleClick || ~most.idioms.isValidObj(obj.hScan2D.LRRLaserSyncPort)
                obj.tbAutoCorrect.Value = true;
                obj.selectLaserSyncInputPort();
            end
            
            if ~most.idioms.isValidObj(obj.hScan2D.LRRLaserSyncPort)
                return
            end

            lastClick = tic();
            tfAutoMeasure = obj.tbAutoCorrect.Value;
            if doubleClick || tfAutoMeasure
                obj.laserTriggerPhase = 0;
                pause(0.05);
                obj.autoCorrectPhaseOffset = true;
            else
                obj.autoCorrectPhaseOffset = false;
                most.util.resetButtonFocus(obj.tbAutoCorrect);
            end
        ...
        function selectLaserSyncInputPort(obj,varargin)
            hDIs = arrayfun(@(hIO){hIO},obj.hScan2D.hDAQ.hDIOs);
            hDIs = [hDIs arrayfun(@(hIO){hIO},obj.hScan2D.hDAQ.hDIs)];

            dialogFig = figure;
            dialogFig.Units = 'normalized';
            dialogFig.Name  = '';
            dialogFig.NumberTitle = 'off';
            dialogFig.MenuBar = 'none';
            dialogFig.Resize = 'off';
            dialogFig.Position = most.gui.centeredScreenPos([0.0836 0.0444],'normalized');
            
            most.gui.uicontrol('parent',dialogFig,'style','text','string','Select laser sync input:','HorizontalAlignment','right','Position', [0 40 150 15],'tag','txLaserSync');
            pmChannel = most.gui.uicontrol('parent',dialogFig,'style','popupmenu','string',[{''} cellfun(@(hIO){hIO.channelName},hDIs)],'Position', [155 43 50 15],'tag','pmLaserSync');
            if most.idioms.isValidObj(obj.hScan2D.LRRLaserSyncPort)
                pmChannel.pmValue = obj.hScan2D.LRRLaserSyncPort.channelName;
            end
            ...
            function enterInput()
                obj.hScan2D.LRRLaserSyncPort = hDIs{pmChannel.Value};
                close(dialogFig);
                delete(dialogFig);
            end
        end
    ...
    methods
        function set.triggerNominalPeriodTicksStr(obj,v)
            try
                ...
                v = str2double(v);
                validateattributes(v,{'numeric'},{'nonnegative','finite','scalar','real'})
            catch ME
                most.ErrorHandler.logAndReportError(false,ME);
            end

            % check for non-integer custom clock inputs
            if strcmp(obj.laserSel, obj.PSEUDO_CLOCK_NAME) && mod(v,1) ~= 0
                warndlg('Custom clock period must be an integer.');
                return
            end
            ...
            if v >= obj.AUTO_CORRECT_BUTTON_CUTOFF_TICKS
                obj.tbAutoCorrect.Visible = 'on';
                obj.tbAutoCorrect.Enable = 'on';
            else
                obj.tbAutoCorrect.Visible = 'off';
                obj.tbAutoCorrect.Enable = 'off';
            end
        ...
        function set.clockMult(obj,v)
            ...
            cutoff = obj.AUTO_CORRECT_BUTTON_CUTOFF_TICKS;
            usingLrrWithAutoThreshold = obj.hScan2D.useCustomFilterClock && obj.hScan2D.customFilterClockPeriod >= cutoff;
            if obj.isH && (v >= cutoff || usingLrrWithAutoThreshold)
                obj.tbAutoCorrect.Visible = 'on';
                obj.tbAutoCorrect.Enable = 'on';
            else
                obj.tbAutoCorrect.Visible = 'off';
        ...
        function set.laserSel(obj,v)
            ...
            cutoff = obj.AUTO_CORRECT_BUTTON_CUTOFF_TICKS;
            usingLrrWithAutoThreshold = obj.hScan2D.useCustomFilterClock && obj.hScan2D.customFilterClockPeriod >= cutoff;
            if obj.isH && (obj.clockMult >= cutoff || usingLrrWithAutoThreshold)
                obj.tbAutoCorrect.Visible = 'on';
                obj.tbAutoCorrect.Enable = 'on';
            else
                obj.tbAutoCorrect.Visible = 'off';
        ...
        % check lasergate compatibility
        % note: use stored backend values and not frontend in case sync fails and system falls back to internal clock
        function v = get.laserGatingAvailable(obj)
            clockTrigger = isa(obj.hScan2D.LaserTriggerPort,'dabs.resources.ios.CLKI') || obj.hScan2D.useCustomFilterClock;
            diTrigger = isa(obj.hScan2D.LaserTriggerPort,'dabs.resources.ios.DI');

            multiplier = obj.hScan2D.customFilterClockPeriod;
            compatibleMultiplier = mod(multiplier,1)==0 && multiplier > 1; % integer > 1

            available = diTrigger || (clockTrigger && compatibleMultiplier);
            v = available;
        end
    end
end
