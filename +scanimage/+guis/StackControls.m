classdef  StackControls < most.Gui    
    ...
    methods (Access = protected)
        function initGui(obj)
            ...
            obj.cbStackEnable = most.gui.uicontrol('Parent',hFlowAx,'Style','checkbox','String','      Enable Stack','Tag','cbStackEnable','HeightLimits',[20 20],'callback',@(src,evt)obj.changedStackEnable(src,evt));
            obj.cbStackEnable.bindings = {obj.hModel.hStackManager,'enable','value'};
            obj.changedStackEnable();
            
            obj.hListeners(end+1) = most.ErrorHandler.addCatchingListener(obj.hModel.hStackManager,'enable','PostSet',@obj.changedStackEnable);
            ...
            obj.hListeners(end+1) = most.ErrorHandler.addCatchingListener(obj.hModel.hStackManager,'arbitraryZsAreSampleRelative',  'PostSet',  @obj.changedArbitraryCoordinateSystem);
            ...
            obj.tabsChanged();
            obj.changedArbZs();
            obj.changedArbitraryCoordinateSystem();
            obj.zsChanged();
    ...
    methods
        ...
        function populateTabs(obj,hParent)
            ...
            function populateArbitraryPanel()
                ...
                most.gui.uicontrol('Parent',hTab,'String','Coordinate System','style','text','Tag','lbArbCoords','HorizontalAlignment','left','RelPosition', [7.66666666666663 144.333333333333 92 17]);
                obj.addUiControl('Parent',hTab,'String',{'Sample' 'Reference'},'style','popupmenu','Tag','pmArbCoords','RelPosition', [106 144.333333333333 78 20],'Callback',@(src,evt)obj.setArbitraryCoordinateSystem(src,evt));
                
                most.gui.uicontrol('Parent',hTab,'String','Copy from ROI group','style','pushbutton','Tag','pbCopyFromRoiGroup','RelPosition', [23 171 130 20],'Callback',@obj.copyArbitraryZFromRoiGroup);
            end
    ...
    methods
        function validateSettings(obj,varargin)            
            ...
            if ~isempty(ss.beams) && all([ss.beams.pzAdjust]==scanimage.types.BeamAdjustTypes.None) || ~isempty(ss.slowbeams) && all([ss.slowbeams.pzAdjust]==scanimage.types.BeamAdjustTypes.None) && obj.hModel.hStackManager.overrideLZs
        ...
        function changedStackEnable(obj, varargin)
            figureBackgroundColor = obj.hFig.Color;
            if obj.hModel.hStackManager.enable
                obj.cbStackEnable.hCtl.BackgroundColor = figureBackgroundColor;
            else
                if most.gui.luminance2perceived(most.gui.rgb2luminance(figureBackgroundColor)) > 0.5
                    obj.cbStackEnable.hCtl.BackgroundColor = figureBackgroundColor * 0.8;
                else
                    obj.cbStackEnable.hCtl.BackgroundColor = min(figureBackgroundColor / 0.5,1);
                end
            end
        end
        ...
        function setArbitraryCoordinateSystem(obj,src,evt)
            strcell = src.String(src.Value);
            str = strcell{1};
            switch str
                case 'Sample'
                    obj.hModel.hStackManager.arbitraryZsAreSampleRelative = true;
                case 'Reference'
                    obj.hModel.hStackManager.arbitraryZsAreSampleRelative = false;
                otherwise
                    %No-op
            end
        end

        function changedArbitraryCoordinateSystem(obj,varargin)
            obj.pmArbCoords.pmValue = most.idioms.ifthenelse(obj.hModel.hStackManager.arbitraryZsAreSampleRelative,'Sample','Reference');
            obj.changeArbZs();
        end
        ...
        function changedArbZs(obj,varargin)
            ...
            primaryZs = zs(:,1);
            obj.etArbitraryZs.String = strjoin(strsplit(mat2str(primaryZs')),'   ');
        end
