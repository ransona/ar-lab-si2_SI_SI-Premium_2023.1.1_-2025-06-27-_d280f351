    ...
    methods
        function obj = LineScanDataView(filename,debug)
            obj.hFig = most.idioms.figure( ...
                'name','ScanImage Data View' ...
                , 'units','pixels'...
                , 'position', most.gui.centeredScreenPos([1600 800]) ...
                , 'CloseRequestFcn', @(varargin)delete(obj) ...
                , 'visible','on' ...
    ...
    methods
        ...
        function loadData(obj,filename)
            ...
            if isfield(header.SI.hScan2D, 'sampleRateCtl')
                sampleRateCtl_Hz = header.SI.hScan2D.sampleRateCtl;
            else
                sampleRateCtl_Hz = 500e3;
            end
            obj.scannerSet.scanners{1}.sampleRateHz = sampleRateCtl_Hz;
            obj.scannerSet.scanners{2}.sampleRateHz = sampleRateCtl_Hz;
            
            if isfield(header.SI.hScan2D, 'linScanParkPosition')
                linScanParkPosition = header.SI.hScan2D.linScanParkPosition;
                obj.scannerSet.scanners{1}.hDevice.parkPosition = linScanParkPosition(1);
                obj.scannerSet.scanners{2}.hDevice.parkPosition = linScanParkPosition(2);
            end
            
            for idx = 1:numel(obj.scannerSet.beams)
                obj.scannerSet.beams(idx).sampleRateHz = sampleRateCtl_Hz;
            end
            
            interpAlgorithm = header.SI.hFastZ.positionInterpolationAlgorithm;
            for idx = 1:numel(obj.scannerSet.fastz)
                obj.scannerSet.fastz(idx).sampleRateHz = sampleRateCtl_Hz;
                obj.scannerSet.fastz(idx).positionInterpolationAlgorithm = interpAlgorithm;
            end
            ...
            obj.hFovAxes.ZLim = [min(min(obj.scanPath.Z,[],'all'),0) max(max(obj.scanPath.Z,[],'all'),0.01)];
