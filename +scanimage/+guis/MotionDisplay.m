classdef MotionDisplay < most.Gui
    ...
    properties (Access = private)
        ...
        hFrameRollingAverageFactorListener
    ...
    methods
        ...
        function delete(obj)
            ...
            most.idioms.safeDeleteObj(obj.hFrameRollingAverageFactorListener);
        end
    end
    ...
    methods (Access = protected)
        function initGui(obj)
            ...
            obj.hFrameRollingAverageFactorListener = most.ErrorHandler.addCatchingListener(obj.hModel.hDisplay,'displayRollingAverageFactor','PostSet',@(varargin)obj.displayRollingAverageFactorChanged);
        end
        
        function initGeneralSettingsPanel(obj,hParent)
            set(hParent,'HeightLimits',[140 140]);
            h = most.gui.uicontrol(...
                ...
                'Tag', 'cbDetectionEnable');
            
            obj.cbAverage = most.gui.uicontrol(...
                'parent', hParent, ...
                'Style', 'checkbox', ...
                'String', 'Average', ...
                'TooltipString', 'Compare averaged live image with reference to detect motion with more confidence but increased lag following motion', ...
                'Bindings',{{obj.hModel.hMotionManager 'useAveragedStripe' 'value'}},...
                'Units', 'pixels', ...
                'RelPosition', [10 60 200 15],...
                'Tag','cbAverage');
            obj.displayRollingAverageFactorChanged();
            
            obj.cbThrottle = most.gui.uicontrol(...
                'parent', hParent, ...
                'Style', 'checkbox', ...
                'String', 'Throttle', ...
                'TooltipString', 'Detect motion once every Average Frame Rolling Factor number of frames has been acquired from the last detection.', ...
                'Bindings',{{obj.hModel.hMotionManager 'estimateOnNewAveragedFrames' 'value'}},...
                'Units', 'pixels', ...
                'RelPosition', [89 60 200 15],...
                'Enable','off',...
                'Tag','cbThrottle');
            
            obj.cbAverage.callback = @(src,evt)toggleEnable(src,evt,obj.cbThrottle);
            ...
            h = most.gui.uicontrol(...
                ...
                'RelPosition', [2 130 85 20],...
                'Tag','pbSelectCorrector');
            ...
            obj.hTextMotionCorrector = most.gui.uicontrol(...
                ...
                'RelPosition', [90 130 120 20],...
                'Tag','etCorrector');
            
            function toggleEnable(src,evt,hUIControl)
                hUIControl.Enable = most.gui.OnOff(src.Value);
            end
    ...
    methods
        function editTable(obj,src,evt)
            ...
            if idx(2) == 2
                obj.enableMotionEstimator(hME,evt.EditData);
            end
        end
        ...
        function enableMotionEstimator(obj,hME,enable)
            hMEs = obj.getMotionEstimators();
            
            switch obj.hModel.hMotionManager.correctorClassName
                case 'scanimage.components.motionCorrectors.MariusMotionCorrector'
                    
                    hMEEnableMask = [hMEs.enable];
                    hAlreadyEnabledMEs = hMEs(hMEEnableMask);
                    
                    allRefZsEqual = all(arrayfun(@(hEnabledME)isequal(hEnabledME.zs,hME.zs),hAlreadyEnabledMEs));
                    if ~allRefZsEqual && ~hME.enable
                        for hAlreadyEnabledME = hAlreadyEnabledMEs
                            if isequal(hME,hAlreadyEnabledME) || isequal(hAlreadyEnabledME.zs,hME.zs) && enable
                                continue;
                            end
                            %most.idioms.warn('Marius Motion Corrector: Cannot process z correlations for estimators with different zs.');
                            hAlreadyEnabledME.enable = 0;
                        end
                    end
        ...
        function enableAllEstimators(obj,tf)
            ...
            for idx = 1:length(hMEs)
                try
                    obj.enableMotionEstimator(hMEs(idx),tf);
                catch ME
        ...
        function displayRollingAverageFactorChanged(obj,varargin)
            obj.cbAverage.Enable = obj.hModel.hDisplay.displayRollingAverageFactor > 1;
            if obj.hModel.hDisplay.displayRollingAverageFactor == 1
                obj.hModel.hMotionManager.useAveragedStripe = false;
                obj.hModel.hMotionManager.estimateOnNewAveragedFrames = false;
                obj.cbThrottle.Enable = false;
            end
        end
                ...
                for idx = 1:length(hMEs)
                    ...
                    if hME.enable
                        ...
                        if ~isempty(sf)
                            ...
                            % does the z match to the nearest 100 nm?
                            zMask = round(obj.currentZ .* 10) == round(hME.zs .* 10);
                            if any(zMask)
        ...
        function updateScanfieldView(obj,forceTraverseEntireHistory)
            ...
            for histIdx = numel(motionHistory):-1:startIdx
                ...
                imageData = roiData.imageData{chIdx}{1}./obj.hModel.hDisplay.displayRollingAverageFactor^obj.hModel.hMotionManager.useAveragedStripe;
        ...
        function updateZContextMenu(obj)
            obj.hMenuAutoScaleZCorrelation.Checked = most.gui.OnOff(obj.autoScaleZCorrelation);
        end
