classdef TileView < most.Gui
    ...
    properties(Hidden, SetAccess=private)
       ...
       hTileToolListener = event.listener.empty();              % [Listener] Listens for destruction of the tile tool
    end
    ...
    properties(Hidden)                       
        ...
        currentFovSize = 100;
        currentFovPos = [0 0];
        hRangeOutline = matlab.graphics.primitive.Line.empty;
    ...
    methods
        ...
        function delete(obj)
            ...
            most.idioms.safeDeleteObj(obj.hFigEditScanTiles);
    ...
    methods
        ...
        function configureListeners(obj)
            ...
            %Change the extent outline if the user zeros there coordinate system.
            obj.hPropListeners =[obj.hPropListeners most.ErrorHandler.addCatchingListener(obj.hModel.hCoordinateSystems.hCSSampleRelative, 'changed', @(varargin)obj.updateExtentsOutline())];
            ...
            obj.hPropListeners = [obj.hPropListeners most.ErrorHandler.addCatchingListener(obj.hModel.hChannels, 'channelsAvailable', 'PostSet', @updateAllTables)];
            ...
            function updateAllTables(varargin)
                obj.updateLiveChanTable(varargin{:});
                obj.updateOverviewChanTable(varargin{:});
                obj.updateScanChanTable(varargin{:});
            end
        end
    ...
    methods
        ...
        function updateLiveDisplay(obj, stripeData)
            persistent lastStripe lastUpdatedTime;
            
            if isempty(lastUpdatedTime)
                lastUpdatedTime = tic();
            end
            
            if toc(lastUpdatedTime) < 0.2
                return;
            end
            lastUpdatedTime = tic();
        ...
        function val = get.liveTileZs(obj)
            if ~most.idioms.isValidObj(obj.hLiveTileDisp) || ~obj.liveTileEnable
                val = [];
            else
    ...
    methods
        ...
        function samplePositionUpdated(obj, varargin)
            ...
            if most.idioms.isValidObj(obj.hLiveTileDisp)
                hCS = obj.hModel.hCoordinateSystems;
                hPt = hCS.focalPoint.transform(hCS.hCSSampleRelative);
                arrayfun(@(tile)tile.setNewSamplePoint(hPt.points),obj.hLiveTileDisp.hTile);
                obj.updateLiveDisplay();
            end
            % Set Z?
        end
    ...
    methods
        ...
        function val = get.liveChansToShow(obj)
            val = obj.liveChansToShow;
            numChannels = obj.hModel.hChannels.channelsAvailable;
            val(end+1:numChannels) = false;
            val(numChannels+1:end) = [];
        end
        ...
        function val = get.liveChanImageColors(obj)
            val = obj.liveChanImageColors;
            numChannels = obj.hModel.hChannels.channelsAvailable;
            val(end+1:numChannels) = {'grey'};
            val(numChannels+1:end) = [];
        end
        ...
        function val = get.liveChanAlphas(obj)
            val = obj.liveChanAlphas;
            numChannels = obj.hModel.hChannels.channelsAvailable;
            val(end+1:numChannels) = 1;
            val(numChannels+1:end) = [];
        end
        ...
        function val = get.overviewChansToShow(obj)
            val = obj.overviewChansToShow;
            numChannels = obj.hModel.hChannels.channelsAvailable;
            val(end+1:numChannels) = false;
            val(numChannels+1:end) = [];
        end
        ...
        function val = get.overviewChanImageColors(obj)
            val = obj.overviewChanImageColors;
            numChannels = obj.hModel.hChannels.channelsAvailable;
            val(end+1:numChannels) = {'grey'};
            val(numChannels+1:end) = [];
        end
        ...
        function val = get.overviewChanAlphas(obj)
            val = obj.overviewChanAlphas;
            numChannels = obj.hModel.hChannels.channelsAvailable;
            val(end+1:numChannels) = 1;
            val(numChannels+1:end) = [];
        end
        ...
        function val = get.scanChansToShow(obj)
            val = obj.scanChansToShow;
            numChannels = obj.hModel.hChannels.channelsAvailable;
            val(end+1:numChannels) = false;
            val(numChannels+1:end) = [];
        end
        ...
        function val = get.scanChanImageColors(obj)
            val = obj.scanChanImageColors;
            numChannels = obj.hModel.hChannels.channelsAvailable;
            val(end+1:numChannels) = {'grey'};
            val(numChannels+1:end) = [];
        end
        ...
        function val = get.scanChanAlphas(obj)
            val = obj.scanChanAlphas;
            numChannels = obj.hModel.hChannels.channelsAvailable;
            val(end+1:numChannels) = 1;
            val(numChannels+1:end) = [];
        end
    ...
    methods (Access = protected)
        function initGui(obj)
            ...
            displayChannels = obj.hModel.hChannels.channelDisplay;
            
            obj.liveChansToShow = false(1,availableChannels);
            obj.liveChansToShow(displayChannels) = true;
            ...
            obj.overviewChansToShow(displayChannels) = true;
            ...
            obj.scanChansToShow(displayChannels) = true;
    ...
    methods
        ...
        function populateFigure(obj)
            ...
            %% Scan Tiles
                ...
                obj.hScnTileChanTbl = most.gui.uicontrol('Parent',hControlsSideBarPanel,'Style','uitable','ColumnFormat',{'char','logical', {'grey' 'red' 'green' 'blue'}, 'numeric'},'ColumnEditable',[false,true, true,true],...
                    'ColumnName',{'Ch.','Show', 'Color', 'Alpha'},'ColumnWidth',{25 35, 50,50},'RowName',[],'RelPosition', [9 697 179 90],'Tag','ScnTileChanTblTag','CellEditCallback',@obj.scanChanTableUpdated);                    
                ...
                pbClearScanTiles = most.gui.uicontrol('Parent', hControlsSideBarPanel, 'String', 'Clear', 'tag', 'pbClearScnTiles','RelPosition', [146 736 43 31] ,'Callback', @obj.clearScanTiles);
                ...
                pbSaveScanTiles = most.gui.uicontrol('Parent', hControlsSideBarPanel, 'String', 'Save', 'tag', 'pbSaveScnTiles','RelPosition', [8 735 43 30] ,'Callback', @obj.saveScanTiles);

                pbLoadScanTiles = most.gui.uicontrol('Parent', hControlsSideBarPanel, 'String', 'Load', 'tag', 'pbLoadScnTiles','RelPosition', [54 735 43 30] ,'Callback', @obj.loadScanTiles);

                most.gui.uicontrol('Parent', hControlsSideBarPanel, 'String', 'Edit', 'tag', 'pbOpenScanTileEditWindow','RelPosition', [100 735 43 30] ,'Callback', @obj.raiseScanTileEditWindow);
        ...
        function updateExtentsOutline(obj)
            hMotor = obj.hModel.hMotors.hMotorXYZ{1};
            axesXY = obj.hModel.hMotors.motorAxisXYZ;
            
            if isprop(hMotor,'range_um')
                xRange = hMotor.range_um(axesXY(1),:);
                yRange = hMotor.range_um(axesXY(2),:);

                points = [xRange(1), yRange(1);
                    xRange(2), yRange(1);
                    xRange(2), yRange(2);
                    xRange(1), yRange(2);
                    xRange(1), yRange(1)];
                ...
                points(:,3) = 0;
                points = scanimage.mroi.coordinates.Points(obj.hModel.hMotors.hCSAxesPosition,points);
                points = points.transform(obj.hModel.hMotors.hCSSampleRelative);
                points = points.points(:,[1 2]);
                
                xx = points(:,1);
                yy = points(:,2);
                zz = repmat(obj.currentZ,5,1);
                
                if most.idioms.isValidObj(obj.hRangeOutline)
                    set(obj.hRangeOutline,'XData',xx,'YData',yy);
                else
                    obj.hRangeOutline = line(obj.hFovAxes,xx,yy,zz,'LineStyle',':','Color','yellow');
                end
            elseif most.idioms.isValidObj(obj.hRangeOutline)
                obj.hRangeOutline.delete();
            end
        end
        ...
        function launchLiveTileDataWindow(obj,varargin)
            if most.idioms.isValidObj(obj.hLiveTileDataWindow)
                ...
                most.idioms.safeDeleteObj(obj.hLiveTileDataWindow);
                obj.hLiveTileDataWindow = most.idioms.figure( ...
                    'Name', 'Live Tile Data' ...
                    , 'CloseRequestFcn', @(src,evt)hideClose(src,evt) ...
                    , 'ToolBar', 'auto'...
                    , 'Resize', 'off' ...
                    , 'Position' , most.gui.centeredScreenPos([200 150]) ...
                    , 'Visible', 'off');
        ...
        function raiseScanTileEditWindow(obj,src,evt)
           hScanTiles = []; %declare variable to share scope with nested functions
           selectedCells = [];
           
            %GUI stuff
            if most.idioms.isValidObj(obj.hFigEditScanTiles)
                obj.hFigEditScanTiles.Visible = 'on';
                most.idioms.figure(obj.hFigEditScanTiles);
            else
                most.idioms.safeDeleteObj(obj.hFigEditScanTiles);
                obj.hFigEditScanTiles = most.idioms.figure('Name', 'Edit Scan Tiles', 'numbertitle', 'off', 'CloseRequestFcn',  @(src,evt)hideClose(src,evt), 'ToolBar', 'auto',...
                    'MenuBar', 'none', 'Resize', 'off', 'Position', most.gui.centeredScreenPos([628 300]), 'Visible', 'on');
                
                obj.hScanTilesEditTable = most.gui.uicontrol('Parent', obj.hFigEditScanTiles, 'Style', 'uitable', 'ColumnFormat', {'numeric', 'numeric', 'numeric', 'numeric', 'numeric', 'numeric', 'numeric', 'char'}, 'ColumnEditable', [true true true true true true true false],...
                    'ColumnName', {'X [um]', 'Y [um]', 'Z [um]', 'Width [um]', 'Height [um]', 'Resolution X', 'Resolution Y' 'Delete'}, 'ColumnWidth', {75 75 75 75 75 75 75 50}, 'RelPosition', [0 300 628 300], 'Tag', 'scnTileEditTableTag', 'CellEditCallback', @scanTileEditTableCbFunc,...
                    'CellSelectionCallback',@(src,evt)scanTileEditTableCellSelectionCallback(src,evt));
            end 
            ...
            function redraw()
                %Update data
                hScanTiles = obj.hModel.hTileManager.hScanTiles;
                
                numScanTiles = numel(hScanTiles);
                
                XYZData = reshape([hScanTiles(:).samplePoint],3,numScanTiles)';
                widths = arrayfun(@(st)max(diff(st.tileCornerPts(:,1),1,1)),obj.hModel.hTileManager.hScanTiles');
                heights = arrayfun(@(st)max(diff(st.tileCornerPts(:,2),1,1)),obj.hModel.hTileManager.hScanTiles');
                resX = arrayfun(@(st)st.resolutionXY(1,1),obj.hModel.hTileManager.hScanTiles');
                resY = arrayfun(@(st)st.resolutionXY(1,2),obj.hModel.hTileManager.hScanTiles');
                
                data_ = num2cell([XYZData widths heights resX resY]);
                data_ = horzcat(data_, arrayfun(@(x)most.constants.Unicode.ballot_x,(1:numScanTiles)','UniformOutput',false));
                
                obj.hScanTilesEditTable.Data = data_;
            end
            
            function scanTileEditTableCbFunc(src,evt)
                row = evt.Indices(1);
                column = evt.Indices(2);
                newData = src.Data(evt.Indices(1),evt.Indices(2));
                
                selectedCells_ = selectedCells;
                selectedCells_(end+1,:) = [row column];
                mask = selectedCells_(:,2) == column;
                selectedCells_(~mask) = [];
                rows = unique(selectedCells_(:,1));
                
                
                for row = rows(:)'
                    scanTile = hScanTiles(row);
                    data = src.Data(row,:);
                    data(column) = newData;
                    
                    switch column
                        case 1
                            determineSizeAndLocation();
                        case 2
                            determineSizeAndLocation();
                        case 3
                            determineSizeAndLocation();
                        ...
                        case 5
                            determineSizeAndLocation();
                        case 6
                            scanTile.resolutionXY(1) = data{6};
                        case 7
                            scanTile.resolutionXY(2) = data{7};
                        otherwise
                    end
                end
                
                hTimer.TimerFcn = @(varargin)updateAndRedraw();
                hTimer.start();
                
                function updateAndRedraw()
                    arrayfun(@(std)std.updateSurface(),obj.hScanTileDisps);
                    redraw();
                end
                        
                function determineSizeAndLocation()
                    x = data{1};
                    y = data{2};
                    z = data{3};
                    halfWidth = data{4}/2;
                    ...
                    scanTile.samplePoint = [x y z];
                    scanTile.tileCornerPts = [x-halfWidth, y-halfHeight;...
            ...
            function scanTileEditTableCellSelectionCallback(src,evt)
                ...
                column = evt.Indices(2);
                deletionColumn = 8;
                if column == deletionColumn
                    ScanTileIdx = evt.Indices(1);
                    scanTile = hScanTiles(ScanTileIdx);
                    scanTile.delete();
                    selectedCells = [];
                else
                    selectedCells = evt.Indices;
                    return;
                end
            ...
            function hideClose(src,evt,varargin)
                obj.hFigEditScanTiles.Visible = 'off';
                most.idioms.safeDeleteObj(hTimer);
                most.idioms.safeDeleteObj(obj.hFigEditScanTiles);
                most.idioms.safeDeleteObj(obj.hScanTilesEditTable);
            end
        end
        ...
        function launchSavedPositionWindow(obj, varargin)
            if most.idioms.isValidObj(obj.hSavedPositionsWindow)
                ...
                most.idioms.safeDeleteObj(obj.hSavedPositionsWindow);
                obj.hSavedPositionsWindow = most.idioms.figure( ...
                    'Name', 'Saved Positions' ...
                    , 'CloseRequestFcn', @(src,evt)hideClose(src,evt) ...
                    , 'ToolBar', 'auto' ...
                    , 'Resize', 'off' ...
                    , 'Position', most.gui.centeredScreenPos([328 300]) ...
                    , 'Visible', 'off');
        ...
        function makeTool(obj,toolClassName)
            ...
            previousLiveTileEnableState = obj.liveTileEnable;
            
            if ~previousLiveTileEnableState
                obj.liveTileEnable = true;
            end
            
            constructor = str2func(toolClassName);
            obj.tileTool = constructor(obj);
            
            obj.hTileToolListener = most.ErrorHandler.addCatchingListener(obj.tileTool, 'ObjectBeingDestroyed', @(varargin)turnOffLiveTile());
            
            function turnOffLiveTile()
                if ~previousLiveTileEnableState
                    obj.liveTileEnable = false;
                end
                
                most.idioms.safeDeleteObj(obj.hTileToolListener);
...
%             end
        ...
        function scrollWheelFcn(obj,~,evt)
            ...
            if most.gui.isMouseInAxes(obj.hFovAxes)
                ...
                if isvalid(obj.Crosshair)
                    ...
                    obj.Crosshair.ZData = min(obj.hFovAxes.ZLim) * ones(1,5) + 0.00001;
                end
                ...
                if isvalid(obj.crosshairCenter)
                    ...
                    obj.crosshairCenter.ZData = min(obj.hFovAxes.ZLim);
        ...
        function setZ(obj, varargin)
            ...
            if tfCont
                ...
                if most.idioms.isValidObj(obj.hRangeOutline)
                    obj.hRangeOutline.ZData = repmat(min(obj.hFovAxes.ZLim),5,1);
                end
            end
        end
