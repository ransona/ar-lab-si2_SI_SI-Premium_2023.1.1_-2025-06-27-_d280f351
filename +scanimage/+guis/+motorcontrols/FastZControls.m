classdef FastZControls < handle    
    ...
    methods
        function obj = FastZControls(hGui,hParent,hFastZ,postfix)
            ...
            obj.hListeners(end+1) = addlistener(hFastZ,'travelRange','PostSet',@(varargin)obj.updateStepIfDiscrete);
            ...
            obj.fastZIncrement = 10; %Eigenset specifically for case where FastZ is a Thorlabs liquid crystal FastZ
        end
        ...
        function updateStepIfDiscrete(obj)
            if isa(obj.hFastZ, 'dabs.thorlabs.LiquidCrystalFastZ')
                obj.fastZIncrement = mean(diff(obj.hFastZ.slicePositions));
            end
        end
    ...
    methods
        function set.fastZIncrement(obj,val)
            if isa(obj.hFastZ, 'dabs.thorlabs.LiquidCrystalFastZ')
                obj.fastZIncrement = mean(diff(obj.hFastZ.slicePositions));
            else
                obj.fastZIncrement = val; 
            end
        end
    end
end
