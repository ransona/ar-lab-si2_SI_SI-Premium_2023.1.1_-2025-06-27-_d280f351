classdef AlignmentControls < most.Gui & most.HasClassDataFile
    ...
    properties (SetObservable)
        ...
        allowFixedControlPoints = true;

        showMotionMarkers = false;
    ...
    properties (SetAccess = protected,SetObservable,Hidden)        
        ...
        % TODO: these vector properties are coupled by index, could be a vector of classes instead
        hControlPoints matlab.graphics.primitive.Line;
        hControlPointMenus matlab.ui.container.ContextMenu;
        controlPointPositions = {};
        controlPointFixed = [];

        hMotionMarkerLine matlab.graphics.primitive.Line;
        ...
        function initGui(obj)
           set(obj.hFig,'Name','ALIGNMENT CONTROLS','Resize','off');
            ...
            obj.addUiControl(...
                ...
                'Tag','pgGenerate',...
                'Enable',most.gui.OnOff(scanimage.SI.PREMIUM));
            ...
            %listeners to motion markers
            obj.hListeners(end+1) = most.ErrorHandler.addCatchingListener(obj.hModel.hMotionManager,...
                'motionMarkersXY', 'PostSet', @obj.motionMarkersXYChanged);
    ...
    methods
        ...
        function raise(obj)
            raise@most.Gui(obj);
            obj.allowFixedControlPoints = true;
            obj.showMotionMarkers = false;
        end
...
%             end
        ...
        function redrawPatternsAndMenus(obj,source)
            if nargin < 2 || isempty(source)
                source = obj.alignmentSource;
            end
            ...
            camerasExist = isprop(obj.hModel,'hCameraManager')...
                && ~isempty(obj.hModel.hCameraManager)...
            ...
            matchesCameraName = strcmp(source, cameraNames);
            
            % TODO: make these types of alignment control "allowFixedControlPoints" for 
            %       enabling the Add Fixed Point menu, instead of based on alignmentSource
            if strcmp(source,'Photostimulation')                
            ...
            elseif any(matchesCameraName)
                ...
                ch = str2double(regexpi(source,'(?<=^Channel\s)[0-9]*$','match','once'));
                if ~isempty(ch) && ~isnan(ch)
                    ...
                    stimPathVisible = 'off';
                elseif strcmpi(source,'SLM Test Pattern')
                    obj.testPattern = obj.hModel.hSlmScan.testPattern;

                    obj.pbPause.Visible = 'off';
                    stimPathVisible = 'off';
                elseif strcmpi(source,'SLM Points')
                ...
                obj.hMainContextMenuAEs.Enable = 'on';
                obj.hMainContextMenuAFx.Enable = obj.allowFixedControlPoints;
                obj.hMainContextMenuAFr.Enable = 'on';
            end
        end
    ...
    methods
        ...
        function set.alignmentSource(obj,val)
            ...
            obj.redrawPatternsAndMenus(val);
        ...
        function set.allowFixedControlPoints(obj,v)
            obj.allowFixedControlPoints = v;
            obj.redrawPatternsAndMenus();
        end

        function set.showMotionMarkers(obj,v)
            obj.showMotionMarkers = v;
    ...
    methods
        ...
        function saveScannerTransform(obj)
            ...
            str = sprintf('This will overwrite the alignment data for %s. Continue?',hTargetScanner.name);
            if strcmp(questdlg(str,'ScanImage','Continue','Cancel','Cancel'), 'Continue')
            ...
            if ~isempty(obj.hModel.hDisplay.rollingStripeDataBuffer{1}{1}.roiData)
                ...
                if tf
                    ...
                    for i = Nroi:-1:1
                        ...
                        obj.referenceData.roiDat(i).centerXY = sf.centerXY;
                    end
        ...
        function addControlPoint(obj, fixed)            
            ...
            hLine = line('XData',vidPt(1),'YData',vidPt(2),'ZData',10,'Parent',obj.hAlignmentAx,'LineStyle','none','Marker','o','MarkerEdgeColor',markerEdgeColor,'MarkerFaceColor',markerFaceColor,'Markersize',10,'LineWidth',1.5,'UIContextMenu',obj.hControlPointMenus(idx));
            obj.hControlPoints(idx) = hLine;
            obj.controlPointPositions{idx} = refPt;
            obj.controlPointFixed(idx) = false;
            
            hCP = obj.hControlPoints(idx);
            set(hCP,'ButtonDownFcn',@(varargin)obj.cpFunc(hCP));

            % TODO: disable "Fix Control Point" when not allowed
            ...
            if nargin > 1 && fixed && obj.allowFixedControlPoints
        ...
        function toggleControlPointFixState(obj, hdl)
            ...
            if ptsFxd(idx)
                ...
                if ~obj.allowFixedControlPoints
                    warndlg('Fixed control points are not allowed with this type of alignment','Warning','modal');
                    error('Fixed control points not allowed for this type of alignment');
                end
                if sum(ptsFxd) >= 4
        ...
        function clearControlPoint(obj, hdl)
            ...
            hControlPoint     = obj.hControlPoints(idx);
            hControlPointMenu = obj.hControlPointMenus(idx);
            ...
            most.idioms.safeDeleteObj(hControlPoint);
            most.idioms.safeDeleteObj(hControlPointMenu);
        end
    ...
    methods (Hidden)
        ...
        function resetAlignmentFig(obj,show)
            ...
            if ~most.idioms.isValidObj(obj.hAlignmentFig)
                ...
                most.gui.tetherGUIs(obj.hFig,obj.hAlignmentFig,'righttop',8);
            end
            ...
            obj.hMainContextMenu = uicontextmenu('Parent',obj.hAlignmentFig);
                ...
                obj.hMainContextMenuAEs = uimenu('Parent',obj.hMainContextMenu,'Label','Auto Estimate Transform','Callback',@(src,evt)obj.estimateTransform(),'Enable',scanimage.SI.PREMIUM);
                ...
                obj.hMainContextMenuAFx = uimenu('Parent',obj.hMainContextMenu,'Label','Add Fixed Control Point','Callback',@(src,evt)obj.addControlPoint(true),'Enable',obj.allowFixedControlPoints);
            ...
            % reset motion markers (used in stage alignment)
            most.idioms.safeDeleteObj(obj.hMotionMarkerLine);
            obj.hMotionMarkerLine = line('Parent',obj.hAlignmentAx,'XData',[],'YData',[],'LineStyle','none','Marker','+','MarkerSize',8,'LineWidth',1.5,'Color',most.constants.Colors.pink);
            obj.hMotionMarkerLine.Visible = obj.showMotionMarkers;
        ...
        function cpFunc(obj,cpHdl)
            if strcmp(get(obj.hAlignmentFig,'SelectionType'), 'normal') % left click
                ...
                set(obj.hAlignmentFig,'WindowButtonMotionFcn',@move,'WindowButtonUpFcn',@stop);
            end
            ...
            function stop(varargin)
                ...
                try
                    if exist('origMarkerSize','var') && ~isempty(origMarkerSize)
                ...
                try
                    if exist('origVideoStreamAlpha','var') && ~isempty(origVideoStreamAlpha)
                ...
                for i = 1:Nr
                    if ~isempty(obj.photostimPattern.rois(i).scanfields)
                        ...
                        if ~(obj.photostimPattern.rois(i).scanfields.isPause || obj.photostimPattern.rois(i).scanfields.isPark)
                            set(obj.hStimPth(i),'ButtonDownFcn',@(varargin)obj.cpFunc(obj.hStimPth(i)));
        ...
        function loadSlmPts(obj)
            ...
            if numPts > 1
                for idx = 1:numPts
                    ...
                    obj.hStimPth(idx).ButtonDownFcn = @(varargin)obj.cpFunc(obj.hStimPth(idx));
        ...
        function motionMarkersXYChanged(obj,varargin)
            dirs = -obj.hModel.hMotionManager.motionMarkersXY;
            
            pointsInRois = cell(0,1);
            for idx = 1:numel(obj.referenceData.roiDat)
                pointsInRois{idx} = obj.referenceData.roiDat(idx).centerXY + dirs;
            end
            pointsInRois = vertcat(pointsInRois{:});
            
            if isempty(pointsInRois)
                pointsInRois = [NaN NaN];
            end
            
            obj.hMotionMarkerLine.XData = pointsInRois(:,1);
            obj.hMotionMarkerLine.YData = pointsInRois(:,2);
            obj.hMotionMarkerLine.ZData = 10*ones(size(pointsInRois,1),1);
            obj.hMotionMarkerLine.Visible = obj.showMotionMarkers;
        end
    end
end
