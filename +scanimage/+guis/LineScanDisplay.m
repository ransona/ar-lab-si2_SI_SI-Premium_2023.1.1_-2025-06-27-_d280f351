classdef LineScanDisplay < handle
    ...
    methods
        ...
        function dat = getPtData(obj,pt,chIdx)
            ...
            pt(pt<1) = [];
            pt(pt>size(obj.hDisplay.lineScanAvgDataBuffer,1)) = [];
            
            isContinuousRange = all(diff(pt)==1) && isscalar(chIdx); 
            
            if isContinuousRange
                dat = zeros(numel(pt),size(obj.hDisplay.lineScanAvgDataBuffer,2),1,'like',obj.hDisplay.lineScanAvgDataBuffer);
                
                pageIdxs = 1:size(obj.hDisplay.lineScanAvgDataBuffer,2);
                dstIdxs = sub2ind(size(dat),ones(size(pageIdxs)),pageIdxs,ones(size(pageIdxs)));
                srcIdxs = sub2ind(size(obj.hDisplay.lineScanAvgDataBuffer),pt(1)*ones(size(pageIdxs)),pageIdxs,chIdx*ones(size(pageIdxs)));
                numElementsToCopy = (pt(end)-pt(1)+1) * ones(size(pageIdxs));
                
                most.memfunctions.inplacewrite(dat,obj.hDisplay.lineScanAvgDataBuffer,dstIdxs,srcIdxs,numElementsToCopy);
            else
                dat = obj.hDisplay.lineScanAvgDataBuffer(pt,:,chIdx);
            end
            
            dat = mean(dat,1);
            dat = dat * (1 / obj.dataMultiplier);
        end
