classdef  BeamControls < most.Gui    
    ...
    properties (SetAccess=protected,Hidden)
        ...
        hCbEnablePowerBox;
        hCbFlybackBlanking;
        hCbTurnAroundBlanking;
        hCbXTurnaroundBlanking;
    ...
    properties (SetAccess=protected,SetObservable)
        ...
        hBeamDevicesListeners = event.listener.empty(1,0);
    end
    ...
    methods
        ...
        function val = get.turnAroundBlanking(obj)
           val = obj.hModel.hBeams.turnAroundBlanking; 
        end
        
        function set.turnAroundBlanking(obj,val)
            obj.hModel.hBeams.turnAroundBlanking = val;
        end
        ...
        function val = get.scanner(obj)
            ...
            if isempty(obj.hModel.hBeams.hBeams) || obj.beamIdx <= 0
                return
            end
    ...
    methods
        ...
        function refreshGui(obj)
            cellfun(@(el)most.idioms.safeDeleteObj(obj.(el)),obj.hCtls);
            obj.initGui(); 
        end
        
        function delete(obj)
            most.idioms.safeDeleteObj(obj.hListeners);
            most.idioms.safeDeleteObj(obj.hBeamDevicesListeners);
    ...
    methods (Access=protected,Hidden)
        ...
        function makeTopPanel(obj,hFlParent)
            ...
            hFlTopHeightLimit = most.gui.uiflowcontainer('Parent',hFlParent,'FlowDirection','LeftToRight','HeightLimits',[50 50]);
                hPnTopItems = uipanel('Parent',hFlTopHeightLimit);
                    hFlTopItems = most.gui.uiflowcontainer('Parent',hPnTopItems,'FlowDirection','LeftToRight');
                        hFlBeamLeadTimeText = most.gui.uiflowcontainer('Parent',hFlTopItems,'FlowDirection','TopDown','WidthLimits',[70 70]);
                            most.gui.uiflowcontainer('Parent',hFlBeamLeadTimeText,'FlowDirection','TopDown','HeightLimits',[3 3]);
                            most.gui.uicontrol('Parent',hFlBeamLeadTimeText,'Style','text','String','Beam Lead Time:','HorizontalAlignment','left','FontSize',9);
                        hFlBeamLeadTimeEdit = most.gui.uiflowcontainer('Parent',hFlTopItems,'FlowDirection','TopDown','WidthLimits',[30 Inf]);
                        ...
                        hFlBeamLeadTimeUnits = most.gui.uiflowcontainer('Parent',hFlTopItems,'FlowDirection','TopDown','WidthLimits',[20 Inf]);
                            most.gui.uiflowcontainer('Parent',hFlBeamLeadTimeUnits,'FlowDirection','TopDown','HeightLimits',[3 3]);
                            most.gui.uicontrol('Parent',hFlBeamLeadTimeUnits,'Style','text','String',[most.constants.Unicode.micro_sign 's'],'HorizontalAlignment','left','FontSize',9);
                        hFlFlybackBlanking = most.gui.uiflowcontainer('Parent',hFlTopItems,'FlowDirection','TopDown','Margin',3,'WidthLimits',[75 75]);
                            obj.hCbTurnAroundBlanking = most.gui.uicontrol('Parent',hFlFlybackBlanking,'Style','checkbox','String',['Blank X ' most.constants.Unicode.leftwards_arrow_with_hook],'HorizontalAlignment','right','Callback',@obj.hCbTurnAroundBlanking_changed);
                            obj.hCbFlybackBlanking = most.gui.uicontrol('Parent',hFlFlybackBlanking,'Style','checkbox','String',['Blank Y ' most.constants.Unicode.north_west_arrow_with_hook],'Callback',@obj.hCbFlybackBlanking_changed,'HorizontalAlignment','right');
            ...
            hFlWrapHeightLimit = most.gui.uiflowcontainer('Parent',hFlParent,'FlowDirection','LeftToRight','HeightLimits',[120 120]);
                obj.hPnSelectedControls = uipanel('Parent',hFlWrapHeightLimit,'FontWeight','bold');
                    hFlSelectedControls = most.gui.uiflowcontainer('Parent',obj.hPnSelectedControls,'FlowDirection','TopDown');
                        ...
                        sliderWidth = 150;
                        hFlPowerSliderW = most.gui.uiflowcontainer('Parent',hFlPowerSlider,'FlowDirection','TopDown','WidthLimits',[sliderWidth sliderWidth]);
                            obj.hSldPowerSetting = most.gui.uicontrol('Parent',hFlPowerSliderW,'Style','slider','Max',100,'Callback',@obj.hSldPowerSetting_changed);
                            ...
                            hFlPowerET = most.gui.uiflowcontainer('Parent',hFlPowerSlider,'FlowDirection','LeftToRight','WidthLimits',[110 110]);
                                ...
                                hFlPowerLbl = most.gui.uiflowcontainer('Parent',hFlPowerET,'FlowDirection','TopDown','WidthLimits',[12 12]);
                                ...
                                hFlPowerSliderW = most.gui.uiflowcontainer('Parent',hFlPowerSlider,'FlowDirection','TopDown','WidthLimits',[sliderWidth sliderWidth]);
                                obj.hEtPowerWatts = most.gui.uicontrol('Parent',hFlPowerET,'Style','edit','String','0 mW','Enable','off');
        ...
        function makeLUTPlot(obj)
            ...
            obj.hFigPlot = most.idioms.figure('CloseRequestFcn',@obj.closeLUTPlot,'Visible','off');
        ...
        function addListeners(obj)
            ...
            obj.hListeners(end+1) = most.ErrorHandler.addCatchingListener(obj.hModel.hBeams,'turnAroundBlanking','PostSet',@obj.redraw);
            ...
            obj.hListeners(end+1) = most.ErrorHandler.addCatchingListener(obj.hModel.hBeams,'hBeams','PostSet',@obj.resetScannerBeamListeners);
            for idx = 1:numel(obj.hModel.hBeams.hBeams)
                obj.hBeamDevicesListeners(end+1) = most.ErrorHandler.addCatchingListener(obj.hModel.hBeams.hBeams{idx},'powerFraction2PowerWattLut','PostSet',@obj.redraw);
            end
        end

        function resetScannerBeamListeners(obj, varargin)
            most.idioms.safeDeleteObj(obj.hBeamDevicesListeners);
            obj.hBeamDevicesListeners = event.listener.empty(1,0);
    ...
    methods
        function redraw(obj,varargin)
            if isempty(obj.hModel.hBeams.currentBeams)
                obj.beamIdx =  0;
            end
        ...
        function redrawTopBottom(obj,varargin)
            ...
            obj.hCbTurnAroundBlanking.Value = obj.turnAroundBlanking;
            obj.hCbEnablePowerBox.Value = obj.enablePowerBox;
        end
        
        function redrawAdjust(obj,varargin)
            if obj.numBeams <= 0 || obj.beamIdx <= 0
            ...
            power_mW = hBeam.convertPowerFraction2PowerWatt(obj.power/100)*1000;
            obj.hEtPowerWatts.String = sprintf('%.4g mW',power_mW);
            ...
            switch obj.pzAdjust
                ...
                case scanimage.types.BeamAdjustTypes.Function
                    ...
                    obj.hEtSourceValue.hCtl.TooltipString = sprintf(['A function of the form\n' ...
                                                       'powerFractions = PowerFunction(currentBeamsPowerFractions,zPositions_um,hBeamObject,customArgumentsVarargin...)\n' ...
                                                       'located on the MATLAB path and saved as a .m file.\n' ...
                                                       'NOTE: power fractions are decimal values from 0 to 1, which represent power percents from 0%% to 100%% respectively']);
        ...
        function redrawTable(obj,varargin)
            ...
            [unusedBeamNames, unusedBeamIndex] = setdiff(rowNames,usedBeamNames);
            ...
            powers(unusedBeamIndex) = 0;
            powers = num2cell(powers);
            
            for i = 1:obj.numBeams
                notInUse = any(strcmpi(rowNames{i},unusedBeamNames));
                if notInUse
                    figColor = obj.hFig.Color;
                    if most.gui.luminance2perceived(most.gui.rgb2luminance(figColor)) < 0.5
                        colorFactor = 1/0.5;
                    else
                        colorFactor = 0.7;
                    end

                    colorCode_ = dec2hex(round(255*min(obj.hFig.Color*colorFactor,1)));
                    colorCode_ = colorCode_';
                    colorCode(1,:) = colorCode_(:);
                    rowNames{i} = ['<html><table border=0 width=400 bgcolor=#' colorCode '><TR><TD>' rowNames{i} '</TD></TR> </table></html>']; % could use HTML to highlight
                end
            end
        ...
        function redrawLUTPlot(obj,varargin)            
            if obj.numBeams <= 0 || obj.beamIdx <= 0 || obj.pzAdjust~=scanimage.types.BeamAdjustTypes.LUT
                obj.closeLUTPlot();
                return;
            end
            ...
            invalidTexFormatId = 'MATLAB:handle_graphics:exceptions:SceneNode';
            prevWarningSetting = warning('off', invalidTexFormatId);
    ...
    methods
        function tableCellSelectionCallback(obj,src,evt)
            ...
            switch evt.Indices(2)
                case 1
                    ...
                    if ~any(strcmpi(rowNames{evt.Indices(1)},usedBeamNames))
                        obj.beamIdx = 0;
                    else
                        obj.beamIdx = evt.Indices(1);
                        obj.highlightCurrentBeamWidget();
                        obj.hEtPowerValue_changed();
                    end
                otherwise
                    %No-op
            end
        ...
        function hCbTurnAroundBlanking_changed(obj,varargin)
           obj.turnAroundBlanking = logical(obj.hCbTurnAroundBlanking.Value);
        end
        ...
        function hSldPowerSetting_changed(obj,varargin)
             powerValue = round(obj.hSldPowerSetting.Value);
             obj.power = powerValue;
