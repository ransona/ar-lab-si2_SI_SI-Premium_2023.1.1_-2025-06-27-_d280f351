classdef roiIntegratorDisplayClass < handle
    ...
    methods
        function obj = roiIntegratorDisplayClass()           
            obj.hFig = handle(most.idioms.figure( ...
                'Name', 'ROI Integrator Traces' ...
                ,'Tag', 'roiIntegratorDisplay'));
            ...
            function makeTopMenuBar(ctr)
                ...
                obj.controls.etClim = handle(uicontrol('Parent',flctr,'Tag','etClim','Style','edit','String','Auto','Callback',@obj.changeCLim,'TooltipString',sprintf('Enter lower and upper bound for Clim as a 1x2 array e.g. [0 1000]\nEnter ''Auto'' for automatic scaling.')));
...
%                 obj.controls.cbEnableCorrAx = handle(uicontrol('Parent',flctr,'Tag','cbEnableCorrAx','Style','checkbox','Value',true,'Callback',@obj.changeEnableCorrAx));
    ...
    methods        
        ...
        function updateDisplay(obj,hIntegrationRois,integrationValues,integrationTimestamps)
            ...
            roisDisplay = logical([hIntegrationRois.display]);
...
%             timestamps_ = timestamps_(resortedIdxs(tfdisplayed));
        ...
        function updateChannelsDisplay(obj)
            if isempty(obj.hSI) || ...
                    isempty(obj.hSI.hChannels.channelDisplay)
                return
            end
            ...
            if ~isa(dispAxes, 'scanimage.mroi.RoiDisplay')
                return;
            end
                ...
                for idx = 1:length(dispAxes)
                    ...
                    for zidx = 1:length(zs)
                        ...
                        if any(ismember(sf.channel,dispChan))
                            [xx,yy] = sf.cornersurf;
                            surfsXData{end+1} = xx + motionOffset(1);
                            surfsYData{end+1} = yy + motionOffset(2);
