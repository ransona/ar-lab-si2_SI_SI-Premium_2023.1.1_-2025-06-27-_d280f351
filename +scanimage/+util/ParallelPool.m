    ...
    methods (Access = private)
        function initPool(obj)
            ...
            if isempty(obj.pool)
                ...
                numWorkers = max(1,numprocessors-4); % leave 2 processors for ScanImage and OS
            ...
            try
                ...
                if isfield(fevalQueueStruct,'JavaQueueObj')
                    obj.JavaQueueObj = fevalQueueStruct.JavaQueueObj;
                elseif isfield(fevalQueueStruct,'QueueImpl')
                    queueImpl = fevalQueueStruct.QueueImpl;
                    assert(isa(queueImpl,'parallel.internal.queue.JavaFevalQueue'));
                    queueImpl = struct(queueImpl);
                    obj.JavaQueueObj = queueImpl.JavaQueueObj;
                  else
                    error('Could not get handle Java Queue');
                end
                warning(warnStruct);
            catch ME
                warning(warnStruct);
