classdef AcquisitionEngine < dabs.vidrio.rdi.Device
    ...
    methods
        function obj = AcquisitionEngine(varargin)
            ...
            if obj.simulate
                ...
                obj.ACCUM_MULTIPLY_SUPPORT = uint32(1);
            end
        end
    end
    ...
    methods
        ...
        function fixedPointValue = convertToFixedPointPrecision(obj,value,fixedPointPrecision)
            if nargin < 3
                fixedPointPrecision = obj.ACCUM_DIVIDE_FIXED_POINT_PRECISION;
            end

            fixedPointValue = uint32(double(value) * (2 ^ fixedPointPrecision));
        end
        
        function writeMaskTable(obj,addr,binVal,multiplier,maxWidth)
            if obj.simulate
                return;
            end
            
            integerWidth = double(maxWidth) - double(obj.ACCUM_DIVIDE_FIXED_POINT_PRECISION);
            minMultiplierValue = 1 / double(2^obj.ACCUM_DIVIDE_FIXED_POINT_PRECISION);
            maxMultiplierValue = 2^integerWidth - minMultiplierValue;

            assert(multiplier <= maxMultiplierValue, ['Multiplier value %.3f at mask address %d is over maximum allowed, if multiplying samples please ensure the coefficient is within bounds\n'...
                'Max multiplier value: %.3f'], multiplier, addr, maxMultiplierValue);
            assert(multiplier == 0 || multiplier >= minMultiplierValue, ['Multiplier value %f at mask address %d is under minimum precision\n'...
                'Minimum multiplier precision: %f'], multiplier, addr, minMultiplierValue);

            obj.maskTableWriteCoefficientReg = obj.convertToFixedPointPrecision(multiplier);
            obj.maskTableWriteBinReg = uint32(binVal);
            obj.maskTableWriteAddrReg = uint32(addr);

            % note the mask data must be written first, and the address second
            % when the address is written, the stored data is moved to the address
        end
...
function s = initRegs()
    ...
    s.dataRegs.ACCUM_MULTIPLY_SUPPORT = struct('address',28,'hide',true);
    s.dataRegs.HSADC_LRR_SUPPORT = struct('address',32,'hide',true);
    s.dataRegs.ACCUM_DIVIDE_FIXED_POINT_PRECISION = struct('address',36,'hide',true);
    
    s.dataRegs.acqPlanWriteReg = struct('address',104,'hide',true);
    s.dataRegs.acqPlanNumSteps = struct('address',108);
    
    s.dataRegs.maskTableWriteCoefficientReg = struct('address',380,'hide',true);
    s.dataRegs.maskTableWriteBinReg = struct('address',384,'hide',true);
    s.dataRegs.maskTableWriteAddrReg = struct('address',112,'hide',true);
    ...
    s.dataRegs.acqParamMaskLSBs = struct('address',208);  % TODO: are any users using this? remove if not
    ...
    s.dataRegs.acqParamLaserClkSyncChIdx = struct('address',388);
    ...
    s.dataRegs.acqParamUniformDivisor = struct('address',376);
