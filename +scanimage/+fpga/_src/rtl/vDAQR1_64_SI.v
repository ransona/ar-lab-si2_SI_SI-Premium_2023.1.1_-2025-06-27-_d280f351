...
module vDAQR1_64_SI #(
	...
	// AFE Clock mux
	// SYNC_TRIGGER_clk must be driven by a BUFGMUX
	(* DONT_TOUCH = "true" *)
	BUFGMUX #(.CLK_SEL_TYPE("ASYNC")) afeClk_mux (
		.O(SYNC_TRIGGER_clk), // 1-bit output: Clock output
		.I0(hsadcDataClk), // 1-bit input: Clock input (S=0)
	...
	wire syncTrigReset;
	SBCC syncTrigRsetCrss(.srcV(syncTrigReset), .srcClk(pcie_aclk), .dstV(SYNC_TRIGGER_reset), .dstClk(hsadcDataClk));


	// SI Inst
	// only one fifo for the one acquisition engine in this project's block diagram
	wire dataFifoFull;
	wire dataFifoWriteEn;
	wire [5:0] dataFifoWriteWidth;
	wire [DATA_FIFO_1_WIDTH-1:0] dataFifoData;
	...
	// compute parameters
	localparam LRR_SAMPLE_PHASE_BITS        = (LRR_80kHz_SUPPORT) ? 15 : 14;
	localparam SAMPLE_PHASE_BITS            = (LRR_ENABLED) ? LRR_SAMPLE_PHASE_BITS : 12;
	localparam AE_1_HS_NUM_LOGICAL_CHANNELS = (LRR_ENABLED && LRR_80kHz_SUPPORT) ? 32 : 64;
	localparam AE_1_NUM_DIVIDERS            = AE_1_HS_NUM_LOGICAL_CHANNELS;
	localparam PHOTON_COUNTING_SUPPORT      = !LRR_ENABLED;
	
	// only 1 acquisition engine for the 64 channel project
	(* DONT_TOUCH = "true" *)
	vDAQ_SI #(
		.NUM_DIO(32),
		.NUM_AE(1),
		.AE_1_HS_NUM_LOGICAL_CHANNELS(AE_1_HS_NUM_LOGICAL_CHANNELS),
		.AE_1_MS_NUM_LOGICAL_CHANNELS(8),
		.AE_1_NUM_DIVIDERS(AE_1_NUM_DIVIDERS),
		.HSADC_SUPPORT(1),
		.NO_MSADC_SUPPORT(1),
	...
	vDAQR1_BD vDAQ_BD_inst(
		...
		// FIFOs for AE 0
		.SI_DATA_FIFO_0_wr_data(dataFifoData),
		.SI_DATA_FIFO_0_wr_en(dataFifoWriteEn),
		.SI_DATA_FIFO_0_wr_width(dataFifoWriteWidth[5:0]),
		.SI_DATA_FIFO_0_full(dataFifoFull),
		
		.SI_AUX_FIFO_0_wr_data(triggerFifoData),
		.SI_AUX_FIFO_0_wr_en(triggerFifoWriteEn),
		.SI_AUX_FIFO_0_full(triggerFifoFull),
