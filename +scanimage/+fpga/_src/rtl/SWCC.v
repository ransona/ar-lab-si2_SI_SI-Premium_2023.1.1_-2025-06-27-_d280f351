...
// SYNCHRONIZED WORD CLOCK CROSSING
// multiple bit version of SBCC
// synchronizes whole words at one time
module SWCC #(
	parameter [7:0] WW = 32,
	...
	SBCC req_crssr(.srcV(scReq), .srcClk(srcClk), .dstV(dcReq), .dstClk(dstClk));
	SBCC ack_crssr(.srcV(dcAck), .srcClk(dstClk), .dstV(scAck), .dstClk(srcClk));
	SBCC rst_crssr(.srcV(dstRst), .srcClk(dstClk), .dstV(scRst), .dstClk(srcClk));
