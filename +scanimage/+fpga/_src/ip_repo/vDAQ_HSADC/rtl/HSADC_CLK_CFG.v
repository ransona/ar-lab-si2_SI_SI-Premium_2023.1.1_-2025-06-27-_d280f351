...
module HSADC_CLK_CFG #(
	...
	parameter SAXIL_CFG_ADDR_WIDTH = 12
)(
	input wire axiClk,
	input wire axiResetN,
	
	// SAXIL control/config bus
	input  wire [SAXIL_CFG_ADDR_WIDTH-1:0] cfgReadAddress,
	output wire [SAXIL_CFG_DATA_WIDTH-1:0] cfgReadData,
	input  wire [SAXIL_CFG_ADDR_WIDTH-1:0] cfgActiveWriteAddress,
	input  wire [SAXIL_CFG_DATA_WIDTH-1:0] cfgWriteData,
	...
	always @(posedge axiClk) begin
		...
		spiClkG <= (spiClkCtr < 8) && cfgInProgress;

		if (~axiResetN) begin
			cs_b <= 1;
			cfgInProgress <= 0;
		end else if (spiClkCtr == 8) begin
	...
	// axi config slave
	reg [SAXIL_CFG_DATA_WIDTH-1:0] cfgReadDataR = 0;
	assign cfgReadData = cfgReadDataR;
	...
	always @(posedge axiClk) begin
		if (cfgActiveWriteAddress == 64)
			sync_ctr <= cfgWriteData;
		else if (sync_ctr)
			sync_ctr <= sync_ctr - 1;
		syncV <= sync_ctr > 0;
		...
		if (~axiResetN) begin
			...
			if (cfgReadAddress < 20)
				...
				for (byteIterR = 0; byteIterR < (SAXIL_CFG_DATA_WIDTH/8); byteIterR = byteIterR + 1)
					...
					cfgReadDataR[SAXIL_CFG_DATA_WIDTH - 1 - byteIterR*8-:8] <= outputBuffer[159-((byteIterR + cfgReadAddress)*8) -: 8];
			else if (cfgReadAddress < 40)
				// read device settings rdbk
				for (byteIterR = 0; byteIterR < (SAXIL_CFG_DATA_WIDTH/8); byteIterR = byteIterR + 1)
					// swap bit order for more intuitive r/w from host and spi outpu
					cfgReadDataR[SAXIL_CFG_DATA_WIDTH - 1 - byteIterR*8-:8] <= readbackBuf[159-((byteIterR + cfgReadAddress - 20)*8) -: 8];
			else case (cfgReadAddress)
				44: cfgReadDataR <= 32'hBC1D_C10C;
				default: cfgReadDataR <= 32'h1331_1331;
			endcase
		
			if (cfgActiveWriteAddress < 20)
				for (byteIterW = 0; byteIterW < (SAXIL_CFG_DATA_WIDTH/8); byteIterW = byteIterW + 1)
					// swap byte order for more intuitive r/w from host
					outputBuffer[159-((byteIterW + cfgActiveWriteAddress)*8) -: 8] <= cfgWriteData[SAXIL_CFG_DATA_WIDTH - 1 - byteIterW*8-:8];
			else if (cfgActiveWriteAddress == 24)
				outputBuffer <= DEFAULT_SETTINGS;
			...
			if (cfgInProgress)
				...
				cfgReq <= 0;
			else if (cfgActiveWriteAddress == 20) begin
				// axi cfg req
				cfgReqRd <= cfgWriteData[16];
				cfgAddrReq <= cfgWriteData[7:0];
				cfgSzReqBits <= cfgWriteData[15:8];
				cfgReq <= 1;
			end
		end
	end
