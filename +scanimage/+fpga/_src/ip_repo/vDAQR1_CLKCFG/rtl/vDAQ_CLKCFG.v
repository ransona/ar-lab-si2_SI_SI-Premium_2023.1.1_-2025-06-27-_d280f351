...
module vDAQ_CLKCFG #(
	...
	// clock selector
	output wire [1:0] extClockSelect,
	...
	reg [1:0] extClockSelectR = 0;
	assign extClockSelect = extClockSelectR;
		...
		if (~axiResetN) begin
			...
			if (~cfgInProgress) begin
				...
				if (cfgReqRd)
					...
					sdoR <= outputBuffer[159-(cfgAddrReq*8+bitCtr)];
	...
	always @(posedge clk40) begin
		...
		if (cfgInProgress && cfgReqRd && addrDone && (spiClkCtr == 1))
			readbackBuf[159-(cfgAddrReq*8+bitCtr-1)] <= sdi;
	end
	...
	always @(posedge clk40) begin
		...
		if (~axiResetN) begin
			...
			if (s0WriteActive)
				case (s0WriteAddress)
					40: extClockSelectR <= s0WriteData;
				//	64: sync cmd
				endcase
			...
			if (cfgInProgress)
				...
				cfgReq <= 0;
			else if (s0WriteActive && (s0WriteAddress == 20)) begin
				// axi cfg req
