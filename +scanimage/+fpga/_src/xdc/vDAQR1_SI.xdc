...
set pin_i1 [get_pins afeClk_mux/I1]
set pin_o  [get_pins afeClk_mux/O]

set ms_clk [get_clocks -of_objects $pin_i0]
set hs_clk [get_clocks -of_objects $pin_i1]

set ms_afe_clk [get_clocks -of_objects $pin_o -filter "MASTER_CLOCK == $ms_clk"]
...
create_generated_clock -name ms_trig_phy_clk -multiply_by 4 -add -master_clock $ms_afe_clk -source $inClkPin $phyClkPin
create_generated_clock -name ms_trig_div_clk -divide_by 1 -add -master_clock $ms_afe_clk -source $inClkPin $sampClkPin
...
# exclusivity for muxed afe and sync trigger sample clks
set all_ms_mux_clks [get_clocks "[get_clocks -filter "MASTER_CLOCK == $ms_afe_clk"] $ms_afe_clk $ms_clk"]
set all_hs_mux_clks [get_clocks "[get_clocks -filter "MASTER_CLOCK == $hs_afe_clk"] $hs_afe_clk $hs_clk"]

set_clock_groups -physically_exclusive -group $all_ms_mux_clks -group $all_hs_mux_clks

set_false_path -from $all_ms_mux_clks -to $all_hs_mux_clks
set_false_path -from $all_hs_mux_clks -to $all_ms_mux_clks
set_false_path -through [get_pins vDAQ_BD_inst/msadcSampleData] -to $all_hs_mux_clks
