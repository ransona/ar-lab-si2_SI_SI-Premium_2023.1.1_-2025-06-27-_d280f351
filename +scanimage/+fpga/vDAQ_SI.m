classdef vDAQ_SI < dabs.vidrio.rdi.Device
    ...
    properties (Constant, Hidden)
        ...
        DIO_BASE_ADDRESS = 400;
        GIT_HASH_ADDRESS = 300;
    end
    ...
    methods
        function obj = vDAQ_SI(dev,simulate)
            ...
            if simulate
                obj.ACCUM_BITS = uint32(12);
                obj.ACCUM_COEFFICIENT_FIXED_POINT_WIDTH = uint32(26);
            end
        end
        ...
        function s = getRegMap(obj)
            ...
            s.dataRegs.ACCUM_COEFFICIENT_FIXED_POINT_WIDTH = struct('address',4194304+200,'hide',true);
            s.dataRegs.ACCUM_BITS = struct('address',4194304+204,'hide',true);

            for i = 0:3
                s.dataRegs.(['gitHash_' num2str(i)]) = struct('address',4194304+obj.GIT_HASH_ADDRESS+i*4,'hide',true);
            end
            
            ndio = 39 + 8*obj.isR1;
            for i = 0:ndio
                s.dataRegs.(['digital_o_' num2str(i)]) = struct('address',4194304+obj.DIO_BASE_ADDRESS+i*4,'hide',true);
            end
        end
        
        function delete(obj)
            if isprop(obj.hClockCfg,'hPLLCheckTimer')
                most.idioms.safeDeleteObj(obj.hClockCfg.hPLLCheckTimer);
            end
            delete(obj.hWaveGen);
            delete(obj.hWaveAcq);
    ...
    methods
        ...
        function actualPhase_deg = setMsadcSamplingPhase(obj, phase_deg)
            ...
            % 1. reset clock phase (temporary glitch)
            [~, actualPhase_deg] = obj.hClockCfg.setClkPhase(1, phase_deg);
            pause(0.05);

            % 2. reinit ADC and acq engine
            obj.initMsadc();
        end
        ...
        function actualDelay_ps = setMsadcSamplingDelay(obj, delay_ps)
            ...
            actualPhase_deg = obj.setMsadcSamplingPhase(phase_deg);
        ...
        function tfAlive = isPCIeLinkAlive(obj)
            lastT = obj.T;
            invalid = intmax('uint32');
            notAlive = obj.T == invalid && lastT == invalid && obj.NUM_AE == invalid;
            tfAlive = obj.T ~= lastT || ~notAlive;
        end
    end
    ...
    methods
        ...
        function val = get.bitfileGitHash(obj)
            val = '';
            for i = 0:3
                word = obj.(['gitHash_' num2str(i)]);
                if word == uint32(1337)
                    val = 'NO HASH PRESENT';
                    return
                end
                if word == uint32(hex2dec('FFFFFFFF'))
                    val = 'VDAQ NOT CONNECTED';
                    return
                end

                bytes = typecast(word,'uint8');
                val = [val char(bytes)];
            end
            val = strip(val);
            val = erase(val,char(0));
            val = fliplr(val);
        end
    end
