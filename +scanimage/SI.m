classdef SI < scanimage.interfaces.Component & most.HasMachineDataFile & dynamicprops & dabs.resources.configuration.HasConfigPage & dabs.resources.widget.HasWidget
    ...
    properties (SetObservable, Hidden, Transient)
        ...
        applicationOpen = false;                % Boolean for whether the GUIs have been initialized yet.
    end
    
    %Properties to include in stripe data apart from aquisitionMex
    ...
    properties(Transient,Constant)
        ...
        VERSION_MAJOR  = 2023;     % Version number
        VERSION_MINOR  = 1;        % Minor release number  (0 = the initial release; positive numbers = maintenance releases)
        VERSION_UPDATE = 1;        % Bugfix release number (0 = the initial release; positive numbers = bugfix releases)
    ...
    properties (Hidden)
        hAutoRestartAcqListener = most.util.DelayedEventListener.empty();       % most.util.DelayedEventListener listening for when AcquisitionMex drops frames and aborts the acquisition and allows to automatically restart the acquisition.
        tfAutoRestartFailedAcquisitions = false;                                % Logical scalar for whether acquisitions should be automatically restarted after improbable acquisition failure (for unattended/overnight acquisition).
    end
    ...
    methods (Hidden)
        ...
        function reinit(obj)
            try
                ...
                assert(hLM.licensed,'No valid license for ScanImage found.');
        ...
        function delete(obj)
            ...
            if most.idioms.isValidObj(obj.hAutoRestartAcqListener)
                most.idioms.safeDeleteObj(obj.hAutoRestartAcqListener);
                obj.hAutoRestartAcqListener = most.util.DelayedEventListener.empty();
            end
        end
    end
    ...
    methods
        ...
        function set.imagingSystem(obj,val)
            if obj.componentUpdateProperty('imagingSystem',val)
                ...
                try
                    ...
                    if ~isempty(mode)
                        obj.hScan2D.isPolygonalScanning = isprop(obj.hScan2D, 'hResonantScanner') && isa(obj.hScan2D.hResonantScanner,'dabs.resources.devices.PolygonalScanner') && strcmpi(mode,'resonant');
        ...
        function set.tfAutoRestartFailedAcquisitions(obj,v)
            validateattributes(v,{'logical'},{'binary'});
            
            oldVal = obj.tfAutoRestartFailedAcquisitions;
            if oldVal == v
                return;
            end
            
            obj.tfAutoRestartFailedAcquisitions = v;
            
            if v
                obj.hAutoRestartAcqListener(end+1) = most.util.DelayedEventListener(10,obj.hUserFunctions,'lostFrames',@(src,evt)obj.restartAcq(evt{1}.UserData));
            else
                most.idioms.safeDeleteObj(obj.hAutoRestartAcqListener);
                obj.hAutoRestartAcqListener = most.util.DelayedEventListener.empty();
            end
    ...
    methods (Static)
        ...
        function copyrightInfo = getScanImageCopyrightInfo()
            copyrightPath = fullfile(scanimage.util.siRootDir(),'ScanImage License Info.txt');
            copyrightInfo = fileread(copyrightPath);
        end
        
        function isSICodeFile = isScanImageSourceCodeFile(filePath)
            if exist(filePath,'file') ~= 2
                isSICodeFile = false;
                return
            end
            
            % get copyright license info and determine search length
            try
                copyrightInfo = scanimage.SI.getScanImageCopyrightInfo();
            catch ME
                isSICodeFile = false;
                return
            end
            copyrightLength = numel(copyrightInfo);
            searchBeforeEndLength = copyrightLength + 200; % add enough for extra characters
            
            % get licenses line-by-line
            copyrightLines = conditionLicenseLines(strsplit(copyrightInfo,newline()));
            licenseFromFileLines = loadLicenseFromFile(filePath,searchBeforeEndLength);
            
            % check line-by-line to see if any license data matches
            licenseLinesFound = cellfun(@(line)any(strcmp(line,licenseFromFileLines)),copyrightLines);
            isSICodeFile = any(licenseLinesFound);
            
            %%% Nested functions
            function licenseLines = loadLicenseFromFile(filePath,searchBeforeEndLength)
                fid = fopen(filePath,'r');
                try
                    fseek(fid,0,'eof');
                    if ftell(fid) < searchBeforeEndLength
                        licenseLines = {};
                        close(fid);
                        return
                    end
                    fseek(fid,-searchBeforeEndLength,'cof');
                    data = fread(fid,Inf,'*char')';
                    licenseLines = strsplit(data,newline());
                    licenseLines = conditionLicenseLines(licenseLines);
                catch ME
                    fclose(fid);
                    ME.rethrow();
                end
            ...
            function lines = conditionLicenseLines(lines)
                for i = 1:numel(lines)
                    % condition line edges so licenses match up
                    lines(i) = strtrim(lines(i));
                    lines(i) = strip(lines(i),'left','%');
                    lines(i) = strtrim(lines(i));
                end
    ...
    methods
        function str = getHeaderString(obj,customProps)
            ...
            if obj.useJsonHeaderFormat
                ...
                if ~isempty(customProps)
                    str = strrep(obj.mdlGetHeaderString('include',customProps,16),'scanimage.SI.','SI.');
                else
                    str = strrep(obj.mdlGetHeaderString([],[],16),'scanimage.SI.','SI.');
                end
            end
        end
        ...
        function restartAcq(obj,acqType)
            % This function pauses some time and restarts acquisition if AcquisitionMex reports
            % acqStatus.numDroppedFramesLogger > 0.
            persistent numRestarts
            if isempty(numRestarts)
                numRestarts = 0;
            end
            
            if numRestarts >= 4
                most.idioms.warn(sprintf('%s%s','Frames were dropped in 5 consecutive acquisitions. Check disk space or reduce data rate. Last acquisition stopped at: ', datetime('now')));
                numRestarts = 0;
                return
            end
            
            t = tic();
            while obj.active % This shouldn't happen. Scan is aborted before notifying lostFrames
                pause(1);
                if toc(t) > 10
                    most.idioms.warn(sprintf('%s%s','Acquisition remained active for 10 seconds while trying to restart an acquisition. Canceling restart.'));
                    return;
                end
            end
            
            if strcmpi(acqType,'grab')
                obj.startGrab();
            else
                obj.startLoop();
            end
        ...
        function zzzFrameAcquiredFcn(obj,~,~) % Executes on Every Stripe as well.
            try
                ...
                while processFrameBatch && toc(loopStart) <= maxBatchTime
                    ...
                    stripeData.focalPoint = obj.focalPoint;
                    stripeData.stackMode = obj.stackMode;
