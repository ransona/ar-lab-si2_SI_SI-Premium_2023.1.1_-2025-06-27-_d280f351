classdef constrastSlider < handle
    ...
    methods
        function obj = constrastSlider(varargin)
            ...
            ip.addOptional('BarColorL','k');
            ip.addOptional('BarColorH','w');
            ip.parse(varargin{:});
            ...
            obj.hBarL = patch('parent',obj.hAx,'xdata',[],'ydata',[],'zdata',[],'FaceColor',ip.Results.BarColorL,'LineWidth',1.5,'edgecolor',ip.Results.BorderColor);
            obj.hBarH = patch('parent',obj.hAx,'xdata',[],'ydata',[],'zdata',[],'FaceColor',ip.Results.BarColorH,'LineWidth',1.5,'edgecolor',ip.Results.BorderColor);
            ...
            if ~isempty(ip.Results.DarkColor) && ~isempty(ip.Results.BrightColor)
                ...
                obj.hCarL.FaceColor = dc;
                obj.hCarH.FaceColor = bc;
            end
            
            if isempty(ip.Results.BarColorL) || isempty(ip.Results.BarColorH)
                obj.hBarL.FaceColor = ip.Results.BarColor;
                obj.hBarH.FaceColor = ip.Results.BarColor;
            end
    ...
    methods (Hidden)
        ...
        function drawCars(obj)
            if most.idioms.isValidObj(obj.hCar)
                ...
                %barL
                xs = [-.5*barT*sind(0:10:180) p(3)*v(1) p(3)*v(1)];
                ys = p(4)/2+ [.5*barT*cosd(0:10:180) -.5*barT, .5*barT];
                obj.hBarL.XData = xs/p(3);
                obj.hBarL.YData = ys/p(4);
                obj.hBarL.ZData = [];
                
                %barH
                xs = [p(3)*v(2) p(3)*v(2) (p(3) + .5*barT*sind(0:10:180))];
                ys = p(4)/2 + [-0.5*barT 0.5*barT .5*barT*cosd(0:10:180)];
                obj.hBarH.XData = xs/p(3);
                obj.hBarH.YData = ys/p(4);
                obj.hBarH.ZData = [];
            end
        end
    ...
    methods
        ...
        function setCarColors(obj,dc,bc)
            colormap(obj.hAx,[linspace(dc(1),bc(1),101)' linspace(dc(2),bc(2),101)' linspace(dc(3),bc(3),101)']);
            obj.hCar.FaceColor = 'texturemap';
            obj.hCar.CData = 0:.01:1;
            obj.hBarH.FaceColor = bc;
