classdef uicontrolLayout < handle
    ...
    methods (Access = private)
        function initFig(obj)
            obj.hFig = most.idioms.figure( ...
                'Name', 'Layout' ...
                ,'CloseRequestFcn',@(varargin)deactivateLayoutMode ...
                ,'Visible','off');
        ...
        function changedPosition(obj)
            if isempty(obj.currentUiControl)
                ...
                pos = round(pos,6); % only keep 6 decimal values to prevent e.g. 0.9999999999999...
                posStr = mat2str(pos);
            end
            ...
            if ~isempty(obj.currentUiControl) && isvalid(obj.currentUiControl)
                if ~isequal(obj.currentUiControl.hCtl.Position,obj.undoInfo.Position)
                    try
                        ...
                        most.idioms.figure(obj.currentFig);
                    catch ME
    ...
    methods
        ...
        function set.currentFig(obj,val)
            ...
            if most.idioms.isValidObj(oldFig) && ~isempty(oldFigInfo)
                oldFig.WindowKeyPressFcn = oldFigInfo.WindowKeyPressFcn;
            end
            
            if most.idioms.isValidObj(obj.currentFig)
