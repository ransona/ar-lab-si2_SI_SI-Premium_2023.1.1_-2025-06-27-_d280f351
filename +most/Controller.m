classdef Controller < most.DClass & most.HasClassDataFile & dynamicprops
    ...
    methods (Access = protected)
        function prepareDelete(obj)
            ...
            obj.hGUIBindingPostSetListeners = struct();
            structfun(@delete,obj.hCallbackPostSetListeners);
            obj.hCallbackPostSetListeners = struct();
        end
    end
    ...
    methods
        ...
        function raiseGUI(obj, guiObject) %#ok<MANU>
            % raiseGUI   make gui visible and raise to top
            %
            %    obj.raiseGUI(guiObject)   raises gui given by 'guiObject'. guiObject is either the name of the gui or a gui handle
            
            if most.idioms.isstring(guiObject)
                guiObject = obj.hGUIs.(guiObject);
            end

            switch get(guiObject,'Visible')
                case 'on'
                    most.idioms.figure(guiObject);
            end
        end
        
        function showGUI(obj,guiObject,~)
            % showGUI   make gui visible
            %
            %    obj.showGUI(guiObject)   shows gui given by 'guiObject'. guiObject is either the name of the gui or a gui handle
            
            if ~ishandle(guiObject)
                assert(most.idioms.isstring(guiObject));
                guiObject = obj.hGUIs.(guiObject);
            end
            set(guiObject,'Visible','on')
            most.idioms.figure(guiObject);
        end
